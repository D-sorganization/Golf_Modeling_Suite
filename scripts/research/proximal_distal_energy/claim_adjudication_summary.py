...
from .claim_audit import build_candidate_inventory, write_candidate_inventory
...
def _write_summary_files(root: Path) -> dict[str, Any]:
...
def _current_inventory(root: Path, registry: dict[str, Any]) -> dict[str, Any]:
...
def _validate_projection_coverage(
    ...
    if unknown:
        raise ValueError(
            f"Claim reviews contain stale candidate IDs: {sorted(unknown)}"
        )
    missing = candidate_ids - set(reviews)
    if missing:
        raise ValueError(
            f"Claim inventory contains unreviewed candidates: {sorted(missing)}"
        )
    for candidate in inventory["candidates"]:
        if candidate["source_path"] != SUMMARY_SOURCE:
            continue
        review = reviews[candidate["candidate_id"]]
        if review["disposition"] != "editorial_or_navigation":
            raise ValueError(
                f"{candidate['candidate_id']}: generated summary must remain editorial"
            )
        if review.get("claim_ids"):
            raise ValueError(
                f"{candidate['candidate_id']}: generated summary cannot map new claims"
            )


def _reconcile_generated_projection(root: Path) -> None:
    data = root / "docs/research/proximal_distal_energy_transfer/data"
    registry_path = data / "claim_audit_registry.json"
    inventory_path = data / "claim_candidate_inventory.json"
    registry: dict[str, Any] = json.loads(registry_path.read_text(encoding="utf-8"))
    previous_inventory: dict[str, Any] = json.loads(
        inventory_path.read_text(encoding="utf-8")
    )
    previous_candidates = {
        item["candidate_id"]: item for item in previous_inventory["candidates"]
    }
    inventory = _current_inventory(root, registry)
    candidate_ids = {item["candidate_id"] for item in inventory["candidates"]}
    reviews = {item["candidate_id"]: item for item in registry["candidate_reviews"]}

    stale_ids = set(reviews) - candidate_ids
    for candidate_id in stale_ids:
        review = reviews[candidate_id]
        previous = previous_candidates.get(candidate_id)
        if (
            review.get("rationale") != SUMMARY_PROJECTION_RATIONALE
            or previous is None
            or previous.get("source_path") != SUMMARY_SOURCE
        ):
            raise ValueError(
                f"Reviewed non-projection candidate disappeared: {candidate_id}"
            )
        del reviews[candidate_id]
    for claim in registry["claims"]:
        stale_claim_links = set(claim.get("candidate_ids", [])) & stale_ids
        if stale_claim_links:
            raise ValueError(
                f"{claim['claim_id']}: generated projection was mapped as evidence"
            )

    candidates = {item["candidate_id"]: item for item in inventory["candidates"]}
    missing_ids = candidate_ids - set(reviews)
    unexpected = {
        candidate_id
        for candidate_id in missing_ids
        if candidates[candidate_id]["source_path"] != SUMMARY_SOURCE
    }
    if unexpected:
        raise ValueError(
            "Only generated reviewer-summary candidates may be auto-adjudicated: "
            f"{sorted(unexpected)}"
        )
    for candidate_id in sorted(missing_ids):
        reviews[candidate_id] = {
            ...
            "claim_ids": [],
            "rationale": SUMMARY_PROJECTION_RATIONALE,
    ...
    registry["audit_scope"]["current_scope"] = (
        ...
        "adjudicated. Repeated methods, summary, limitation, provenance, and "
        "model-tier passages inherit the primary claim boundaries; generated "
        "reviewer tables and editorial anchors are explicitly classified as "
        "nonclaims."
    )
    _validate_projection_coverage(registry, inventory)
    registry_path.write_text(json.dumps(registry, indent=2) + "\n", encoding="utf-8")
    write_candidate_inventory(inventory_path, inventory)


def write_summary(root: Path) -> dict[str, Any]:
    _write_summary_files(root)
    _reconcile_generated_projection(root)
    return _write_summary_files(root)
...
def validate_summary(root: Path) -> dict[str, Any]:
    ...
    inventory = _current_inventory(root, registry)
    if inventory["source_digest"] != registry["paper"]["source_digest"]:
        raise ValueError("Generated reviewer projection is stale in the claim audit")
    _validate_projection_coverage(registry, inventory)
    return expected

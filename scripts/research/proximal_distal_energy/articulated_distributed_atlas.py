    ...
    for station_slot, station_count in enumerate(config.station_counts):
        ...
        for velocity_slot, factor in enumerate((1.0, -1.0)):
            for step_slot, step in enumerate(config.forward.time_steps_s):
                ...
                _record_pair(
                    buffers,
                    (state_slot, station_slot, velocity_slot, step_slot),
                    traces,
                    config,
                    station_count,
                )
...
def _gates(buffers: _Buffers, config: DistributedAtlasConfig) -> dict[str, Any]:
    ...
    refinement = np.max(buffers.normalized_energy_residual, axis=(0, 1, 2, 4, 5))
    time_passed = bool(
...
def _record(
    ...
    return {
        "schema_version": "articulated-distributed-grip-atlas/v1",
        "study_id": "distributed-grip-contact-discretization",
        "design": {
        ...
        "claim_boundary": {
            "supported": "distributed tension-fiber discretization can be compared without silently increasing total stiffness or reinitializing horizons",
            "measured_pressure_or_finger_anatomy": "not_identified",
            "friction_tissue_shaft_or_ground": "not_modeled",

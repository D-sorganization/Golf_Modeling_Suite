...
def club_com_acceleration(
    inertials: PlanarInertials,
    q: np.ndarray,
    v: np.ndarray,
    qdd: np.ndarray,
) -> np.ndarray:
    """Exact club-segment COM acceleration from joint state and acceleration.

    This evaluates ``J(q) qdd + Jdot(q, v) v`` analytically. It avoids
    differentiating sampled velocities and therefore remains well defined at
    ...
    omega_club = v[:, 0] + v[:, 1]
    alpha1 = qdd[:, 0]
    alpha_club = qdd[:, 0] + qdd[:, 1]
    return inertials.l1 * (
        alpha1[:, None] * _unit_perp(theta1) - omega1[:, None] ** 2 * _unit(theta1)
    ) + inertials.lc2 * (
        alpha_club[:, None] * _unit_perp(theta_club)
        - omega_club[:, None] ** 2 * _unit(theta_club)
    )
...
def wrist_interface_powers(
    ...
    - ``joint_force_power``: joint reaction force on the club, dotted with
      ...
      force). The club centre-of-mass acceleration is evaluated analytically
      as ``J qdd + Jdot qdot``.
    ...
    - ``club_energy_rate``: analytic rate of the club's total mechanical
      (kinetic plus potential) energy. It must equal joint-force power plus
      interface-moment power pointwise.
    """
    omega_rel = v[:, 1]
    omega_club = v[:, 0] + v[:, 1]
    tau_wrist = u[:, 1]
    ...
    v_com = club_com_velocity(inertials, q, v)
    if not (len(t) == len(q) == len(v) == len(qdd) == len(u)):
        raise ValueError("t, q, v, qdd, and u must have equal sample counts")
    a_com = club_com_acceleration(inertials, q, v, qdd)
    ...
    kinetic_energy_rate = inertials.m2 * np.sum(v_com * a_com, axis=1) + (
        inertials.i2_com * omega_club * (qdd[:, 0] + qdd[:, 1])
    )
    club_energy_rate = kinetic_energy_rate - gravity_power_on_club
    ...
    return {
        ...
        "club_kinetic_energy_rate": kinetic_energy_rate,
...
def find_impact(
    ...
    inertials: PlanarInertials,
    *,
    max_arm_angle_rad: float | None = MAX_ARM_ANGLE_AT_IMPACT_RAD,
    ...
    rotated past ``max_arm_angle_rad`` when the club gets there (outside the
    registered delivery zone), return ``None``. Pass ``None`` to apply no
    angular registration rule.
    """
    candidate = first_club_vertical_crossing(t, q, v, inertials)
    if candidate is None:
        return None
    if max_arm_angle_rad is not None and candidate[2] > max_arm_angle_rad:
        return None
    return candidate


def first_club_vertical_crossing(
    t: np.ndarray,
    q: np.ndarray,
    v: np.ndarray,
    inertials: PlanarInertials,
) -> tuple[float, float, float] | None:
    """Return the first geometric club-vertical crossing without scoring it.

    Separating event detection from the registered delivery-zone criterion
    makes every attempted program auditable, including crossings excluded from

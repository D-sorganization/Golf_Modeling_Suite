...
Usage:
    python scripts/validate_physics.py [--engine {mujoco,drake,pinocchio,all}] [--type {analytical,complex,all}]
"""
...
def run_tests(engine_filter: str, test_type: str) -> bool:
    ...
    # If engine filter is specific, we might skip tests (handled by valid skipping in pytest files)
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            logger.error(f"X Physics Validation FAILED (Exit Code: {result.returncode})")
            return False

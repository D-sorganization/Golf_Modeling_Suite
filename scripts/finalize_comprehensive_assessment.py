...
def load_general_data():
    ...
    try:
        ...
        logger.error(f"Summary JSON is malformed: {e}")
        return None
...
def load_pragmatic_data():
    ...
    if not PRAGMATIC_REPORT.exists():
        ...
        return {"issues": []}
    try:
        with open(PRAGMATIC_REPORT, encoding="utf-8") as f:
            return json.load(f)
    except json.JSONDecodeError as e:
        logger.warning(f"Pragmatic report is malformed: {e}")
        return {"issues": []}
...
def main():
    ...
    if not isinstance(pragmatic_issues, list):
        logger.warning("Pragmatic report issues were not a list; ignoring content")
        pragmatic_issues = []

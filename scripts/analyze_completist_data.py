...
def create_issue_file(item: Mapping[str, Any], issue_id: int) -> str:
    ...
    for existing_file in glob.glob(os.path.join(ISSUES_DIR, "Issue_*.md")):
        ...
        if (f"`{f_path}" in content or f"`./{clean_f_path}" in content or f"`{clean_f_path}" in content) and f"```python\n{context}\n```" in content:
            ...
            new_content = re.sub(r"(:)`" + re.escape(f_path) + r":\d+`", r"\g<1>`" + f_path + f":{l_no}`", new_content)
            new_content = re.sub(r"(:)`\./" + re.escape(clean_f_path) + r":\d+`", r"\g<1>`./" + clean_f_path + f":{l_no}`", new_content)
            new_content = re.sub(r"title: \"Incomplete .*?\"", f"title: \"{title}\"", new_content)
            ...
            if match_id:
                ...
                new_filepath = os.path.join(ISSUES_DIR, f"Issue_{issue_id_str}_{fname_title}.md")
...
def generate_report() -> None:
    ...
    def is_abstract(finding: Finding) -> bool:
        ...
        if f not in abstracts_by_file:
            return False
        l = int(finding["line"])
        return any(0 <= l - al <= 5 for al in abstracts_by_file[f])

    criticals = [s for s in (stubs + ni_errors) if "test" not in s["file"].lower() and not is_abstract(s)]

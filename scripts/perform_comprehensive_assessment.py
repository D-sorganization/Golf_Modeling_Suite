...
def generate_completist_report():
    ...
    prompt_content = read_file(os.path.join(ARCHIVE_PROMPTS_DIR, "Assessment_Prompt_Completist.md"))
    output_format = extract_output_format(prompt_content)
    if not output_format:
        # Fallback if specific prompt not found
...
def assess_category(category_char, category_name, codebase_stats):
    # Try looking in both archive and current location
    ...
    summary = []
    findings = []
    score = 8  # Baseline, adjusted by rules
    ...
    if category_char == 'A':  # Architecture
        ...
        perf_imports = [i for i in codebase_stats['imports'] if i in ['cProfile', 'timeit', 'multiprocessing']]
        summary.append(f"Performance tools detected: {', '.join(perf_imports) if perf_imports else 'None'}.")
        ...
        summary.append(f"Setup files: setup.py/pyproject.toml={'Yes' if has_setup else 'No'}, requirements.txt={'Yes' if has_reqs else 'No'}.")
        ...
        summary.append(f"Try/Except blocks: {total_try} across {total_funcs} functions.")

    elif category_char == 'I':  # Security
        risky = [i for i in codebase_stats['imports'] if i in ['subprocess', 'pickle', 'os']]
        ...
        if avg_loc > 200:
            findings.append("Average file size is high, suggesting potential refactoring needs.")
            score = 6
        ...
        viz_imports = [i for i in codebase_stats['imports'] if i in ['matplotlib', 'plotly', 'seaborn', 'altair']]
    ...
    # Preserve placeholders for human review if they exist, but append automated data
    ...
    # Do NOT replace placeholders with "See summary details" blindly
    # report = re.sub(r"\[.*?\]", "See summary details.", report)

    # Calculate score based on findings length (heuristic)

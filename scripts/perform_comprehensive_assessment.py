...
def collect_codebase_stats():
    ...
    for filepath in get_files("**/*.py"):
        if "venv" in filepath or "site-packages" in filepath: continue

        file_stats = analyze_file(filepath)
        if not file_stats: continue
...
def extract_output_format(prompt_content):
    ...
    if not section:
        pattern = re.compile(f"## Output (Template|Format)(.*?)(?=\n## |\Z)", re.DOTALL)
...
def generate_completist_report():
    ...
    if not output_format: output_format = "# Assessment: Completist Audit\n..."
...
def assess_category(category_char, category_name, codebase_stats):
    ...
    if category_char == 'A': # Architecture
        ...
        findings.append(f"High TODO density indicates technical debt.")

...
| SK-START-017 | Built wheel is installed outside the checkout    | Direct, `shared.python`, and legacy `src.shared.python` spellings share parent-owned module identities; standalone CLI executes | Import-alias architecture tests, build-hook and console-script packaging suites                                                                                     | **Candidate pass / final repin pending** — the exact candidate wheel now passes identity, dependency-closure, byte-fidelity, and `python -m sidekick --help` probes; rerun after the protected Tools merge is pinned |

...
startup/recovery issue #8102, Tools #3936, #3938, and #3950, the selected-Tools
API-child defect #8120, Diagnostics provider-manifest defect #8121, and merged
Tools PR #3937 (`4744422d3`).
...
| SK-START-017 | Built wheel is installed outside the checkout     | Direct, `shared.python`, and legacy `src.shared.python` spellings share parent-owned module identities; standalone CLI executes                         | Import-alias architecture tests, build-hook and console-script packaging suites                                                                                     | **Artifact and candidate GUI pass / final repin pending** — clean-wheel alias smoke, native CLI calculation, and computer-controlled classic PyQt6 acceptance pass; merge Tools #3944 and repin its protected merge before closure |
| SK-START-018 | Units loads while deprecations are errors         | The real converter renders all six rows and converts valid values without importing a deprecated global state manager                                   | Tools `test_state_manager_side_effects.py` warnings-as-errors subprocess regression and calculator-mixin tests                                                      | **Pass after Tools #3950 fix** — the initial sweep rendered only `SidekickPlaceholder`; the repaired candidate rendered all six rows and converted `100 °C` to `212 °F`                                                            |
| SK-START-019 | API starts from an explicitly selected Tools tree | The background API child inherits the same validated Tools roots as the sidebar, ordered ahead of partial Upstream packages                             | `test_background_api_child_uses_explicit_tools_package_roots`, `test_get_subprocess_env_prioritizes_selected_provider_paths`, and exact candidate import probe      | **Pass after #8120 fix** — the initial child crashed on `ModuleNotFoundError: chat.websocket_protocol`; the repaired candidate connected on its dynamic API port using the selected Tools authority                                |
| SK-START-020 | Diagnostics runs with provider model packs        | Parent `models.yaml` validation and provider-expanded runtime-registry validation remain separate; a healthy install has no false missing-model failure | `TestProviderExpandedDiagnostics` plus the launcher diagnostics suites                                                                                              | **Pass after #8121 fix** — the initial screen reported 29 provider IDs missing and `Status: DEGRADED`; the repaired screen reported 46 parent models, 75 runtime models, `0 failed`, and `Status: HEALTHY`                         |
...
9. Open Units, confirm all six conversion rows render, and convert `100 °C`
   to `212 °F`.
10. Confirm the background API imports `chat.websocket_protocol` from the same
    selected Tools checkout as the sidebar, not an UpstreamDrift partial copy.
11. Stop only the launcher's verified API process tree after Chat connects.
    Confirm Chat reports the API unavailable, a new API child and dynamic
    listener appear, and Chat reconnects.
12. Close the launcher, verify its child API exits, and confirm the unrelated
    port-8000 process is untouched.
13. Record pass/fail, screenshot evidence, tested SHAs, and any new issue
...
- A clean wheel built from the exact pin passed the installed artifact alias
  smoke outside the checkout. Direct, `shared.python`, and legacy
  `src.shared.python` imports shared the parent-owned graph.
- The native Windows `sidekick.exe` build completed at 241.5 MB, passed
  `--help`, and executed the release-equivalent WGS calculator smoke with valid
  JSON output.
- Computer-controlled classic PyQt6 testing exposed and repaired four additional
  startup/interaction defects: approved C3D extensions were activated after
  embeddable bootstrap, the Sidekick tile launched an inert adapter subprocess,
  the active fleet theme lacked `success_hover`, and saved-session restoration
  emitted before Tools message controllers existed.
- With the Upstream fixes and Tools #3944 candidate head
  `f075ff713f67ee582907bdedd7f3bb32cacbcee8`, the launcher started with 19
  embeddable tools, restored an existing Sidekick session without a crash,
  evaluated `2 + 2` to `4` in the visible Calculator tab, hid the sidebar, and
  reopened Chat by double-clicking the Sidekick tile. The deliberately
  unavailable local API remained a visible Chat-only degraded state while local
  tools stayed usable.
- The source standalone command `python -m sidekick gui --skip-onboarding`
  opened a visible Sidekick window with the same saved-session candidate and
  exited cleanly through the window close control. The packaged GUI must be
  rebuilt from the eventual protected pin before this becomes a final artifact
  pass.
- Tools #3944 must still pass protected checks and merge. UpstreamDrift must
  repin that exact protected merge and repeat the source, wheel, binary, and
  computer-controlled gates before #8102 is marked complete.

## 2026-07-27 extended screen-control and child-authority result

- The full default-tab sweep exposed a Tools-owned Units failure under
  UpstreamDrift's deprecations-as-errors policy. The tab silently fell back to
  `SidekickPlaceholder` because the calculator mixin imported the deprecated
  global `state_manager`. Tools #3950 replaces that access with
  `get_state_manager()` and adds a warnings-as-errors subprocess regression.
  The repaired candidate rendered all six conversion rows and converted
  `100 °C` to `212 °F`.
- The background API child initially omitted the selected Tools source roots
  from its `PYTHONPATH` and crashed with
  `ModuleNotFoundError: No module named 'chat.websocket_protocol'`
  (#8120). The repaired candidate resolves one validated Tools
  authority for both the sidebar and API child, passes its
  `src/shared/python` and `src` roots explicitly, and orders them before
  UpstreamDrift's partial shared packages. The focused launcher suites passed
  71 tests, and an exact candidate probe imported `chat.websocket_protocol`
  from Tools before successfully importing `src.api.server`.
- A final fault-injection run left an unrelated blocker on port 8000, connected
  Chat through the launcher's dynamic API port, then terminated the complete
  verified API process tree. The liveness monitor spawned a replacement API
  and Chat returned to `Connected`. Closing through the PyQt6 window removed
  the launcher and API descendants while the unrelated port-8000 blocker
  remained alive, satisfying SK-START-002, SK-START-013, and SK-START-016
  together.
- The broader launcher sweep passed search/clear, Engines and Library
  navigation, and the Settings, Configuration, MCP Servers, Appearance,
  Startup, Notifications, and Performance surfaces. Diagnostics initially
  compared the 75-entry provider-expanded registry to the 46 parent
  `models.yaml` entries and falsely failed all 29 provider-only IDs (#8121).
  After separating those contracts, computer control reported 46 parent
  models, 75 runtime models, `0 failed`, and `Status: HEALTHY`.

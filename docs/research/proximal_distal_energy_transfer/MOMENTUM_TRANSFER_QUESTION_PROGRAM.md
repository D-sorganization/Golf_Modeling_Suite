...
| Q2  | What are the geometry dependencies?                                                                                    | **Partly answered.** Relative link angle, force direction, velocity direction, grip separation, reference point, swing-plane orientation, and contact moment arm gate sign and pathway. Force magnitude alone is insufficient. | Analytical sine/cosine gates, force–velocity projection, coincident/reversed grip controls, moving-base and flexible-club studies, reduced two-engine spatial contact. | No subject-scaled arm/scapular geometry, calibrated grip compliance, distributed 3-D shaft, or population atlas.                                                 | Factorial geometry atlas with sign-reversing/null controls, subject-scaled articulated arms, independent engines, and held-out digital twins.                                                                                                          |
...
| Q4  | Does passive or drift-mediated transfer require less timing precision?                                                 | **Unresolved for humans.** A state trigger can outperform a clock trigger for selected synthetic objectives, but that is not evidence of lower human timing demand.                                                            | Held-out engineering perturbations, clock/state trigger comparison, lower-tail speed and orientation proxies.                                                          | Perfect state access, no observer delay/noise, no neural delay, no impact outcome, and no participant variability.                                               | Compare clock, state, impedance, and observer-based policies under phase error, delay, noise, saturation, and adverse loads; measure viable timing-window volume and participant variability.                                                          |
| Q5  | Are any strategies self-correcting or more robust to noise?                                                            | **Not established.** Selected programs are robust for selected metrics, but “self-correcting” requires a closed-loop recovery definition.                                                                                      | Common-random-number perturbations, task Jacobian, lower-tail outcomes, adverse strategy comparisons.                                                                  | No attraction-region, error-decay, recovery-time, observer, or human perturbation experiment.                                                                    | Apply matched state perturbations; estimate recovery probability, contraction/error decay, basin size, and worst-case outcomes; require benefit across held-out disturbances without hidden force/load cost.                                           |

...
| Q6  | Must proximal velocity be maximized to maximize transfer?                                                              | **No general rule is supported.** Exact equal-energy rate sweeps are nonmonotonic; the large pre-impact trend nearly vanishes; eight approximately work-matched pairs favor lower rate; and the identical-state acceleration response reverses before impact. | Three rate-matching rules, 45 acceleration interventions, full actuator-work ledgers, rotating-base study, killswitches, braking and load outcomes.                                           | Work-matched pairs are reused and none is also load matched; acceleration is only pointwise; no impact ball outcome, anatomical capacity, or subject calibration. | Execute jointly work/load/state-matched forward rate and acceleration optimization; then locate interior optima and reversals across spatial, subject-scaled, and held-out human tiers.                                                                |

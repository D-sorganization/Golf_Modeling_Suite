...
\begin{tcolorbox}[enhanced jigsaw, leftrule=.75mm, bottomtitle=1mm, breakable, opacitybacktitle=0.6, coltitle=black, colframe=quarto-callout-note-color-frame, toprule=.15mm, left=2mm, opacityback=0, colbacktitle=quarto-callout-note-color!10!white, titlerule=0mm, rightrule=.15mm, arc=.35mm, colback=white, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Scope and Evidence Categories}, bottomrule=.15mm, toptitle=1mm]
...
rates are evaluated. The grid is centered on the achieved proximal rate
and adds offsets from -75\% through +75\% of
...
fixed within a family. Changing velocity necessarily changes kinetic
energy. This is therefore a local state-field sensitivity sweep, not an
equal-energy comparison or realizable trajectory optimization.
...
proximal velocity against impact speed and negative interface work for
every valid-impact trajectory. Color denotes release time, exposing the
timing confound.}
...
test force allocation and force-generated couple, while the moving-base
flexible tier can test translation and shaft response. Neither has an
anatomical rotating torso. A forward, rotating-base, two-hand model with
subject-scaled parameters and declared base work is required before the
torso-velocity question can be tested as posed.
...
The committed evidence contains 90 pointwise cases (five phase labels,
two velocity-matching rules, and nine proximal rates) and all 60
attempted trajectory programs. The machine-readable JSON files record
every row, phase summary, closure test, invalid-impact attempt, Pareto
membership, model-tier statement, falsifier, limitation, and dependency
hash; paired NPZ files preserve the numerical arrays. Repeated
generation is byte-identical for both JSON and NPZ artifacts. The
pointwise generating scripts are
...
late drift- mediated interface power over a declared local range, but
the effect is phase-, range-, and state-definition-dependent and can
reverse earlier in the downswing. A separate finite control grid
supplies a counterexample to using release-state proximal rate as a
standalone rule. The study does not establish an optimal human thorax
velocity, a causal shoulder strategy, a bilateral hand allocation, or a
universal instruction to release while rotating rapidly.

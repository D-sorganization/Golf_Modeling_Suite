...
downswing should the P→D transfer occur? Classical double-pendulum
analyses, delayed-release simulation studies, and optimal-control
results consistently favor a strategy in which energy is first
accumulated in the proximal segments (and in the arms-plus-club system
rotating as a quasi-rigid body), with the transfer to the club
deliberately \emph{restrained} early in the downswing and
\emph{accelerated} late. Empirical skill-level comparisons show that
expert golfers differ from amateurs more in transition timing,
segment-speed magnitudes, and transfer efficiency than in the mere
presence of a P→D order, and methodological work cautions that
...
acceleration attributable to accumulated motion (the drift term) from
the acceleration attributable to instantaneous muscular effort (the
control term). We formalize the ``retain-early / release-late''
hypothesis in counterfactual terms, inventory the models and
energy-transfer computations currently available in the repository,
identify the measurement gaps (notably segmental energy-flow
accounting), and specify falsifiable simulation experiments ---
torque-timing sweeps, counterfactual energy budgets, and re-simulation
parity studies --- that can decide the question quantitatively. No new
experimental data are reported; every quantitative claim herein is
sourced to the cited literature or to inspectable code in the
repository.
\end{abstract}
...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, colframe=quarto-callout-note-color-frame, toprule=.15mm, colbacktitle=quarto-callout-note-color!10!white, rightrule=.15mm, toptitle=1mm, left=2mm, breakable, bottomtitle=1mm, colback=white, coltitle=black, titlerule=0mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Provenance and scope}, opacityback=0, leftrule=.75mm, opacitybacktitle=0.6, arc=.35mm]
...
(sub-issues
\href{https://github.com/D-sorganization/UpstreamDrift/issues/8374}{\#8374}--\href{https://github.com/D-sorganization/UpstreamDrift/issues/8379}{\#8379}).
It is a \emph{review and modeling-perspective} paper: it reports no new
experimental data. Sections that propose experiments
(Section~\ref{sec-proposed-experiments}) describe planned work tracked
in issue
...
against publication records at the time of writing; repository links
point to the \texttt{main} branch of
...
cues (e.g., ``fire the club from the top'' versus ``hold the lag and let
it release''), opposite torque programs at the wrists and torso, and
different predictions for what counterfactual decompositions of a
measured swing should show. It also connects to a long-standing puzzle
in the modeling literature: simple double-pendulum analyses have argued
since the 1960s that the highest clubhead speeds arise when the distal
joint is \emph{not} driven early --- indeed when the release of the club
is passively or actively delayed (\citeproc{ref-cochran1968}{Cochran and
Stobbs 1968}; \citeproc{ref-jorgensen1970}{T. Jorgensen 1970};
...
\item
  ...
  the applied torques would produce at the current pose if the system
  were momentarily at rest, isolating instantaneous muscular effort from
  motion-dependent effects.
\end{itemize}

Preliminary ZTCF/ZVCF analyses conducted with the repository's MATLAB
pipeline (see Section~\ref{sec-ztcf}) motivated the hypothesis examined
in this article: that \emph{restraining} the P→D handoff early in the
downswing --- keeping kinetic energy in the proximal segments and in the
arms-plus-club system rotating as a quasi-rigid unit --- and
accelerating the handoff late, may be the more effective strategy. This
article evaluates that hypothesis against the published literature and
specifies how it can be tested quantitatively with the tools already
present in the repository.

\textbf{Contributions.} The article makes five contributions:
...
(\citeproc{ref-milburn1982}{Milburn 1982}). It is worth pausing on the
...
not merely ordering it.

Putnam's segment-interaction analyses put the principle on a dynamical
footing and simultaneously exposed its limits
(\citeproc{ref-putnam1991}{Putnam 1991},
\citeproc{ref-putnam1993}{1993}). In a linked chain, each segment's
angular acceleration depends not only on the joint torques but on
\emph{motion-dependent interaction torques} --- centripetal and Coriolis
terms proportional to the squares and products of segment angular
velocities. Putnam showed that the P→D pattern in striking and throwing
arises to a substantial degree from these interaction torques:
proximal-segment deceleration and distal-segment acceleration late in
the motion are coupled through the joint force, and they occur even
without a distal joint torque. Two implications matter for golf. First,
the kinematic order does not by itself reveal \emph{which} torques
produced it: an identical peak sequence can be produced by different
mixes of muscular and interaction torques. Second, the deceleration of
proximal segments observed in experts need not be an active ``braking''
strategy --- it can be, at least in part, the dynamical
\emph{consequence} of the distal segment accelerating and back-reacting
on the chain. Marshall and Elliott further cautioned that planar
peak-ordering analyses miss long-axis rotations, which peak very late in
racquet skills and contribute substantially to end-point speed
...
for the present question is that it makes the \emph{timing} of the P→D
handoff an explicit, controllable variable (the ``release'': the point
in the downswing at which the wrist angle is allowed, or driven, to
open) and exposes the mechanism by which delay pays.
...
mode is: \emph{load the proximal rotation first, hold the distal joint
folded (retaining the system's energy in a compact, fast-rotating
configuration), then let geometry and --- in the last phase --- modest
distal torque move the energy outward rapidly}. In the vocabulary of
this article, the double-pendulum literature is already an argument for
\textbf{retain-early / release-late}.
...
literature optimizes \emph{within a model}, and the sensitivity of the
optimal timing to model structure (hub mobility, shaft flexibility,
actuator rate limits) is exactly why a multi-engine platform with
cross-engine validation (see Section~\ref{sec-repo}) is the right
instrument for revisiting the question.
...
budgets of the golf swing (\citeproc{ref-nesbit2005kinematic}{Nesbit
2005}; \citeproc{ref-nesbit2005work}{Nesbit and Serrano 2005}). Using
...
plus hips) contributes on the order of 68.7--72.2\% of the total work of
the swing, the shoulder and arm joints 24.3--28\%, and the legs only
3.3--3.8\% (\citeproc{ref-nesbit2005work}{Nesbit and Serrano 2005}). Two
further observations from the same study bear directly on this article's
...
\item
  ...
  best.} Kwon's laboratory and others have documented skilled golfers
  whose peak ordering deviates from the canonical pelvis-thorax-arm-club
  pattern, and recent work has examined how alternative sequence
  patterns among elite players relate to clubhead speed
  (\citeproc{ref-lee2026}{Lee, Ehrlich, and Cain 2026}). On-plane
  analyses of the ``axle-chain'' system in 66 skilled golfers found
  clubhead speed associations concentrated in specific angular-motion
  characteristics rather than in gross ordering
  (\citeproc{ref-madrid2020}{Madrid et al. 2020}).
\item
  \textbf{Within-player timing differences do not straightforwardly
  explain shot quality.} Neal and colleagues, comparing well-timed and
  mistimed shots \emph{within} highly skilled players, found no
  ...
  the felt sense of ``timing'' apparently lives somewhere other than the
...
The empirical picture that survives these refinements: \emph{skilled
golfers generate larger proximal speeds, separate transition from
release more distinctly, and transmit impulse/energy to the club more
efficiently; merely exhibiting a P→D peak order is neither unique to
skill nor sufficient for it} (\citeproc{ref-cheetham2008}{Cheetham et
al. 2008}; \citeproc{ref-zheng2008}{Zheng et al. 2008};
\citeproc{ref-frontiers2026footground}{{``Foot--Ground Interaction and
Clubhead Speed: Impulse-Based Energy Transfer as the Key Mechanism in
...
with the modeling results of Section~\ref{sec-mechanics}: in a skilled
swing, the dominant energy \emph{generation} occurs in the trunk-hip
complex during the first two-thirds of the downswing
(\citeproc{ref-nesbit2005work}{Nesbit and Serrano 2005}); the dominant
energy \emph{transfer to the club} occurs in the final phase, through
the wrist interface, by a mechanism that is substantially
parametric/geometric (release of the wrist-cock under centrifugal
...
the lower distal peak speeds and lower transfer ratios observed in
amateur cohorts (\citeproc{ref-cheetham2008}{Cheetham et al. 2008};
...
and arms, including the club moving with the arms as a folded unit) and
\(E_c(t)\) the kinetic energy of the club, over a downswing
\(t \in [t_0,
t_{\mathrm{imp}}]\). Define the transfer functional as the inter-segment
power crossing the wrist interface (joint-force power plus moment power,
in the sense of Section~\ref{sec-rw}), \(P_{\mathrm{w}}(t)\). The two
strategies are:
...
\item
  ...
  energy early, and the golfer continues adding energy to an
  already-fast club.
\item
  \textbf{S2 (retain early, release late):} keep \(P_{\mathrm{w}}(t)\)
  small during the first part of the downswing (the wrist interface
  transmits centripetal force but little power; the club rides with the
  arms), while proximal torques maximize \(E_p\); then allow/drive a
  large \(P_{\mathrm{w}}\) concentrated in the final phase.
\end{itemize}
...
\item
  ...
  angular velocity cheaply; releasing early raises the inertia while
  proximal torque is still trying to accelerate the system
...
\item
  ...
  small muscles can only add meaningful power while wrist angular
  velocity is modest --- i.e., late torque acts over a brief window but
  at high force transmission; early wrist torque squanders the window
  and can even require \emph{negative} (restraining) torque later
  (\citeproc{ref-sprigings2000}{Sprigings and Neal 2000}).
\item
  \textbf{Parametric transfer needs developed centrifugal loading.} The
  passive outward flow of energy at release, and the inward-pull gain,
  are both proportional to the centrifugal field established by proximal
  rotation; they are weak early and strong late
  (\citeproc{ref-white2006}{White 2006}; \citeproc{ref-miura2001}{Miura
  2001}).
\item
  \textbf{Interaction torques do the late work for free.} Putnam-style
...
that once S2 is grossly achieved, the residual performance variance
lives in finer variables (release profile shape, hub-path geometry,
impulse-transfer efficiency) --- exactly the variables the
...
answers \emph{``what would the current muscular effort produce from a
standstill in this pose?''} --- it isolates the instantaneous
contribution of control from the contribution of accumulated motion.
Because Equation~\ref{eq-eom} is affine in \(\tau\), the actual
...
sample of an already-measured trajectory. It is explicitly \emph{not} a
forward-integrated rollout: each sample answers a local ``what if''
without propagating the hypothetical forward. This is the right tool for
constructing drift/control \emph{time profiles} along a real swing.
...
run without gravity, the ZVCF and ZTCF contributions superpose to the
BASE motion, mirroring Equation~\ref{eq-superposition} at the level of
simulated trajectories. The MATLAB pipeline additionally computes, for
every joint and every table (BASE, ZTCF, DELTA), the linear work
...
\item
  ...
  energy input at proximal joints, while at the wrist interface the
  control contribution to club angular acceleration should be small or
  even negative (restraining lag against the developing centrifugal
  field). The ZTCF profile of the club should show that, had torques
  been removed, the club would have released outward --- i.e., the drift
  field already ``wants'' to open the wrist, and control is
  \emph{holding energy back}.
\item
  \textbf{Late downswing:} the \emph{drift} term (interaction dynamics:
  parametric transfer, centrifugal release) should dominate club
  acceleration, with a brief late window of positive wrist control power
  (\citeproc{ref-sprigings2000}{Sprigings and Neal 2000}). The DELTA
  (active) share of club-side work should be concentrated before
  release; the ZTCF (passive) share should account for most club energy
  gain after release (\citeproc{ref-white2006}{White 2006}).
\end{itemize}

This is the pattern the repository's preliminary MATLAB analyses
motivated, and it is stated here as a \emph{hypothesis to be
quantified}, not a result: the figures and tables that would
substantiate it are the deliverable of issue
\href{https://github.com/D-sorganization/UpstreamDrift/issues/8377}{\#8377}.
...
\textbf{What is established.} (i) Skilled golf swings exhibit P→D
ordering with successive speed amplification, robustly across shot types
and populations (\citeproc{ref-tinmark2010}{Tinmark et al. 2010};
...
and anthropometry-dependent (shaft flexibility, club MOI, segment
lengths), and model sensitivity is untested across engines. (c) Whether
\emph{actively} restraining release (negative wrist torque early)
outperforms merely \emph{not driving} release is unresolved in 3D models
with realistic actuators --- Jorgensen's and White's analyses suggest
passive holding suffices in 2D (\citeproc{ref-jorgensen1999}{T. P.
Jorgensen 1999}; \citeproc{ref-white2006}{White 2006}), while
Sprigings-Neal-type actuator models leave room for an active-restraint
benefit (\citeproc{ref-sprigings2000}{Sprigings and Neal 2000}). These
are precisely the questions the repository's toolchain can answer.
...
and magnitude swept on a grid; impact defined by club alignment.
Outcome: clubhead speed at impact as a function of \(t_{\mathrm{on}}\)
and profile. S2 predicts a late-interior optimum of \(t_{\mathrm{on}}\)
and a non-inferior or superior restrain-then-drive profile. The MJX/Warp
batched backends make the sweep cheap; the ODE backend provides the
reference.
...
drift-side and control-side club power. Outcome: the fraction of final
club kinetic energy attributable to drift vs control in early vs late
downswing halves, as a function of performance. S2 predicts the drift
share of late club energy gain rises with clubhead speed across the
sweep.

\textbf{E3 --- Re-simulation parity.} Implement the killswitch
re-simulation ZTCF in Python on the backend-neutral model; compare
against both the pointwise ZTCF profiles and the MATLAB pipeline on
matched trajectories (cross-engine comparison service). Outcome:
documented agreement/divergence between the two ZTCF
operationalizations, closing the top gap of Table~\ref{tbl-gaps}.

\textbf{E4 --- Wrist-interface power accounting.} Implement
Robertson-Winter segmental energy flow (joint force power
\(\mathbf F\cdot\mathbf v\) plus moment power
\(\boldsymbol\tau\cdot\boldsymbol\omega\) at each interface; issue
...
Section~\ref{sec-timing} defines --- the primary evidence either
strategy needs.

\textbf{E5 --- Mocap replication.} Run the E2/E4 analyses on measured
swings ingested through the repository's motion pipeline (C3D → tracked
motion), across available skill levels. Outcome: observational test of
the counterfactual predictions of Section~\ref{sec-ztcf} against human
data, with the uncertainty-reporting conventions of the internal
research review.

\section{Limitations}\label{sec-limitations}

This article synthesizes literature and specifies methodology; it
reports no new data, and the S2 hypothesis, while strongly supported by
the modeling literature, remains unquantified in the repository's own
terms until the experiments of Section~\ref{sec-proposed-experiments}
are executed. The kinematic-sequence evidence base carries known
methodological sensitivity (\citeproc{ref-marsan2019}{Marsan et al.
2019}); several classical results rest on planar models whose
generalization to 3D swings with mobile hubs is a live modeling question
(\citeproc{ref-mcphee2022}{McPhee 2022}); and the work-budget
percentages cited from (\citeproc{ref-nesbit2005work}{Nesbit and Serrano
2005}) derive from four subjects. One recent source
(\citeproc{ref-frontiers2026footground}{{``Foot--Ground Interaction and
Clubhead Speed: Impulse-Based Energy Transfer as the Key Mechanism in
...
proxy at the time of writing; readers should resolve the DOI for full
bibliographic details. Finally, energy-based arguments here concern
clubhead \emph{speed}; accuracy, consistency, and injury-risk trade-offs
of late-transfer strategies (\citeproc{ref-hume2005}{Hume, Keogh, and
Reid 2005}; \citeproc{ref-lindsay2008}{Lindsay, Mantrop, and Vandervoort
2008}) are outside scope.
...
can pay for it}. Skilled players differ from amateurs precisely along
this axis: not in whether their segments peak in order, but in how much
energy the proximal system holds at transition, how late and how
efficiently it is handed to the club. The ZTCF/ZVCF counterfactual
framework implemented in UpstreamDrift turns this qualitative account
into measurable time profiles --- drift versus control, passive versus
active, early versus late --- and the experiment suite specified here
(epic
\href{https://github.com/D-sorganization/UpstreamDrift/issues/8373}{\#8373})
is designed to either quantify the retain-early/release-late advantage
or falsify it. Either outcome would be a contribution.
...
inventory in Section~\ref{sec-repo} reflects the codebase at the commit
referenced by the pull request that introduced this document.

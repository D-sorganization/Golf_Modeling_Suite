...
while timestep and projection studies bound the numerical result. This
is a planar model-feasibility result, not physiological or human
evidence. A common-observable model ladder then carries
...
\begin{tcolorbox}[enhanced jigsaw, left=2mm, rightrule=.15mm, colback=white, breakable, leftrule=.75mm, titlerule=0mm, opacityback=0, toprule=.15mm, bottomrule=.15mm, bottomtitle=1mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Scope and Evidence Categories}, arc=.35mm, opacitybacktitle=0.6, coltitle=black, colbacktitle=quarto-callout-note-color!10!white]
...
\item
  ...
  two-hand tiers. The two-hand forward result still uses a fixed
  shoulder base, rigid club, and planar constraints; it does not
  establish persistence under moving-base, compliant, or spatial
  dynamics.
\item
...
\tightlist
\item
  \textbf{Counterfactual transport.} Carry the now-executed same-state
  torque-killswitch from fixed-base rigid planar models into a dynamic
  moving base, compliant club, and then a spatial full-body model.
  Preserve the same force, couple, power, work, and closure observables
  at every tier.
\item
...
\item
  ...
  history. A separate forward rigid-club two-hand model now demonstrates
  finite zero-command persistence, but moving-base, compliant-club,
  spatial, and human variants remain open validation steps.
\item

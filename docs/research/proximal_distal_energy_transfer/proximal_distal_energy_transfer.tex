...
or identify a human strategy. A frozen participant-held-out
...
\begin{tcolorbox}[enhanced jigsaw, bottomtitle=1mm, breakable, opacityback=0, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, bottomrule=.15mm, leftrule=.75mm, toptitle=1mm, colframe=quarto-callout-note-color-frame, colback=white, left=2mm, opacitybacktitle=0.6, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Scope and Evidence Categories}, coltitle=black, toprule=.15mm, rightrule=.15mm, titlerule=0mm]
...
Enter Independent Forward Solvers Without Preload}
...
early, middle, and late phases through both native forward solvers: 54
shared initial states and 108 trajectories. Every engine pair reports
the same initial-state digest and passes the existing trajectory,
wrench, and normalized-energy comparison gates over the preregistered 4
ms initialization audit. The largest cross-engine club-position
difference is 0.083 \(\mu\)m, the largest relative contact-wrench RMS
difference is 0.0145\%, and the largest normalized-energy difference is
\(2.19\times10^{-7}\).
...
\subsubsection{Cross-Engine Validity Horizon and Adverse-Load
Map}\label{cross-engine-validity-horizon-and-adverse-load-map}
...
from the first integration step. Both native engines receive identical
state and parameter records. Failed, unstable, and non-finite cases
...
\caption{\label{fig-forward-contact-validity-horizon}Cross-Engine
Forward-Contact Validity Horizon and Adverse-Load Map}
...
strengthens the reduced cross-engine reference. The articulated tier
reproduces its power, energy, refinement, and engine- parity logic only
through 5 ms, not the full 50 ms interval. Neither result establishes
anatomy, passive human transfer, timing economy, slack benefit, or
coaching strategy.
...
virtual-power residual is \(3.89\times10^{-16}\) W. Damping power is
...
cross-engine trajectory and force-history differences below \(10^{-7}\)
relative.
...
remains negative; and the largest MuJoCo--Pinocchio trajectory and
force-history discrepancies are \(1.54\times10^{-14}\) and
\(1.06\times10^{-11}\) relative, respectively.
...
All numerical and cross-engine gates pass. Across the complete atlas,
the largest force is 23.643 N, virtual-power residual is
...
\subsubsection{Distributed Grip Contact Discretization Across Nested
Horizons}\label{distributed-grip-contact-discretization-across-nested-horizons}
...
three phase samples. Forward and reversed initial club-velocity
perturbations, one-, three-, and five-fiber grips, 1.0 and 0.5 ms steps,
and native MuJoCo and Pinocchio produce 288 trajectories. Each
trajectory is advanced once to 50 ms and summarized at nested 4, 10, 25,
and 50 ms horizons, yielding 1,152 engine--horizon summaries without
resetting the state at each horizon. No active joint torque, hand
driver, or direct club actuation is applied.
...
Discretization Across Nested Articulated Horizons}

\end{figure}%

All registered numerical and native-engine gates pass. The largest
station force is 4.030 N and the largest force couple is 0.375 N m.
Coincident and reversed-moment-arm residuals are exactly zero; the
largest virtual-power residual is \(1.14\times10^{-15}\) W and positive
damping power is zero. The worst normalized work--energy residual
decreases from 0.03119 to 0.01560 when the step is halved. Maximum
trajectory and station-force discrepancies between engines are
\(7.97\times10^{-14}\) and \(2.85\times10^{-12}\) relative, with no
active-set parity failure. At the fine step, the largest
three-to-five-fiber state discrepancy is 0.00355 relative; the
one-to-three discrepancy is 0.0419.

The active-set result is structural rather than a benefit result. The
one-fiber comparator remains fully taut, whereas the off-axis
multi-fiber grips retain at least one open fiber over 66.7--100\% of
samples in the registered cells. No active-set transition occurs, so
opening or reattachment is right-censored through 50 ms. At 50 ms, the
engine-averaged ensemble mean club translation speeds are 0.2444,
0.2379, and 0.2314 m/s for one, three, and five fibers. These
unmatched-load values show discretization dependence; they do not rank
physical grips or establish that partial opening helps or harms
delivery.

The fibers remain memoryless, frictionless engineering lines rather than
calibrated pressure, fingers, tissue, or measured grip mechanics. The
club is rigid and the support pathway is unchanged. Consequently this
gate establishes contact-discretization sensitivity and a 50 ms
numerical horizon only. Shaft bending/torsion, ground/free-moment work,
matched delivery, and governed human measurements remain required before
...
The contact fibers remain frictionless, memoryless engineering lines
with state-registered free lengths. Ground reaction and free moment
remain absent; shaft deformation does not establish tissue loading,
timing economy, impact performance, or coaching strategy. The next
separate transport gate must add a finite ground/free-moment pathway
under the same initial-energy, matched-load/work, killswitch, reversal,
power, refinement, and two-engine controls before the two pathways are
combined.
...
Both tiers still lack calibrated tissue, pressure, friction, and ground
coupling. The next test must add the ground pathway separately before
combining it with grip and shaft mechanics. Only trajectories passing
those gates may test whether drift-mediated transfer reduces timing
demand, creates recovery, benefits from slack, or supports a human
strategy.
...
\item
  \textbf{Bounded Cross-Engine Attachment Dynamics Do Not Establish Full
...
contact unexecuted'' to ``reduced forward contact supported.'' The later
...
contains every earlier mechanism. Ten bounded findings are supported
...
\item
  ...
  bending and torsion to the distributed grip, but all shaft parameters
  remain synthetic and the support pathway remains unchanged rather than
  a finite ground/free-moment system.
\end{itemize}

The next decisive model test adds a finite ground/free-moment pathway
before combining it with the qualified distributed grip and passive
shaft. It must emit the same common schema and retain the exact
same-state command-removal branch. Failure of force/moment balance,
power invariance, contact-rank agreement, refinement, or matched-event
parity at those tiers would falsify transport of the reduced
passive-persistence claim.
...
tier now advances twelve selected states through 50 ms, and a separate
passive-shaft extension adds first-mode bending/torsion with
matched-load/work and native-engine controls. & Calibrate distributed
grip and shaft properties, add finite ground/free-moment coupling and
contact loss, combine the separately qualified pathways, and then repeat
the horizon, geometry, killswitch, and held-out human bilateral-wrench
tests. \\
...
The next articulated atlas extends a frictionless tension-fiber
comparator to one, three, and five stations per hand while holding total
stiffness and damping fixed. Its 288 trajectories pass nested 4, 10, 25,
and 50 ms power, energy, refinement, geometry-null, and
MuJoCo--Pinocchio gates. The one-fiber branch stays taut; multi-fiber
branches retain partially open active sets but show no transition
through 50 ms. Their delivery-state differences therefore demonstrate
discretization sensitivity, not a physical benefit of slack or
distributed pressure. Measured contact pressure, friction, shaft/ground
work, matched tasks, and governed human outcomes remain necessary.

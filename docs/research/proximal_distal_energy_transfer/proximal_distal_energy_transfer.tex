...
proper 3-D frame transformations, while explicitly leaving full-body
cross-engine dynamics unexecuted. A separate hand-path attribution study
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, bottomtitle=1mm, colback=white, toptitle=1mm, breakable, left=2mm, coltitle=black, opacitybacktitle=0.6, titlerule=0mm, leftrule=.75mm, toprule=.15mm, colframe=quarto-callout-note-color-frame, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Scope and Evidence Categories}, colbacktitle=quarto-callout-note-color!10!white, bottomrule=.15mm, opacityback=0, rightrule=.15mm]
...
two-hand systems, and arbitrary three-dimensional frame rotations.
...
historical mobile-hub tier prescribes hub motion and calculates the
required reaction change. By contrast,
Section~\ref{sec-coupled-base-flex} now evolves base translation,
two-hand reactions, and club flex together. These distinctions are part
of the result.
...
Transport to spatial full-body dynamics therefore remains an open test
...
The next decisive test is an event-aligned spatial experiment in two
independent dynamics engines that emits the same common schema and
preserves the declared same-state intervention. Failure of force/moment
balance, power invariance, constraint-rank agreement, or matched-state
parity at that tier would falsify the claim that the lower-order
mechanism has been transported correctly.
...
\item
  ...
  and one lumped flex mode; base rotation, distributed beams, and
  full-body spatial dynamics remain appropriate staged extensions.
\item
...
\item
  ...
  It does not establish persistence under spatial full-body,
  compliant-contact, or human dynamics.
\item
...
\tightlist
\item
  \textbf{Spatial Counterfactual Transport.} Carry the now-executed
  same-state torque-killswitch from rigid and coupled planar models into
  a spatial full-body model in two independent engines. Preserve the
  same force, couple, power, work, and closure observables at every
  tier.
\item
...
\item
  ...
  persistence, but spatial and human variants remain open validation
  steps.
\item

...
from the top produced 15\% less clubhead speed than a fully passive
wrist, while delaying the wrist drive to the grid-selected onset
produced 12\% more. Of the 92 idealized commands tested, a
...
moving-base, two-hand forward solve. The declared baseline stays within
a 5\% deflection screen; the zero-command grip couple remains negative
for 30 ms; coincident and reversed moment-arm controls close exactly;
and work--energy error decreases under timestep refinement. The result
supports only the declared synthetic planar mechanism. A
...
remains inconclusive at that tier. A subsequent reduced forward-contact
...
evidence of a neural synergy or human self-stabilization. A
phase-resolved proximal-velocity study then evaluates 90 exact
same-state cases under relative- and absolute-club-rate matching. High
proximal-link speed strongly increases late drift power in some states,
but the sign reverses earlier and in the delivery state under
absolute-rate matching. A separate 60-program trajectory grid is more
adversarial: among 26 valid impacts, release-state proximal velocity
correlates negatively with impact speed and positively with negative
grip work. The combined result rejects proximal speed as a standalone
release rule and motivates a rotating-base, two-hand falsification tier.
...
\begin{tcolorbox}[enhanced jigsaw, arc=.35mm, colframe=quarto-callout-note-color-frame, breakable, opacityback=0, rightrule=.15mm, toprule=.15mm, leftrule=.75mm, titlerule=0mm, colback=white, bottomtitle=1mm, coltitle=black, toptitle=1mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Scope and Evidence Categories}, bottomrule=.15mm, left=2mm, opacitybacktitle=0.6, colbacktitle=quarto-callout-note-color!10!white]
...
ones. Its interpretation as mechanical energy transfer has also
influenced applied swing descriptions (\citeproc{ref-smith2015}{Smith et
al. 2015}).
...
constrained-dynamics observable. This chapter asks the narrower,
...
components and three moments about a declared plate origin. Under a
planar, rigid-ground convention, the center of pressure (COP) is
inferred from the two horizontal moments and vertical force; the
remaining vertical moment is the free moment. These derived quantities
become unstable when vertical force is small and are frame- and
sign-convention dependent. Pressure insoles directly measure normal
pressure rather than the full six-axis wrench; recovering shear force
and free moment requires an additional model. Joo and colleagues, for
example, treated the nonvertical components as predictions learned
against force-plate data rather than direct pressure measurements
(\citeproc{ref-joo2016}{Joo, Oh, and Mun 2016}).

The golf literature also does not support one universal COP waveform.
Ball and Best found two recurring direction-of-hit COP strategies across
professional through high-handicap golfers
(\citeproc{ref-ballbest2007}{Ball and Best 2007}). Han and colleagues
associated specific lead-foot and trail-foot moment features with
clubhead speed in a skilled male sample (\citeproc{ref-han2019}{Han et
al. 2019}), while Worsfold and colleagues measured shoe--turf torque
differences by handicap and footwear condition
(\citeproc{ref-worsfold2008}{Worsfold, Smith, and Dyson 2008}). The
recent systematic review emphasizes continued variation in terminology,
normalization, events, and analysis choices
(\citeproc{ref-watson2026grf}{Watson et al. 2026}). These are population
associations and strategy descriptions, not evidence that a particular
GRF feature uniquely identifies a biological command.
...
Here \(h_0\) is velocity independent, \(h_v\) is velocity dependent,
\(u\) is the declared controllable input, \(Q_{\mathrm{ext}}\) contains
other known applied loads, and \(\lambda\) is the reaction expressed in
contact coordinates. Define \(A=JM^{-1}J^\mathsf{T}\). When \(J\) has
full row rank and \(A\) is well conditioned,
...
Equation Equation~\ref{eq-grf-reaction} gives four additive terms:
...
inertias, marker kinematics, filtering, or external loads appear in the
same residual one might otherwise mislabel as control. Dynamic
inconsistency between measured motion and measured external force is a
known inverse-dynamics problem (\citeproc{ref-sturdy2022}{Sturdy et al.
2022}; \citeproc{ref-werling2023}{Werling et al. 2023}).
...
The ground-reaction zero-velocity counterfactual (ZVCF) evaluates the
...
Both are \textbf{pointwise}. Neither advances a zero-torque or
zero-velocity system through time. They also overlap in
...
In force-plate patterns, \(\lambda_0\) supplies the
...
Coriolis, and moving-constraint terms, and \(\lambda_u\) captures how
...
split and \(1.9\times10^{-13}\) N for the independently evaluated ZVCF
identity. Treating ZTCF as a drift-only predictor of the modeled total
support reaction gives \(R^2=0.871\) in the target-horizontal component
and \(R^2=0.814\) in the swing-plane vertical component. The
corresponding RMSE values are 64.6 N and 89.8 N. They are 1.02 and 1.42
times the model's projected planar-weight scale, respectively. Thus the
waveform association is strong while the pointwise amplitude error
...
\item
  ...
  reaction. Zeroing velocity tests how much of the model peak requires
  motion; zeroing control tests how much persists passively at the
  achieved state.
\item
...
whole-body and club kinematics, segment inertial estimates, and a
declared contact/frame convention. The primary endpoint should be
held-out waveform error for each force and moment component,
supplemented by vector impulse error, peak timing error, COP error
during adequately loaded samples, and free- moment error. Results should
be stratified by participant rather than randomly splitting time samples
from the same swing.
...
The practical observation motivating this study is that releasing while
the proximal system is moving rapidly can feel more like a ``fling'':
the club gains speed while the hands appear to do less negative work.
That observation is mechanically plausible, but the phrase \emph{high
shoulder velocity} conflates at least four quantities: thorax angular
velocity, shoulder-joint velocity, hand path velocity, and the angular
velocity of the proximal link in a reduced pendulum. This section tests
only the last quantity. In the model, \(\dot q_1\) is the angular
velocity of the proximal rigid link about a fixed hub. It is neither
measured thorax rotation nor an anatomical shoulder-joint coordinate.

The distinction matters empirically. Upper-body kinematic studies report
large thorax speed and strong coupling between thorax and pelvis, but
those observations do not identify a causal club-transfer mechanism
(\citeproc{ref-horan2012}{Horan and Kavanagh 2012};
\citeproc{ref-bourgain2022}{Bourgain et al. 2022}). Bilateral grip-force
measurements also show that the hands can supply different force and
moment histories, so a net grip load does not identify how the player
produced it (\citeproc{ref-koike2020}{Choi and Park 2020}). The present
result is therefore a model-level map from achieved state to
instantaneous acceleration, force, and power contributions. It is not a
coaching rule.

At a fixed configuration \(q\) and applied torque \(u\), the equations
are
...
controls zeroed. It includes velocity-dependent, gravitational, and
constraint effects represented by this model. It does \textbf{not} mean
passive human motion. Reaction-force and grip-power contributions are
reconstructed from the same pointwise split and must satisfy
componentwise closure before a sample is admitted.

\section{Two Velocity-Matching Rules}\label{two-velocity-matching-rules}
...
link or in the laboratory frame. We therefore evaluate two declared
...
\tightlist
\item
  \textbf{Preserve Relative Club Rate:} hold \(\dot q_2-\dot q_1\) fixed
  while changing \(\dot q_1\). This co-transports the club with the
  proximal link and asks what happens when the whole angular velocity
  state is shifted.
\item
  \textbf{Preserve Absolute Club Rate:} hold \(\dot q_2\) fixed while
  changing \(\dot q_1\). This changes the relative rate and asks whether
  proximal speed alone improves the transfer when distal
  laboratory-frame speed is matched.
\end{enumerate}

For each of five downswing phases, nine proximal rates spanning
20--180\% of the reference magnitude are evaluated. Configuration,
torque command, mass, inertia, lengths, and gravity are held fixed
within a phase. This is a local state sweep, not a realizable trajectory
optimization. Its purpose is to test the sign and sensitivity of the
proposed mechanism before adding more degrees of freedom.
...
\caption{\label{fig-shoulder-velocity-drift-power}Drift grip power
...
matching, the slope of drift grip power with proximal rate is \(+9.72\)
W/(rad/s) at transition, \(-1.12\) in early downswing, \(-8.25\) in
mid-downswing, \(+65.06\) in delivery, and \(+283.74\) before impact.
Under absolute-rate matching, the corresponding slopes are \(+0.12\),
\(-0.43\), \(-18.78\), \(-5.96\), and \(+184.58\) W/(rad/s). Thus the
largest late-swing benefit survives both matching rules only in the
pre-impact state. The delivery-state claim survives when relative club
rate is preserved but reverses when absolute club rate is preserved.
...
\caption{\label{fig-shoulder-velocity-braking-map}Negative grip power
across phase and velocity-matching rule. Shading marks states in which
the grip reaction removes modeled mechanical energy from the club at
that instant.}
...
a relationship among phase, configuration, and relative motion, not a
consequence of proximal speed in isolation.
...
\caption{\label{fig-shoulder-velocity-phase-sensitivity}Signed slope of
drift grip power with proximal angular velocity. Positive cells indicate
that faster proximal motion increases drift-mediated grip power within
the declared local sweep.}
...
proximal velocity against impact speed and negative grip work for every
valid-impact trajectory. Color denotes release time, exposing the timing
confound.}

\end{figure}%

Within this finite grid, the empirical premise does not survive as a
standalone model rule. Proximal-link velocity at release correlates with
lower impact speed (\(r=-0.692\)) and greater post-release negative grip
work (\(r=+0.943\)) among the valid trials. In a descriptive
standardized regression that also includes release time, proximal-drive
cut time, and post-cut proximal torque, the release-velocity coefficient
remains negative (\(\beta=-0.514\)). This is not a causal estimate: the
regressors are mechanically coupled, the grid is small, and conditioning
on valid impact creates selection. It nevertheless rejects the claim
that waiting for the modeled proximal link to attain a larger angular
...
N m proximal drive, and reaches 38.85 m/s. Its post-release grip-force
work is 127.21 J, decomposed into 132.08 J drift and -4.87 J control,
with 6.11 J of negative grip work and a 315.39 N peak grip force. The
exact work-closure residual is \(2.1\times10^{-14}\) J. The large peak
load makes clear why speed alone is an incomplete objective.
...
generally demand more peak grip force, while low-force candidates
sacrifice speed or accept more braking work. Several nominally different
programs are trajectory-equivalent because their proximal-drive cut
occurs after impact; they are retained for auditability rather than
silently deduplicated. The result is a Pareto set, not a unique optimum.

\section{A Control Strategy Stated as Testable
Conditions}\label{a-control-strategy-stated-as-testable-conditions}

Within this model tier, a strategy for exploiting drift-mediated
transfer is better stated as five conditions than as a command to
maximize torso speed:
...
\item
  ...
  insufficient.
\item
  \textbf{Avoid Sustained Negative Grip Power.} The measurable adverse
  outcome is not a vaguely defined ``braking force'' but negative grip
  power and its time integral. A high force perpendicular to hand or
  club velocity can transmit little power, whereas a smaller force
  aligned against velocity can remove energy.
\item
  \textbf{Reduce Direct Distal Opposition Only When Drift Transfer Is
  Positive.} A wrist-torque reduction is useful only if the
  state-matched drift term is already accelerating the club and the
  resulting grip power is nonnegative. Releasing earlier can enter the
  adverse mid-downswing region.
\item
  \textbf{Treat Torso Speed as a Boundary Condition, Not the Objective.}
  A player can influence pelvis/thorax torque, hand-path curvature, arm
  configuration, bilateral force allocation, and wrist moment. The
  optimization objective is club delivery speed subject to
  negative-work, peak-force, face/path, robustness, and anatomical
  constraints; torso angular velocity is one state variable in that
  constrained problem.
\end{enumerate}
...
A candidate two-hand strategy is Pareto-optimal only if no alternative
raises delivery speed without worsening at least one registered cost.
This prevents a large speed gain obtained through excessive grip load,
opposing hand work, or an unstable timing boundary from being called
optimal. The existing fixed-shoulder two-arm model can test force
allocation and force-generated couple, while the moving-base flexible
tier can test translation and shaft response. Neither has an anatomical
rotating torso. A forward, rotating-base, two-hand model is required
before the torso-velocity question can be tested as posed.
...
conditions. The preregistered estimands should include phase-resolved
grip power, bilateral common/differential-mode power, proximal
actuator-power proxies, club kinetic-energy change, and the conditional
effect of thorax velocity at matched club state. Because published golf
kinematic methods remain heterogeneous
...
trajectory programs. The machine-readable JSON files record every row,
phase summary, closure test, invalid-impact attempt, Pareto membership,
model-tier statement, falsifier, limitation, and source hash; paired NPZ
files preserve the numerical arrays. The pointwise generating scripts
are \texttt{shoulder\_velocity\_transfer.py},
...
model, high proximal-link angular velocity can markedly increase late
drift-mediated grip power, but the effect is phase- and
state-definition-dependent and can reverse earlier in the downswing. The
study does not establish an optimal human torso velocity, a causal
shoulder strategy, or a universal instruction to release while rotating
rapidly.
...
impulse is 0.1954 N m s. A relaxed start produces an error impulse of
0.2011 N m s for either program. Thus preload improves continuity for a
fixed desired program, while a sign reversal can discard that advantage
under this particular dead-zone model.
...
448 GPa with the reference geometry) so that the declared drive remains
inside a conservative small-deflection screen. That multiplier is a
numerical stress-test choice, not a material, commercial-shaft, or
...
The observed maximum is 0.03378, so the declared baseline passes. The
5\% value is a conservative model-use screen for this resource, not a
universal theorem about beam validity. Torsion, shear deformation,
geometric nonlinearity, impact, and material anisotropy remain absent
even when the screen passes.

At a 0.25 ms step, the base moves by at most 26.65 mm, the modal tip
deflection reaches 33.78 mm, and the first three modal coordinates all
depart from zero. Peak shaft strain energy is 0.533 J and peak clubhead
speed is 7.000 m/s. The force-generated couple spans -18.65 to 31.06 N
m. Mechanical energy changes by 40.285 J while integrated applied and
dissipative work is 40.279 J, leaving a 0.00574 J absolute residual.
These are internal consistency observations for the synthetic case, not
estimates of golfer or equipment performance.
...
relative to six modes is 0.0483 mm and its maximum clubhead-position
discrepancy is 3.42 \(\mu\)m. Retaining three modes reduces those values
to 0.000406 mm and 0.106 \(\mu\)m. Under the short pulse, the one-mode
deflection discrepancy grows to 0.267 mm, 5.53 times its smooth-input
value, while the three-mode discrepancy remains 0.00108 mm and the
clubhead-position discrepancy remains 0.196 \(\mu\)m.
...
negative for the complete 30 ms branch, reaching -14.94 N m. This is a
...
0.00169 J, while the negative interval remains 30 ms and the minimum
couple converges from -14.94 to -14.97 N m. This separates a persistent
mechanism result from a fixed-step artifact without claiming zero
numerical error.
...
None of those failures occurs in this synthetic planar case. The
supported claim is consequently narrow: a finite-element-derived modal
shaft can be coupled into the moving-base two-hand forward solve while
preserving the registered negative-couple intervention and numerical
identities. Equipment calibration, anatomical arms, base rotation,
three-dimensional shaft bending and torsion, compliant tissue, impact,
muscle coordination, optimal control, human strategy, and coaching
benefit remain untested. Those omissions define the next model tiers;
they are not conclusions supplied by this one.
...
\item
  \textbf{Forward Cross-Engine Contact Is Untested at This Tier.}
  Same-state inverse dynamics removes trajectory divergence by design.
  The independently integrated reduced contact experiment in
  Section~\ref{sec-spatial-forward-contact} addresses that separate
  estimand without promoting this prescribed-load result.
\item
...
\chapter{Independently Executed Spatial Forward
Contact}\label{sec-spatial-forward-contact}
...
Pinocchio independently integrate the same 13-coordinate contract using
their native forward-dynamics algorithms
(\citeproc{ref-todorov2012mujoco}{Todorov, Erez, and Tassa 2012};
\citeproc{ref-carpentier2019pinocchio}{Carpentier et al. 2019}). Neither
engine receives a prescribed contact-force history. Each contact force
is calculated from the engine's own achieved relative hand--club
displacement and velocity. The common contact law, reference driver,
timestep, and output convention are held fixed so that engine
implementation is the principal changed factor.
...
Contact Geometry and Engine-Solved Force Vectors}
...
Both engines use the same semi-implicit position/velocity update after
their native acceleration evaluation. This is a controlled numerical
choice, not an assertion that the engines have identical internal
integrators. It removes an uninteresting integration-method difference
while retaining independent mass, bias, gravity, kinematics,
spatial-force mapping, and forward-dynamics implementations. The
Pinocchio adapter uses a local-world-aligned velocity at the moving club
origin; using a world-origin spatial velocity instead creates an
artificial contact-velocity discrepancy. That frame distinction is now
...
\caption{\label{fig-spatial-forward-cross-engine}Independent MuJoCo and
Pinocchio Forward Dynamics Satisfy Declared Gates}

\end{figure}%

\section{Forward Cross-Engine
Results}\label{sec-spatial-forward-results}

Figure~\ref{fig-spatial-forward-cross-engine} shows that the two
independently integrated trajectories remain close. In the
...
contact law. It remains narrower than a cross-engine anatomical
comparison because the engines realize one reduced common model.
...
branch also retains 7.55 degrees of club-axis out-of-plane evolution, a
peak club-long-axis rotation rate of 13.59 rad/s, and a peak
force-generated long-axis couple of 0.242 N m. These observables are
separated deliberately: the negative quantity is a projection of contact
moment on the instantaneous swing normal, while long-axis couple and
rotation report torsional evolution. Calling either projection ``the
club torque'' without naming its axis would be ambiguous.
...
\item
  baseline or killswitch trajectories exceed a declared cross-engine
  region;
\item
...
rather than as evidence for a software platform. Ten presets identify
reproducible entry points: analytical double pendulum, forward two-hand,
coupled moving-base/flexible-club, isolated synthetic distributed shaft,
forward distributed modal shaft, matched-task arm--wrist allocation and
transmission, reduced spatial common state, reduced two-engine spatial
forward contact, uncertainty/control, and experimental readiness. Preset
names preserve the model ladder and never imply that a higher unexecuted
tier is available.
...
for the declared synthetic planar tier. It records post-killswitch
contact persistence as supported only in the reduced spatial carriage
model, human predictions as untested, and a universal controller as
unsupported.

Five release-level completion gates remain visible: subject-scaled
articulated spatial contact with calibrated grip and distributed shaft;
equipment-calibrated beam and measured grip properties; measured
tissue-level preload and slack identification; governed held-out human
...
Cross-engine trajectory, wrench, orientation, and energy gates pass, and
the work--energy residual decreases monotonically as timestep is halved.

This executes independent forward spatial contact at a reduced mechanism
tier. It does not add anatomical arms, subject-specific inertias,
muscles, measured grip compliance, a distributed shaft, or human
observations. The evidence boundary therefore moves from ``forward
contact unexecuted'' to ``reduced forward contact supported; articulated
human transport untested.''
...
summarizes the evidence boundary. Nine statements survive the executed
tiers:
...
\item
  ...
  Pinocchio dynamics within declared trajectory, wrench, orientation,
  and energy regions.
\end{enumerate}
...
anatomical arms, that its magnitude is invariant to a distributed shaft,
or that independently authored native humanoid models produce matched
force histories. Those are separate structural and empirical questions.
...
\textbf{Reduced Spatial Forward Contact.} This tier adds independently
solved two-hand compliance, state divergence, a same-state driver
killswitch, long-axis rotation, swing-plane evolution, and reduced
ground-pathway bookkeeping. It transports the declared mechanism through
two native engines. Its hand carriages and declared contact law prevent
...
\item
  ...
  anatomy; and
\item
  the forward spatial tier uses two translational hand carriages,
  declared Kelvin--Voigt contacts, and a rigid club rather than
  anatomical arms, measured tissue, or the distributed shaft.
\end{itemize}

The next decisive model test replaces the carriages with subject-scaled
articulated arms and couples the distributed shaft while emitting the
same common schema and preserving the exact same-state command-removal
branch. Failure of force/moment balance, power invariance, contact-rank
agreement, or matched-event parity at that tier would falsify transport
of the reduced passive-persistence claim.
...
and model tier are declared. Figure
...
The numerical difference is small, model-dependent, and not a
performance claim. Its value is logical: activation and series-force
states make role history observable even when pre- and post-transition
net moments match. The result is consistent with the broader proposition
that maintaining load direction can preserve an already-developed
transmission state. It neither proves a scapular mechanism nor
establishes the wrist as passive.
...
its coordinate transformations alter power, the preparation effect
disappears under plausible parameter and timestep variation, or measured
participants do not exhibit the predicted force/stiffness history.
Backend disagreement should first be classified as a convention,
numerical, structural, parameter, or data discrepancy; it is not
automatically biological evidence.
...
budgets of the swing (\citeproc{ref-nesbit2005kinematic}{Nesbit 2005b};
...
\citeproc{ref-zheng2008}{Zheng et al. 2008};
\citeproc{ref-milburn1982}{Milburn 1982}). Section
...
compression bounded-actuator theory predicts, while the grid-selected
onsets move slightly \emph{earlier} (0.175--0.100 s versus 0.200--0.100
s), since a velocity-limited wrist needs a slightly longer window to
deliver its work. The S2 ordering is therefore not an artifact of
unbounded actuators.
...
arrives, the model's version of premature delivery --- whereas the two
highest-speed tested programs deliver impact with the arm 0.7--0.8 rad
\emph{past} vertical (hands ahead of the clubhead in this coordinate
convention). This is model geometry, not a classification of a player's
technique.
...
\item
  ...
  (selected drive) versus 34.0 J (early drive) --- the early-driven club
  back-reacts on the arm and \emph{steals its runway}.
\item
...
dominate the club's energy supply (late-half integral 131.7 J versus
25.7 J of late wrist actuator work). The wrist moment channel
contributes a brief late pulse, exactly the ``latter stages'' window
identified by Sprigings and Neal (\citeproc{ref-sprigings2000}{Sprigings
and Neal 2000}). The right panel verifies the accounting: the club's
mechanical-energy rate tracks the sum of joint-force and moment power
across the swing pointwise after analytic COM acceleration replaces
sampled velocity differentiation, confirming the implementation of the
segmental balance of Section~\ref{sec-rw}.
...
distal drive is lower than the passive condition (−15\% in the baseline
case); (ii) identical distal effort applied late is worth +12\%; (iii) a
...
the declared one-at-a-time parameter perturbations, and 20--50 ms
command transitions, although effect sizes and selected onset times
change (Section~\ref{sec-results-e1b},
...
grid-selected swing) is White's parametric transfer made explicit in
...
(\citeproc{ref-white2006}{White 2006}). Timed restraint is faster in the
tested family through the mirror mechanisms: it holds the compact
...
then releases into a fully developed centrifugal field where the passive
channel converts arm energy to club energy at the maximum rate. The
negative early wrist work (−1.9 J) is small in absolute terms and
...
\item
  ...
  removal in both native engines. Its hand carriages and declared
...
question testable without treating peak order as energy transfer,
consistent with recent human transfer-efficiency findings
...
mechanism gate. Native MuJoCo and Pinocchio independently integrate two
finite-mass hand carriages, paired compliant interfaces, and a free
rigid club without direct club actuation. Their baseline and same-state
killswitch trajectories satisfy the declared position, orientation,
wrench, and energy regions. Removing the complete grounded driver leaves
a negative force-generated swing-normal couple for 37.5 ms in both
engines; coincident and reversed-moment-arm controls close exactly, and
the work--energy residual decreases under timestep refinement. This
supports reduced spatial contact persistence, not anatomical arms,
...
\item
  ...
  same-state killswitch, force, couple, power, work, pathway, and
  closure observables.
\item
...
\item
  ...
  three-dimensional club, ground pathways, and couple the executed
  distributed/modal shaft reference into the forward constrained solve
  without changing the registered interface observables. Each rung
...
Choi, Hoseong, and Sung-Young Park. 2020. {``Three Dimensional Upper
Limb Joint Kinetics of a Golf Swing with Measured Internal Grip
Force.''} \emph{Sensors} 20.
...
Joo, Seung-Bum, Sang-Eun Oh, and Joon-Ho Mun. 2016. {``Improving the
...
Sturdy, Jordan T., Kirsten Veerkamp, Eckehard Dorschky, Bettina Muller,
Adam Trepczynski, William R. Taylor, Jochem Menger, and Antonie J. van
den Bogert. 2022. {``Automated Optimization of Residual Reduction
Algorithm Parameters in {OpenSim}.''} \emph{Journal of Biomechanics}
137: 111087. \url{https://doi.org/10.1016/j.jbiomech.2022.111087}.
...
Jennifer L. Hicks, and Scott L. Delp. 2023. {``{AddBiomechanics}:
Automating Model Scaling, Inverse Kinematics, and Inverse Dynamics from
Human Motion Data Through Sequential Optimization.''} \emph{PLOS ONE} 18
(11): e0295152. \url{https://doi.org/10.1371/journal.pone.0295152}.

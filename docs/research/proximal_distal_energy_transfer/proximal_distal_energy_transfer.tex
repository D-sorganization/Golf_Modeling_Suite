...
\begin{tcolorbox}[enhanced jigsaw, breakable, colframe=quarto-callout-note-color-frame, leftrule=.75mm, coltitle=black, bottomtitle=1mm, left=2mm, bottomrule=.15mm, opacitybacktitle=0.6, titlerule=0mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Scope and Evidence Categories}, toptitle=1mm, opacityback=0, colback=white, toprule=.15mm, colbacktitle=quarto-callout-note-color!10!white, arc=.35mm, rightrule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, breakable, colframe=quarto-callout-note-color-frame, leftrule=.75mm, coltitle=black, bottomtitle=1mm, left=2mm, bottomrule=.15mm, opacitybacktitle=0.6, titlerule=0mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Plain-language picture}, toptitle=1mm, opacityback=0, colback=white, toprule=.15mm, colbacktitle=quarto-callout-note-color!10!white, arc=.35mm, rightrule=.15mm]
...
Is slack beneficial or necessary? & Partly answered at synthetic
constitutive and short-horizon forward tiers. Five declared classes have
been exercised separately. A stateful distributed-grip comparator now
retains elastic tangential displacement, Coulomb slip, opening, and
dissipation, but fails two registered refinement cases and lacks native
cross-engine parity. No global benefit, necessity, intentionality, or
delivery advantage is supported. & Extend the stateful law through
matched work, load, and delivery; resolve the failed refinement and
native parity gates without changing their thresholds; add
subject-specific tissue and shaft/ground coupling; acquire independent
contact, shaft, tendon, activation, and bilateral-wrench
measurements. \\
\end{longtable}
...
\subsection{Stateful Stick--Slip
Countermodel}\label{stateful-stickslip-countermodel}

The next preregistered step replaces the memoryless tangential law with
an elastic--perfectly-plastic engineering comparator. Each grip station
retains elastic tangential displacement, applies a radial return at the
Coulomb cap, accumulates plastic dissipation during slip, releases
stored interface energy on opening, and carries projection loss when the
local tangent plane changes. This is a stateful contact algorithm, not a
finger, skin, tendon, neural, or anatomical model. The
timestamp-explicit forward integrator evaluates left-node kinematics,
advances the interface to the end of the increment, and then applies
semi-implicit mechanics. Its energy and coupling-work defects are
therefore separate numerical observables rather than terms silently
absorbed into physical dissipation.

The prospective matrix binds one source state, nine variants, three time
steps (1.0, 0.5, and 0.25 ms), and MuJoCo and Pinocchio, for 54
registered cases. The first execution produced no trajectories: all 54
cases were retained as unavailable after a Windows native-library
load-order failure and an absent Pinocchio module. The disclosed runtime
amendment changed only the native preload launcher. The qualified
execution completed all 27 MuJoCo cases, retained all 27 Pinocchio cases
as typed unavailable, and had no execution failures. The exact plan,
amendment, and execution identities are recorded in
\href{../data/articulated_stateful_distributed_plan.json}{\texttt{articulated\_stateful\_distributed\_plan.json}},
and all atomic checkpoints plus the deterministic aggregate are
published in
\href{../data/articulated_stateful_distributed_smoke/}{\texttt{articulated\_stateful\_distributed\_smoke/}}.

All nine MuJoCo variants show decreasing passive-energy defect under
time-step refinement. The frozen coupling-work refinement gate
nevertheless fails for the frictionless preload-release control (ratios
1.000 and 1.000) and for the low-friction slip probe at the finest
comparison (0.834 versus the registered 0.800 ceiling). The nominal
ratios are 0.558 and 0.528. High friction is numerically identical to
nominal because the nominal trajectory remains below the Coulomb cap;
increasing an inactive cap cannot identify a friction effect. At the
finest step, the nominal family contains 80 elastic-stick and 40 open
station intervals, the frictionless control contains 80 slip and 40 open
intervals, the low-friction probe contains 68 slip, 12 stick, and 40
open intervals, and the opening probe is open in all 120 intervals.

\begin{figure}

\centering{

\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures/fig_stateful_distributed_grip_falsification.pdf}

}

\caption{\label{fig-stateful-distributed-grip-falsification}Stateful
Distributed-Grip Falsification. Passive-energy defects contract in all
nine MuJoCo variants, but the frictionless preload-release and
low-friction slip cases fail the frozen coupling-work refinement
criterion. Contact-regime counts and fine-step speed differences are
short-horizon engineering-model diagnostics. Pinocchio was unavailable,
so promotion and cross-engine parity are withheld.}

\end{figure}%

The separately integrated fine-step variants differ from nominal
clubhead speed by -0.02149 to +0.00834 m/s over only 5 ms. Those
differences are useful software and model-sensitivity diagnostics, but
they are neither a matched delivery comparison nor evidence that
opening, preload removal, friction, or ``slack'\,' creates speed. In
particular, the opening probe changes the active constraint and releases
stored interface energy; its positive short-horizon difference cannot be
interpreted as a beneficial strategy. The campaign is therefore a
successful falsification advance precisely because it exposes where the
algorithm converges, where its registered coupling ledger does not, and
which engine evidence is missing. Promotion remains false until the
refinement and native-parity failures are resolved prospectively.

\subsection{Rigid Refinement Across Screening
States}\label{rigid-refinement-across-screening-states}

The original rigid smoke failed the frozen 0.800 work-refinement gate
for the nominal and high-damping variants. A disclosed development pilot
on that same state suggested that finer grids might contract, but those
post-result outputs were not eligible as confirmatory evidence. The
prospective extension therefore retains the original failures, excludes
the pilot step sizes, and evaluates all 18 previously declared screening
states rather than selecting a favorable state. It crosses those states
with the two original failing variants, unseen 0.20, 0.10, and 0.05 ms
steps, and the two registered native engines. The exact contract is
published in
\href{../data/articulated_rigid_refinement_plan.json}{\texttt{articulated\_rigid\_refinement\_plan.json}}.

All 216 registered cases were retained atomically: 108 MuJoCo cases
completed, 108 Pinocchio cases were typed unavailable, and none failed
during execution. Every completed case satisfies the frozen pointwise,
momentum, work, and trajectory-energy closure tolerances.
Momentum-refinement ratios lie between 0.49485 and 0.49849.
Work-refinement behavior is more state dependent: 33 of 36 MuJoCo
state--variant groups pass, while nominal case 4 at 0.00 s and nominal
case 13 at 0.00 and 0.24 s fail. Their maximum successive work-residual
ratios are 1.492, 0.852, and 1.827, respectively, against the unchanged
0.800 ceiling. All 18 high-damping groups pass. The exact 216-file
inventory and deterministic summary are published in
\href{../data/articulated_rigid_refinement/}{\texttt{articulated\_rigid\_refinement/}}.

\begin{figure}

\centering{

\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{figures/fig_rigid_refinement_screening_states.pdf}

}

\caption{\label{fig-rigid-refinement-screening-states}Rigid Refinement
Across Screening States. The heatmaps and statewise scatter retain all
three nominal refinement failures; the high-damping variant passes at
every screened state. All individual closure tolerances pass, but
nonmonotonic work-residual contraction and unavailable Pinocchio parity
keep promotion false.}

\end{figure}%

This extension does not support a blanket claim that finer steps resolve
the original failure. Instead, it localizes the remaining numerical
vulnerability to state-dependent nominal trajectories and makes the next
falsifier explicit: an independently registered integrator or quadrature
comparison must reproduce or remove those three nonmonotonic cases
without changing the states, tolerances, or ledger definition. The
result concerns numerical attribution in a five-millisecond engineering
model. It does not establish a physical transfer mechanism, a human
movement pattern, or a coaching strategy, and native cross-engine parity
remains unavailable.
...
The current census contains 334 material claims. The normalized outcome
...
\texttt{project\_derivation\_or\_document} & 119 \\
\texttt{project\_executable\_or\_generated} & 305 \\
...
\bottomrule\noalign{}
\endlastfoot
\texttt{articulated\_spatial} & 38 \\
\texttt{compliant\_shaft} & 66 \\
\texttt{cross\_tier\_synthesis} & 83 \\
...
\texttt{two\_hand\_or\_bilateral} & 97 \\
...
ch08b momentum transfer questions & 16 & 16 / 0 / 0 & project authored
only \\
...
values up to floating-point environment differences. The prospective
native-engine structural factorial uses a stricter contract: the frozen
source and execution-module digests, package and engine versions,
declared runner placement, thread settings, and numerical-kernel
dispatch are qualified before execution. The actual campaign runner must
reproduce that audit exactly before any case is evaluated. The OpenBLAS
kernel path is pinned with
\href{https://github.com/OpenMathLib/OpenBLAS/wiki/Faq/0ed685dd80eab9b96751a75b2713049891bd4f16}{\texttt{OPENBLAS\_CORETYPE=Haswell}},
and a replay-only run independently tests the qualification without
invoking the campaign. Runtime drift is retained as adverse
computational evidence and stops the slice; it is not repaired by
rounding results or relaxing a numerical tolerance. Evidence from
different runtime identities is not pooled. The final optimizer requires
PyMuPDF, performs lossless object/stream compaction, and refuses an
artifact that changes the page count, URI-link count, or outline-entry
count.

...
| **Gear Effect Simulation**   | **TrackMan**, **Foresight**<br>Methods for simulating ball flight    | `compute_gear_effect_spin` in `src/shared/python/physics/impact_model.py`. Uses heuristic constants `100.0`, `50.0`.                             | **MEDIUM**  | Document source of constants (`100.0`, `50.0`). If derived from TrackMan data, ensure "fair use" or clean-room implementation.                                                          | `Issue_011` (tracker document not present in this repository)                                   |
...
- **Tracker**: `Issue_011` (tracker document not present in this repository)

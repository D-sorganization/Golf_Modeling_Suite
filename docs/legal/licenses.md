...
| `casadi`                 | optimal-control                  | `>=3.6.0,<4.0.0`                 | LGPL-3.0                               | Commercial-OK          | Opt-in solver backend; dynamically linked Python wheel, never vendored.            |
| `click`                  | core                             | `>=8.3.3`                        | BSD-3-Clause                           | Commercial-OK          | Direct security floor for transitive CLI use.                                      |
| `cryptography`           | dev                              | `>=46.0.7`                       | Apache-2.0 OR BSD-3-Clause             | Commercial-OK          | Security tests.                                                                    |
| `crocoddyl`              | crocoddyl                        | `>=2.0.0,<4.0.0`                 | BSD-3-Clause                           | Commercial-OK          | Opt-in DDP backend; subprocess-probed, conda-forge preferred (mixed-wheel hazard). |
...
| `ezc3d`                  | c3d, dev                         | `>=1.5.0,<2.0.0`                 | MIT                                    | Commercial-OK          | Runtime C3D reader/writer (canonical parser + only writer); also dev tests.        |
...
| `jax`                    | mjx                              | `>=0.4.30`                       | Apache-2.0                             | Commercial-OK          | Optional MJX autodiff/runtime dependency.                                          |
| `jaxlib`                 | mjx                              | `>=0.4.30`                       | Apache-2.0                             | Commercial-OK          | Optional MJX XLA runtime dependency; verify accelerator deployment terms.          |
...
| `mako`                   | core                             | `>=1.3.12`                       | MIT                                    | Commercial-OK          | Alembic migration template renderer with an explicit security floor.               |
...
| `mujoco-mjx`             | mjx                              | `>=3.6.0`                        | Apache-2.0                             | Commercial-OK          | Optional differentiable MuJoCo backend.                                            |
...
| `nimblephysics`          | nimble-oracle                    | `==0.10.52.2`                    | MIT                                    | Commercial-OK, Opt-in  | Offline gradient oracle only; uneven wheel coverage, not a runtime engine.         |
...
| `pikepdf`                | publication                      | `>=10.0.0`                       | MPL-2.0                                | Commercial-OK          | Release-only PDF linearization; not imported by the application runtime.           |
| `pin`                    | pinocchio                        | `>=2.6.0,<5.0.0`                 | BSD-2-Clause                           | Commercial-OK          | Pinocchio PyPI package.                                                            |
| `pin-pink`               | pinocchio                        | `>=1.0.0,<4.0.0`                 | Apache-2.0                             | Commercial-OK          | Task-based differential IK; guarded imports, diff_ik fallback when absent.         |
...
| `pyjwt`                  | core                             | `>=2.13.0`                       | MIT                                    | Commercial-OK          | JWT auth runtime dependency with an explicit security floor.                       |
...
| `pymupdf`                | publication                      | `>=1.26.0`                       | AGPL-3.0 OR Artifex commercial license | Needs review           | Release-only PDF tooling; do not bundle without a separate license review.         |
...
| `rerun-sdk`              | visualization                    | `>=0.15.0,<1.0.0`                | MIT OR Apache-2.0                      | Commercial-OK          | Opt-in recorded 3D diagnostics (ADR-0027 follow-up).                               |
...
| `starlette`              | core                             | `>=1.0.1`                        | BSD-3-Clause                           | Commercial-OK          | FastAPI ASGI runtime with an explicit security floor.                              |
...
| `tornado`                | core                             | `>=6.5.7`                        | Apache-2.0                             | Commercial-OK          | Direct security floor for Bokeh's asynchronous runtime.                            |

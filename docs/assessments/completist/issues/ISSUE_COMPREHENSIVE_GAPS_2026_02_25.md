...
*   **Specific Gaps:**
    *   `_connect_ros2`: Missing ROS2 node initialization, subscribers, and publishers (marked with TRACKED_TASK).
    *   `_connect_udp`: Missing UDP socket creation and binding (marked with TRACKED_TASK).
    *   `_connect_ethercat`: Missing EtherCAT master initialization (marked with TRACKED_TASK).
...
*   **Specific Gaps:**
    *   **Environmental Gradient Modeling:** Missing implementation for wind shear and temperature gradients (TRACKED_TASK).
    *   **Hydrodynamic Lubrication:** Missing wet ball physics (TRACKED_TASK).
    *   **Dimple Geometry Optimization:** Missing dimple pattern effects (TRACKED_TASK).
    *   **Turbulence Modeling:** Missing turbulence effects (TRACKED_TASK).
    *   **Mud Ball Physics:** Missing mud adhesion effects (TRACKED_TASK).
...
*   **Specific Gaps:**
    *   **Torsional Dynamics:** The current model ignores shaft twisting during the swing (TRACKED_TASK).
    *   **Asymmetric Cross-Sections:** No support for modeling spine alignment or manufacturing tolerances (TRACKED_TASK).
...
*   **Specific Gaps:**
    *   **Effective Mass:** Uses scalar formula (`1 / (1/m + r^2/I)`), ignoring the full 3D inertia tensor and impact vector direction (marked with TRACKED_DEFECT).
...
*   **Specific Gaps:**
    *   **Fallback Calculation:** Incorrectly sums static gravity ($W=mg$) instead of accounting for dynamic acceleration ($F=m(g+a)$) when native contact data is unavailable (marked with TRACKED_DEFECT).

...
**Learning:** `np.linalg.norm(..., axis=1)` creates an intermediate array for the result of `vertices` magnitude. `np.max(np.linalg.norm(vertices, axis=1))` computes magnitude for each vertex, then takes maximum. Using `np.einsum('ij,ij->i', vertices, vertices)` computes squared distance. It's faster to compute `np.max` of squared distances then take the `sqrt`.
**Action:** Replace `np.max(np.linalg.norm(vertices, axis=1))` with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` when doing bounding sphere radius computations to optimize by avoiding intermediate array allocations.

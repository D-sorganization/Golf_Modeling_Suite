...
## 2026-06-25 - Replace iterrows with vectorized column_stack
**Learning:** When constructing multi-dimensional NumPy arrays (e.g., 3D trajectories) from a Pandas DataFrame in UI drawing code, using list comprehensions with `df.iterrows()` creates massive overhead due to row-by-row Series creation. Using vectorized `np.column_stack` with `.values` is about ~250x faster.
**Action:** Avoid `df.iterrows()` when building trajectory arrays. Instead, extract the whole columns as NumPy arrays using `.values` and use `np.column_stack((df['X'].values, df['Y'].values, ...))` for significant speedups.

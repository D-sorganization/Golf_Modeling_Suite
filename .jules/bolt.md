...
**Learning:** `np.linalg.norm(..., axis=1)` creates temporary array allocations which are costly for computing norms of small, inner dimensions across large outer dimensions (like a large array of 3D vertices).
**Action:** In `fit_sphere` or similar situations, compute bounding sphere radii using `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` instead of `np.max(np.linalg.norm(vertices, axis=1))` to prevent temporary array allocations and redundant memory overhead.

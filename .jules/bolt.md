...
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` allocates intermediate arrays for the norms of all vertices. Calculating the maximum of squared distances first with `np.max(np.einsum('ij,ij->i', vertices, vertices))` and taking a single `np.sqrt()` avoids intermediate N-sized array allocations and is significantly faster.
**Action:** Use `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` instead of `np.max(np.linalg.norm(vertices, axis=1))` when computing the maximum magnitude across an array.

...
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` computes N square roots and allocates intermediate temporary arrays. `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` computes exactly 1 square root and avoids intermediate array allocation.
**Action:** When computing the maximum vector magnitude in an array, use `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` instead of `np.max(np.linalg.norm(vertices, axis=1))` to avoid allocating intermediate temporary arrays.

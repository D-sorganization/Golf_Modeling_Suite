...
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` computes N square roots when only 1 is needed (the square root of the maximum squared magnitude). By swapping the operations to compute the maximum of the squared magnitudes first, and then taking a single square root at the end, significant performance is gained.
**Action:** Use `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` instead of `np.max(np.linalg.norm(vertices, axis=1))` to avoid allocating intermediate temporary arrays and computing N square roots.

...
## 2026-07-20 - Avoid False np.linalg.norm -> np.vdot optimizations with function duplication
**Learning:** When optimizing `np.linalg.norm(func(...))` to use `math.sqrt(np.vdot(..., ...))` for small arrays to reduce overhead, naively replacing the arguments like `np.vdot(func(...), func(...))` creates a severe performance regression. Python will evaluate the potentially expensive `func(...)` twice, completely negating any micro-optimization.
**Action:** Always evaluate the function once, store its result in a local variable (e.g., `vec = func(...)`), and then pass the variable to `vdot` (e.g., `math.sqrt(np.vdot(vec, vec))`).

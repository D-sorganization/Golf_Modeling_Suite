...
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` computes N square roots when finding the maximum distance, which is inefficient. Replacing it with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` evaluates squared distance first, computes the maximum, and performs only one square root, avoiding intermediate temporary array allocations and yielding a ~4.6x speedup.
**Action:** When computing the maximum norm over an axis, evaluate the squared distance first using `np.einsum`, find the maximum, and then apply `np.sqrt` to improve performance.

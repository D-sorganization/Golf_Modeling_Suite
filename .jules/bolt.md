...
## 2026-05-02 - Optimize bounding sphere radius computation in mesh primitive fitting
**Learning:** `np.linalg.norm(..., axis=1)` is relatively slow due to overhead in NumPy and intermediate allocations. Replacing it with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` computes squared distances directly and avoids N square roots, replacing it with just one at the end, yielding significant performance speedups (e.g. ~4.6x).
**Action:** When computing the maximum vector magnitude, such as for a bounding sphere radius, use `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` instead of `np.max(np.linalg.norm(vertices, axis=1))` to avoid intermediate temporary array allocations and overhead.

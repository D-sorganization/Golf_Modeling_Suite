...
**Learning:** `np.linalg.norm` evaluates element-wise square roots and allocates intermediate temporary arrays. Since `max` and `sqrt` are commutative for positive numbers, computing the maximum sum-of-squares first using `np.einsum`, then applying `sqrt` avoids memory allocations and performs exactly 1 square root instead of N square roots. When using `np.einsum` to optimize operations like squared distances on mesh vertices, integer overflow and subsequent `NaN` values occur when the original vertex coordinates are integer types.
**Action:** Replace `np.max(np.linalg.norm(vertices, axis=1))` with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices_float, vertices_float)))` where `vertices_float = np.asarray(vertices, dtype=np.float64)` when calculating bounding sphere radii from mesh vertices to improve performance.

...
## 2026-05-01 - Optimize maximum bounding sphere radius computation
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` is slower when calculating bounding sphere radii as it allocates intermediate N-sized array to compute norms. Using `np.max(np.einsum('ij,ij->i', vertices, vertices))` avoids intermediate array allocations.
**Action:** When calculating maximum magnitudes using norms, calculate the maximum of squared distances using `np.einsum` first and then apply a single `np.sqrt()`.

...
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` computes the norms for all vertices, allocating intermediate arrays, before finding the maximum. It is significantly faster to find the maximum squared norm first, and only compute the square root once: `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))`.
**Action:** Replace `np.max(np.linalg.norm(vertices, axis=1))` with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` inside bounding sphere calculations. This speeds up computation by reducing intermediate array allocations and square roots.

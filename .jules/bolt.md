...
## 2024-05-24 - Optimize bounding sphere radius computation using einsum
**Learning:** Computing bounding sphere radius with `np.linalg.norm(vertices, axis=1)` allocates temporary arrays for the squared differences before summation, causing memory overhead. By replacing it with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))`, the identical maximum distance is computed while avoiding intermediate Nx3 array allocations and reducing memory overhead.
**Action:** When computing maximum magnitudes for bounding spheres along an axis, use `np.sqrt(np.max(np.einsum('ij,ij->i', x, x)))` instead of `np.max(np.linalg.norm(x, axis=1))`.

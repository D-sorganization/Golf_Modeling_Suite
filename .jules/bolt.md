...
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` calculates the square root for every vertex, which is expensive. `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` computes the squared norms first, finds the maximum, and only then computes a single square root, saving $N$ square root calculations and intermediate array allocations.
**Action:** Replace `np.max(np.linalg.norm(v, axis=1))` with `np.sqrt(np.max(np.einsum('ij,ij->i', v, v)))` when computing maximum vector magnitudes to improve performance.

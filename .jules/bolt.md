...
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` computes square roots for all elements and allocates intermediate N-sized arrays. By replacing it with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))`, we compute the maximum of the squared distances using `einsum` (which avoids intermediate array allocation) and then compute only a single square root at the end.
**Action:** Replaced `np.max(np.linalg.norm(...))` with `np.sqrt(np.max(np.einsum(...)))` inside `_cg_primitive_fitting.py` to optimize bounding sphere computation.

...
**Learning:** Computing the maximum of squared distances first using `np.einsum('ij,ij->i', vertices, vertices)` and applying a single `np.sqrt()` is mathematically equivalent to `np.max(np.linalg.norm(vertices, axis=1))` but significantly faster.
**Action:** Replaced `np.max(np.linalg.norm(vertices, axis=1))` with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` in mesh primitive fitting.

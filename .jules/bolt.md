...
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` is slower because `np.linalg.norm` computes distances and allocates intermediate arrays for each dimension before finding the max.
**Action:** Use `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` instead. It calculates the max squared distance first using efficient `np.einsum`, avoiding `np.linalg.norm`'s overhead, and only applies `np.sqrt` once to the single maximum value, providing a significant speedup.

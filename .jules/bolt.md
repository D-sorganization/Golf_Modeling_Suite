...
**Learning:** Computing `np.max(np.linalg.norm(vertices, axis=1))` allocates intermediate memory for the norm calculation.
**Action:** Replaced `np.max(np.linalg.norm(vertices, axis=1))` with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` to optimize bounding sphere computations and avoid temporary array allocations.

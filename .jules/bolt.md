...
## 2026-05-19 - Optimize bounding sphere radius computation
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` computes the norms for all elements, creating intermediate arrays. Replacing it with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` avoids N-sized intermediate allocations.
**Action:** When computing bounding sphere radius over vertices, use `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` instead of `np.max(np.linalg.norm(vertices, axis=1))` to prevent memory overhead and speed up computation.

...
**Learning:** `np.linalg.norm(..., axis=1)` is relatively slow for computing bounding sphere radius because of internal overhead in NumPy and intermediate allocations. Replacing it with `np.sqrt(np.max(np.einsum('ij,ij->i', x, x)))` computes the identical L2 norm while avoiding the overhead and allocations, yielding significant performance speedups.
**Action:** When computing vector magnitudes or Euclidean norms along an axis for maximum radius, use `np.sqrt(np.max(np.einsum('ij,ij->i', x, x)))` instead of `np.max(np.linalg.norm(x, axis=1))` to improve performance.

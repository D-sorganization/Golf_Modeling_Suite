...
## 2026-06-26 - Replacing np.linalg.norm with math.hypot in non-critical paths
**Learning:** While `math.hypot(dx, dy, dz)` is significantly faster than `np.linalg.norm` for 3D vectors due to avoiding NumPy's dispatch and array creation overhead, applying this micro-optimization in non-critical paths (e.g., updating UI metrics or single-shot offline video exports) yields zero measurable real-world benefit. Reviewers correctly reject such changes as violating the rule against premature optimization of cold paths.
**Action:** Do not replace `np.linalg.norm` with `math.hypot` in GUI handlers or single-execution configuration functions. Only apply this optimization in tight simulation loops or large data transformations where the microsecond savings accumulate meaningfully.

...
**Learning:** Computing the maximum bounding sphere radius via `np.max(np.linalg.norm(vertices, axis=1))` is slow because `np.linalg.norm` allocates a temporary array for the intermediate norms. Using `np.einsum` to compute the squared magnitudes followed by a single square root on the maximum value is mathematically equivalent and avoids the temporary allocations, yielding better performance.
**Action:** Replace `np.max(np.linalg.norm(vertices, axis=1))` with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` when computing bounding sphere radii.

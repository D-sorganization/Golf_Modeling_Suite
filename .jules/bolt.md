...
**Learning:** Using `np.max(np.linalg.norm(vertices, axis=1))` computes intermediate arrays and is slower.
**Action:** Replaced with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` to avoid N-sized intermediate array allocations when computing bounding sphere radii.

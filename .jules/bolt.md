...
## Optimize Sum of Squares in mesh primitive fitting
**Learning:** np.max(np.linalg.norm(vertices, axis=1)) allocates intermediate temporary arrays. Replacing it with np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices))) computes exactly 1 square root instead of N square roots and avoids allocations.
**Action:** Replaced np.max(np.linalg.norm(vertices, axis=1)) with np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices))) inside _cg_primitive_fitting.py

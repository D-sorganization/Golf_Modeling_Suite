...
## 2025-05-18 - Optimize bounding sphere computation
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` is relatively slow for small inner dimensions because of internal overhead in NumPy and intermediate allocations. Replacing it with `np.sqrt(np.max(np.einsum('ij,ij->i', x, x)))` computes the identical L2 norm while avoiding the overhead and allocations, yielding significant performance speedups.
**Action:** When computing bounding sphere radiuses, use `np.sqrt(np.max(np.einsum('ij,ij->i', x, x)))` instead of `np.max(np.linalg.norm(x, axis=1))` to improve performance.

...
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` allocates intermediate memory to compute the norm for all vertices before taking the max, which is inefficient. Replacing it with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` first computes the max squared distance using a memory-efficient dot product, then applies `sqrt` only once. This avoids redundant temporary array allocations and yields significant speedups for large arrays.
**Action:** When computing the maximum Euclidean distance from a center (e.g., bounding sphere radius), use `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` instead of `np.max(np.linalg.norm(vertices, axis=1))`.

...
**Learning:** `np.linalg.norm(..., axis=1)` is relatively slow for computing distances to find the maximum radius. Replacing it with `np.sqrt(np.max(np.einsum('ij,ij->i', x, x)))` avoids intermediate N-sized array allocations and redundant memory overhead, yielding significant speedups.
**Action:** When computing the maximum vector magnitude (e.g., bounding sphere radii), use `np.sqrt(np.max(np.einsum('ij,ij->i', x, x)))` instead of `np.max(np.linalg.norm(x, axis=1))` to prevent temporary array allocations and redundant memory overhead.

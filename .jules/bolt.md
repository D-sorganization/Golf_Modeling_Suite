...
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` computes the square root for every vertex and allocates an N-sized array before taking the maximum. Replacing it with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` computes the sum of squares directly, finds the maximum, and evaluates the square root only once, avoiding intermediate N-sized array allocations and providing a significant speedup.
**Action:** When finding the maximum Euclidean norm of a collection of vectors, use `np.sqrt(np.max(np.einsum('ij,ij->i', v, v)))` instead of `np.max(np.linalg.norm(v, axis=1))`.

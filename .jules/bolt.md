...
## 2024-05-19 - Optimize bounding sphere radius computation
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` computes the Euclidean norm for all vertices, resulting in an intermediate $N$-sized array of floats. For large inputs, this memory allocation overhead slows down execution. By rearranging the calculation to `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))`, we determine the maximum squared distance directly, avoiding a large temporary array and yielding significant performance speedups.
**Action:** When computing the maximum vector magnitude in an array, replace `np.max(np.linalg.norm(vertices, axis=1))` with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` to bypass intermediate array allocation overhead.

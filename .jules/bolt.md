...
**Learning:** Computing `np.max(np.linalg.norm(vertices, axis=1))` allocates intermediate temporary arrays and computes N square roots. Using `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` computes only one square root and avoids array allocations.
**Action:** Use `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` to compute bounding sphere radius efficiently.

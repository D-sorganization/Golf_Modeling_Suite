...
## 2025-05-18 - Optimize bounding sphere radius computation
**Learning:** `np.max(np.linalg.norm(vertices, axis=1))` computes the Euclidean norm for all vertices, requiring temporary memory allocations and overhead. We can avoid this by replacing it with `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))`.
**Action:** For bounding sphere optimizations across arrays of vertices or distances, use `np.sqrt(np.max(np.einsum('ij,ij->i', vertices, vertices)))` instead of `np.max(np.linalg.norm(vertices, axis=1))` to avoid intermediate allocations and speed up computation.

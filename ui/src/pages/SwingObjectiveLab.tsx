 ...
 * running the direct collocation comparison via POST /api/tools/swing-objectives/compare,
...
export function SwingObjectiveLabPage(): React.ReactElement {
  ...
  const [clubLength, setClubLength] = useState<number>(1.1);

  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [comparison, setComparison] = useState<SwingComparisonResponse | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    async function loadPresets() {
      try {
        const response = await apiFetch<PresetListResponse>(
          "/api/tools/swing-objectives/presets",
        );
  ...
  const handleRunComparison = useCallback(async () => {
    ...
    try {
      ...
      const result = await apiFetch<SwingComparisonResponse>(
        "/api/tools/swing-objectives/compare",
        {
          method: "POST",
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to compute swing objective comparison.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <WorkspaceShell>
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-700 bg-gray-800 px-4 py-3">
        <div>
          <h1 className="text-xl font-semibold text-gray-100">
            Swing Objective Lab
          </h1>
          <p className="text-sm text-gray-300">
            Evaluate competing downswing optimization objectives under a shared
            effort budget.
          </p>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={hubTorque}
                  onChange={(e) =>
                    setHubTorque(parseFloat(e.target.value) || 0)
                  }
                  className="w-full bg-gray-900 border border-gray-700 rounded px-3 py-1.5 text-sm text-gray-100 focus:ring-2 focus:ring-blue-500"
              ...
              <div>
                ...
                <input
                  ...
                  value={wristTorque}
                  onChange={(e) =>
                    setWristTorque(parseFloat(e.target.value) || 0)
                  }
                  className="w-full bg-gray-900 border border-gray-700 rounded px-3 py-1.5 text-sm text-gray-100 focus:ring-2 focus:ring-blue-500"
              ...
              <div>
                ...
                <input
                  ...
                  value={nodeCount}
                  onChange={(e) =>
                    setNodeCount(parseInt(e.target.value, 10) || 21)
                  }
                  className="w-full bg-gray-900 border border-gray-700 rounded px-3 py-1.5 text-sm text-gray-100 focus:ring-2 focus:ring-blue-500"
              ...
              <div>
                ...
                <input
                  ...
                  value={armMass}
                  onChange={(e) =>
                    setArmMass(parseFloat(e.target.value) || 5.0)
                  }
                  className="w-full bg-gray-900 border border-gray-700 rounded px-3 py-1.5 text-sm text-gray-100 focus:ring-2 focus:ring-blue-500"
              ...
              <div>
                ...
                <input
                  ...
                  value={armLength}
                  onChange={(e) =>
                    setArmLength(parseFloat(e.target.value) || 0.65)
                  }
                  className="w-full bg-gray-900 border border-gray-700 rounded px-3 py-1.5 text-sm text-gray-100 focus:ring-2 focus:ring-blue-500"
              ...
              <div>
                ...
                <input
                  ...
                  value={clubLength}
                  onChange={(e) =>
                    setClubLength(parseFloat(e.target.value) || 1.1)
                  }
                  className="w-full bg-gray-900 border border-gray-700 rounded px-3 py-1.5 text-sm text-gray-100 focus:ring-2 focus:ring-blue-500"
          ...
          {comparison?.is_degenerate && (
            ...
            >
              <div className="font-bold flex items-center gap-2 text-amber-300">
                <span aria-hidden="true">⚠️</span> Configuration Degeneracy
                Detected
              </div>
              <p>
                Near the golfer&apos;s minimum downswing duration the
                constraints pin the trajectory and every objective returns the
                same swing, filling the matrix with 100% entries that read as
                unanimous mechanism agreement but are an artifact of the
                configuration.
              </p>
            </div>
          )}
                    ...
                    <tbody>
                      {comparison.objective_keys.map((rowKey, rowIndex) => (
                        <tr key={rowKey} className="hover:bg-gray-750/50">
                          ...
                          {comparison.matrix[rowIndex]?.map(
                            (score, colIndex) => {
                              const colKey =
                                comparison.objective_keys[colIndex];
                  ...
                  >
                    <thead>
                      <tr className="bg-gray-900/90 text-gray-300 text-xs uppercase tracking-wider">
                        ...
                        <th
                          scope="col"
                          className="p-3 border border-gray-700 text-right"
                        >
                          Attained Value
                        </th>
                        <th

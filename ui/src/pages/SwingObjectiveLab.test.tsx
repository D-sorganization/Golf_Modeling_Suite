...
describe("SwingObjectiveLabPage", () => {
  ...
  it("executes comparison and renders labelled matrix and metrics table", async () => {
    ...
    const cell00 = screen.getByTestId(
      "matrix-cell-clubhead_speed-clubhead_speed",
    );
    expect(cell00).toHaveTextContent("100.0%");
    expect(cell00).toHaveAttribute(
      "aria-label",
      expect.stringContaining(
        "Optimized for Clubhead Speed, evaluated on Clubhead Speed: 100.0%",
      ),
    );
    ...
    expect(cell01).toHaveAttribute(
      "aria-label",
      expect.stringContaining(
        "Optimized for Clubhead Speed, evaluated on Centrifugal Release: 98.5%",
      ),
    );
  ...
  it("surfaces plain-language degeneracy warning when is_degenerate is true", async () => {
    ...
    expect(
      screen.getByText(
        /Near the golfer's minimum downswing duration the constraints pin the trajectory/i,
      ),
    ).toBeInTheDocument();
  });
  ...
  it("renders error alert when API call fails", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(/Collocation solver diverged/i),
      ).toBeInTheDocument();
    });
  });
});

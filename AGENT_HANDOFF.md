...
- #8684 governs distributed grip, shaft, and ground:
  ...
  - #8752 is **OPEN** on `research/8752-articulated-uncertainty`. The v2
    ...
    shapes. The campaign has 2/19 corners complete and is running stiffness-high.
  ...
  - #8800 is **OPEN** and blocks #8752. Its governed generator regenerates all
    ...
    Common-support analysis reproduces nominal 126/384 shaft and 0/384 ground while preserving identities, denominators, failures, resolution, and unpooled secants. Fail-closed per-cell gate derivation combines rigid/coupled shaft and fixed/coupled ground branches, broadcasts parity across engine identities, and retains every simultaneous failure class. The cell-pack assembler derives identities, outcomes, and gates from one atlas mapping; atomic digest-bound NPZ output uses no pickle. Corner assembly rejects partial execution, global/per-cell gate failures, missing authority, and overlap between retained failures and executed states. Axis assembly requires the registered low/high pair on one pathway, keeps one-sided support separate, and emits null for empty shared support. The v2 result requires all 14 corner/pathway records, unique safe cell-pack paths/digests, and six axis/pathway summaries. Bundle validation reopens every NPZ and reconciles pathway/digest/counts; plan reconciliation also requires exact authorities, denominators, failures, and axis scales. All reject partial/tampered evidence and remain model-dependent, not causal, population, mechanism, ranking, or coaching claims.

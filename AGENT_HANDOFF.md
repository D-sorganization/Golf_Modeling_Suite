...
- #8684 governs distributed grip, shaft, and ground:
  ...
  - #8752 is **OPEN** on `research/8752-articulated-uncertainty`. The v2
    ...
    shapes. The campaign has 2/19 corners complete; stiffness-high shaft finished at 80/384 with gates passing, and its ground pathway is running (recorded as pending, not complete). The reusable `articulated_checkpoint_audit` CLI classifies the first 47/72 checkpoints as partial with seven complete of eight observed state slots; they are no-pickle, finite, uniquely identity-correct, match the explicitly required schema/design digest, and are shape/dtype-conformant with nominal. Set digest `8ba6a0dfdb10`; seven checkpoint tests pass. The independent `articulated_headline_record_audit` reads one immutable snapshot and rejects torn JSON, schema/design/config/source drift, nonprefix corners, invalid pathway states, and premature completion. It validates the live prefix as 3/19 rows, 2 fully accounted, 5 terminal pathways, stiffness-high ground active, and explicitly not release evidence; all 18 record-audit tests pass.
  ...
  - #8800 is **OPEN** and blocks #8752. Its governed generator regenerates all
    ...
    Its v1 evidence contract preserves checkpoint identity and per-cell arrays. A shared runtime authority boundary now resolves planned/feasible states without deleting the retained low-height failure, rebuilds and revalidates exact scaled case models, emits detached digest-bound provenance, rejects untyped authorities and invalid designs, and passes 14 focused authority tests; the frozen runners are not yet wired to consume it. Shaft matching is coupled/rigid station-force/dissipated-work; ground matching is coupled/fixed grip-force/total-work. Planned, feasible, and executed denominators remain separate; partial records cannot qualify.

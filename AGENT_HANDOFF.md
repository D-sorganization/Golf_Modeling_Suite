...
- #8684 governs distributed grip, shaft, and ground:
  ...
  - #8752 is **OPEN** on `research/8752-articulated-uncertainty`. The v2
    ...
    shapes. The campaign has 2/19 corners complete; stiffness-high shaft finished at 80/384 with gates passing, and its ground pathway is running (recorded as pending, not complete). The reusable `articulated_checkpoint_audit` CLI classifies the first 25/72 checkpoints as partial, no-pickle, finite, uniquely identity-correct, schema/design uniform, and shape/dtype-conformant with nominal; set digest `933ce25a5afa` and six adverse/contract tests pass.

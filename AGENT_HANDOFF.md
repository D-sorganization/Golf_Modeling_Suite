...
- #8684 governs distributed grip, shaft, and ground:
  ...
  - #8800 is **OPEN** and blocks #8752. Its governed generator regenerates all
    ...
    Common-support analysis reproduces nominal 126/384 shaft and 0/384 ground while preserving identities, denominators, failures, resolution, and unpooled secants. Nominal-only and corner-only execution identities remain distinct; corner-only cells cannot be mislabeled common unmatched. Fail-closed gate derivation combines rigid/coupled shaft and fixed/coupled ground branches, broadcasts parity across engines, and retains simultaneous failures. Cell packs derive identities/outcomes/gates from one atlas and use atomic no-pickle NPZ. Corner assembly rejects partial execution, global/per-cell failures, missing authority, and executed/failed-state overlap. Axis assembly requires registered low/high pairs, keeps one-sided support separate, and emits null for empty shared support. The v2 result requires 14 corner/pathway records, unique safe cell-pack paths/digests, and six axis/pathway summaries. Bundle and plan validation reconcile NPZ contents, authorities, denominators, failures, and scales. All reject partial/tampered evidence and remain model-dependent, not causal, population, mechanism, ranking, or coaching claims.

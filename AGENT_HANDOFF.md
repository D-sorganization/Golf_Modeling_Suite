...
- #8684 governs distributed grip, shaft, and ground:
  ...
  - #8752 is **OPEN** on `research/8752-articulated-uncertainty`. The v2
    ...
    shapes. The campaign has 2/19 corners complete; stiffness-high shaft finished at 80/384 with gates passing, and its ground pathway is running (recorded as pending, not complete).
  ...
  - #8800 is **OPEN** and blocks #8752. Its governed generator regenerates all
    ...
    Common-support analysis reproduces nominal 126/384 shaft and 0/384 ground while preserving identities, denominators, failures, resolution, and unpooled secants. Nominal-only and corner-only execution identities remain distinct; corner-only cells cannot be mislabeled common unmatched. Cell-evidence v2 stores finite corner-minus-nominal change/resolution only on persistent support and NaN/false elsewhere. Fail-closed gates combine both compared branches, broadcast parity across engines, and retain simultaneous failures. Atomic no-pickle cell packs derive identities/outcomes/gates from one atlas. Corner assembly rejects partial execution, global/per-cell failures, missing authority, and executed/failed-state overlap. Axis assembly requires registered low/high pairs, keeps one-sided support separate, and emits null for empty shared support. The v2 result requires 14 corner/pathway records, unique safe cell-pack paths/digests, and six axis/pathway summaries. Bundle/plan validation reconcile NPZ contents, authorities, denominators, failures, and scales. All reject partial/tampered evidence and remain model-dependent, not causal, population, mechanism, ranking, or coaching claims.

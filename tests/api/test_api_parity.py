...
# Routes are registered at /api prefix (see server.py register_routes call)
_API = "/api"
...
class TestCoreEndpoints:
    ...
    def test_health_check(self, client: TestClient) -> None:
        """GET /api/health returns healthy status with engine count."""
...
class TestEngineEndpoints:
    ...
    def test_list_engines(self, client: TestClient) -> None:
        """GET /api/engines returns all available engines."""
    ...
    def test_probe_known_engine(self, client: TestClient, engine_name: str) -> None:
        ...
        response = client.get(f"{_API}/engines/{engine_name}/probe")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_probe_unknown_engine(self, client: TestClient) -> None:
        ...
        response = client.get(f"{_API}/engines/totally_fake_engine/probe")
    ...
    def test_load_engine_via_api_path(self, client: TestClient) -> None:
        ...
        response = client.post(f"{_API}/engines/putting_green/load")
    ...
    def test_load_engine_via_typed_path(self, client: TestClient) -> None:
        """POST /api/engines/{type}/load accepts valid engine type."""
        response = client.post(f"{_API}/engines/putting_green/load")
        assert response.status_code in [200, 400, 500]
...
class TestSimulationEndpoints:
    ...
    def test_simulate_missing_body(self, client: TestClient) -> None:
        """POST /api/simulate rejects empty body."""
        response = client.post(f"{_API}/simulate", json={})
        assert response.status_code == 422  # Pydantic validation

    def test_simulate_invalid_engine(self, client: TestClient) -> None:
        """POST /api/simulate with unknown engine returns an error indicator."""
...
class TestAuthEndpoints:
    ...
    def test_register_missing_fields(self, client: TestClient) -> None:
        """POST /api/auth/register rejects empty body."""
        response = client.post(f"{_API}/auth/register", json={})
        assert response.status_code == 422

    def test_login_missing_credentials(self, client: TestClient) -> None:
        """POST /api/auth/login rejects empty body."""
        response = client.post(f"{_API}/auth/login", json={})
        assert response.status_code == 422
    ...
    def test_me_without_auth(self, client: TestClient) -> None:
        ...
        response = client.get(f"{_API}/auth/me")
...
class TestExportEndpoints:
    ...
    def test_export_nonexistent_task(self, client: TestClient) -> None:
        """GET /api/export/{task_id} returns 404 for unknown task."""
        response = client.get(f"{_API}/export/nonexistent-task-id")

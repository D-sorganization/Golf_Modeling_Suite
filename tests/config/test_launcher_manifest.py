...
class TestLogoValidation:
    ...
    def test_all_tiles_have_logo_files(self, manifest: LauncherManifest) -> None:
        ...
        assert not missing, f"Missing logo files for tiles: {missing}. Expected in: {ASSETS_DIR}"
...
class TestOrdering:
    ...
    def test_model_explorer_is_first(self, manifest: LauncherManifest) -> None:
        ...
        assert first.id == "model_explorer", f"First tile should be model_explorer, got: {first.id}"

    def test_ordered_ids_returns_deterministic_list(self, manifest: LauncherManifest) -> None:
...
class TestCategories:
    ...
    def test_get_tile_returns_none_for_invalid(self, manifest: LauncherManifest) -> None:

...
def test_uncertainty_claims_have_explicit_finding_outcomes(registry) -> None:
    """The v2 registry must not infer outcomes from status strings."""

    claims = {row["claim_id"]: row for row in registry["claims"]}
    for claim_id in sorted(CLAIM_IDS):
        assert claims[claim_id]["adjudication_outcome"] == "supported"

...
def test_synth_then_fit_recovers_trajectory(synth_pair_upper, sim_opts_short) -> None:
    """The synth->fit oracle makes substantial, plausible progress.
    ...
    truth ``θ`` carries a non-trivial *constant* torque from ``t=0``, so the
    synthesized swing is more dynamic than under the old reversed ordering
    (which deferred the torque to late time via the ``t^6`` term). The
    finite-difference SLSQP path at ``maxiter=20`` therefore no longer
    reaches the incidental sub-cm trajectory RMSE the old conditioning
    produced; what it *does* do — and what we assert as the wiring
    contract — is drive the cost down by a large factor while keeping
    ``θ`` plausible (well inside the bound box, not parked at a corner).
    """
    ...
    # The fit must converge to a finite optimum and make large progress.
    # The synthetic target is representable by the same forward sim, so the
    # cost landscape has a deep basin; the SLSQP path drives the running-min
    # cost down by well over an order of magnitude from its starting value.
    assert np.isfinite(result.final_rmse_m)
    history = np.asarray(result.history, dtype=np.float64)
    assert history.size >= 2, f"expected >= 2 cost evaluations; got {history.size}"
    cost0 = float(history[0])
    cost_best = float(np.min(history))
    assert cost_best < cost0 / 10.0, (
        f"optimizer made too little progress: best cost {cost_best:.4e} is "
        f"not below initial {cost0:.4e} / 10; the fit may not be wired "
        f"correctly"
    )

...
class TestErrorMetadata:
    ...
    def test_status_codes_are_valid_http(self):
        ...
        for code, metadata in ERROR_METADATA.items():
            ...
            assert status in valid_status_codes, f"Invalid status code {status} for {code}"
...
class TestAPIError:
    ...
    def test_post_init_injects_trace_context(self):
        ...
        with (
            patch("src.api.utils.error_codes.get_request_id", return_value="req_test123"),
            patch("src.api.utils.error_codes.get_trace_context", return_value=mock_context),
        ):
            error = APIError(
                code=ErrorCode.INTERNAL_ERROR,
                message="Test",

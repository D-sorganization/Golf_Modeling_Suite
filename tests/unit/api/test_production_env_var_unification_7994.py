...
``src/api/database.py`` and ``src/api/debug_guard.py`` read
...
Both spellings now resolve through
``src.shared.python.config.environment.resolve_environment`` and the gate fails
closed: if *either* variable names production, production wins.
"""
...
from src.api import database, debug_guard
...
def test_production_gate_honours_both_spellings(
    ...
    assert env_mod.is_production_environment() is expected
...
def test_resolve_environment_is_not_cached(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert env_mod.resolve_environment() == "development"

    monkeypatch.setenv("ENVIRONMENT", "production")
    assert env_mod.resolve_environment() == "production"

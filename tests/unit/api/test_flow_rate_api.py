...
def test_convert_flow_rate_invalid_category() -> None:
    ...
    assert "Unknown category" in str(response.json()["detail"])
...
def test_convert_flow_rate_invalid_source_unit() -> None:
    ...
    assert response.status_code == 422
    assert "Unknown from_unit" in str(response.json()["detail"])
...
def test_convert_flow_rate_invalid_target_unit() -> None:
    ...
    assert response.status_code == 422
    assert "Unknown to_unit" in str(response.json()["detail"])

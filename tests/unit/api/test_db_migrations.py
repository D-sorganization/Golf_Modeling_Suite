...
INITIAL_MIGRATION_PATH = (
    REPO_ROOT / "src" / "api" / "migrations" / "versions" / "20260323_0000_0001_initial_schema.py"
)


def _import_migration_module() -> ModuleType:
    """Import the initial migration module dynamically."""
    spec = importlib.util.spec_from_file_location("initial_migration", INITIAL_MIGRATION_PATH)
...
class TestInitialMigrationMetadata:
    ...
    def test_initial_migration_has_upgrade(self):
        ...
        mod = _import_migration_module()
        assert callable(getattr(mod, "upgrade", None)), "Initial migration must define upgrade()"

...
def test_release_review_covers_every_release_claim_and_closes_only_review() -> None:
    ...
    assert summary["release_claim_count"] == 31
    assert summary["reviewed_release_claim_count"] == 31
    assert summary["open_release_review_count"] == 0
    assert summary["scientifically_open_gate_count"] == 31
    assert summary["atomic_claim_count"] == 266

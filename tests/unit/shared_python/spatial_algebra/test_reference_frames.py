...
class TestReferenceFrames:
    ...
    def test_fit_instantaneous_swing_plane(self):
        ...
        fit_instantaneous_swing_plane(np.array([1,0,0]), np.array([0,0,-1]), np.array([1,0,0]))
    ...
    def test_transformer(self):
        ...
        with pytest.raises(ValueError):
            transformer.global_to_swing_plane(WrenchInFrame(np.zeros(3), np.zeros(3), ReferenceFrame.GLOBAL))
            
        with pytest.raises(ValueError):
            transformer.get_swing_plane_decomposition(WrenchInFrame(np.zeros(3), np.zeros(3), ReferenceFrame.GLOBAL))

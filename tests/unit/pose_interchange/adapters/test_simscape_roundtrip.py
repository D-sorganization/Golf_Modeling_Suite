"""Round-trip parity tests for :class:`SimscapeAdapter`."""
...
def test_simscape_roundtrip_100_random(random_poses: list[CanonicalPose]) -> None:
...
def test_simscape_q_roundtrip_identity(random_poses: list[CanonicalPose]) -> None:
    adapter = SimscapeAdapter()
    for pose in random_poses:
...
def test_simscape_rejects_short_q() -> None:
    adapter = SimscapeAdapter()

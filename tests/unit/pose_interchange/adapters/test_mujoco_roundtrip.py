"""Round-trip parity tests for :class:`MujocoAdapter`."""
...
def test_mujoco_roundtrip_100_random(random_poses: list[CanonicalPose]) -> None:
...
def test_mujoco_q_roundtrip_identity(random_poses: list[CanonicalPose]) -> None:
    adapter = MujocoAdapter()
    for pose in random_poses:
...
def test_mujoco_quaternion_is_w_first(random_poses: list[CanonicalPose]) -> None:
...
def test_mujoco_rejects_short_q() -> None:
    adapter = MujocoAdapter()

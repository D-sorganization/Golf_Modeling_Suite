"""Round-trip parity tests for :class:`DrakeAdapter`."""
...
def test_drake_roundtrip_100_random(random_poses: list[CanonicalPose]) -> None:
...
def test_drake_q_roundtrip_identity(random_poses: list[CanonicalPose]) -> None:
    """from_canonical(to_canonical(q)) ~= q to 1e-9 over random samples."""
    adapter = DrakeAdapter()
    for pose in random_poses:

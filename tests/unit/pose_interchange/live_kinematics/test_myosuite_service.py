"""Regression tests for the MyoSuite :class:`LiveKinematicsService`.

These tests cover:

- The fall-back contract when neither :mod:`mujoco` nor :mod:`myosuite`
  is importable (registry must yield a
  :class:`MockKinematicsService` whose ``engine_name`` is ``"myosuite"``).
- A mock-fallback round-trip parity check: loading the canonical
  reference setup pose through the MyoSuite factory must produce the
  same link transforms (within ``1e-9``) as a stand-alone
  :class:`MockKinematicsService` configured for ``engine_name="myosuite"``.
- The real-engine ``set_pose`` path (with a fake :mod:`mujoco`) writes
  the canonical pose into ``qpos`` using the MuJoCo free-joint layout —
  mirrors the MuJoCo sibling exactly because MyoSuite is MJCF-backed.

The third group is the only one that needs to monkeypatch :mod:`mujoco`;
the first two only exercise pure-Python paths.
"""
...
def test_create_myosuite_service_falls_back_to_mock_without_wheel() -> None:
    """When ``_myosuite_is_importable`` is forced to ``False`` the factory
    must return a MyoSuite-named :class:`MockKinematicsService`."""
    with patch(
        "src.shared.python.pose_interchange.services.myosuite._myosuite_is_importable",
        return_value=False,
    ):
        service = create_myosuite_service()
    assert isinstance(service, MockKinematicsService)
    assert service.engine_name == ENGINE_NAME


def test_mock_fallback_get_link_transforms_matches_reference(tmp_path: Path) -> None:
    """Loading the canonical address pose through the MyoSuite factory's
    mock path must yield byte-identical transforms vs a stand-alone
    :class:`MockKinematicsService`. This is the issue #6091
    parity-with-MockKinematicsService acceptance criterion.
    """
    with patch(
        "src.shared.python.pose_interchange.services.myosuite._myosuite_is_importable",
        return_value=False,
    ...
    reference_mock = MockKinematicsService(engine_name=ENGINE_NAME)
    pose = canonical_from_reference_setup()

    for svc in (service, reference_mock):
        svc.load(tmp_path / "ignored.xml")
        svc.set_pose(pose)

    actual = service.get_link_transforms()
    expected = reference_mock.get_link_transforms()

    assert set(actual.keys()) == set(expected.keys())
    for name, transform in actual.items():
        np.testing.assert_allclose(transform, expected[name], atol=1e-9)


# ---- Real-engine path tests (with fake :mod:`mujoco`) ----------------------


def _install_reference_pose_stub(monkeypatch: pytest.MonkeyPatch) -> str:
    """Install minimal motion_matching diagnostics stubs.

    Mirrors the MuJoCo sibling test helper so the service module can be
    re-imported with a single-joint canonical layout, keeping the test
    independent of the real motion_matching package.
    """
...
def test_set_pose_uses_joint_addresses_for_fixed_base_models(
    ...
    """MyoSuite delegates ``set_pose`` to the MuJoCo free-joint layout,
    so a fixed-base, single-hinge model should write radians at qpos[0]."""
...
def test_set_pose_respects_nonzero_free_joint_address(
    ...
    """The free-joint pelvis prefix (``[x,y,z,qw,qx,qy,qz]``) should be
    written verbatim, identical to the MuJoCo sibling."""

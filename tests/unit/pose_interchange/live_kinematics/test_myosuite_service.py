"""Regression tests for MuJoCo live-kinematics pose application."""
...
def _import_service_module(
    ...
    fake_myosuite: types.ModuleType,
):
    canonical_joint = _install_reference_pose_stub(monkeypatch)
    monkeypatch.setitem(sys.modules, "mujoco", fake_myosuite)
...
def _build_fake_myosuite(joint_names: list[str]) -> types.ModuleType:
...
def test_set_pose_uses_joint_addresses_for_fixed_base_models(
    ...
    fake_myosuite = _build_fake_myosuite(["mj_trail_elbow"])
    ...
    model = SimpleNamespace(
        ...
        jnt_type=np.array([fake_myosuite.mjtJoint.mjJNT_HINGE]),
    )
    ...
    assert len(fake_myosuite.forward_calls) == 1


def test_set_pose_respects_nonzero_free_joint_address(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fake_myosuite = _build_fake_myosuite(["mj_trail_elbow", "root_free"])
    service_module, canonical_joint = _import_service_module(
        monkeypatch,
        fake_myosuite,
    )
    ...
    model = SimpleNamespace(
        ...
        jnt_type=np.array(
            [
                fake_myosuite.mjtJoint.mjJNT_HINGE,
                fake_myosuite.mjtJoint.mjJNT_FREE,
            ]
        ),
    )

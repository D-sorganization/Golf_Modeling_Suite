...
class TestParametricBuilderRoundtrip:
    ...
    def test_default_humanoid_roundtrip(self) -> None:
        ...
        assert len(parsed.links) > 10, f"Expected >10 links for humanoid, got {len(parsed.links)}"
    ...
    def test_parametric_builder_produces_valid_xml(self) -> None:
        ...
        for link_elem in root.findall(".//link"):
            ...
            assert inertial is not None, f"Link '{link_elem.get('name')}' missing <inertial>"
            mass = inertial.find("mass")
            assert mass is not None
            mass_val = float(mass.get("value", "0"))
            assert mass_val > 0, f"Link '{link_elem.get('name')}' has non-positive mass {mass_val}"
...
class TestCompositeJointExpansion:
    ...
    def test_universal_joint_expands_to_two_revolute(self) -> None:
        ...
        # 2 revolute joints
        revolute_joints = [j for j in parsed.joints if j.joint_type == JointType.REVOLUTE]

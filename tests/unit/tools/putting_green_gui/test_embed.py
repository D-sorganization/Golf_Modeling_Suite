"""Smoke tests for the putting_green_gui EmbeddableTool adapter.
...
pytest.importorskip("PyQt6", reason="PyQt6 is required for putting_green_gui")
...
def adapter():
    ...
    from src.tools.putting_green_gui.gui import _EmbedAdapter
...
def test_tool_id_is_stable() -> None:
    ...
    assert _EmbedAdapter.tool_id == "putting_green_gui"
    assert _EmbedAdapter().tool_id == "putting_green_gui"
...
def test_gui_module_registers_adapter_on_import() -> None:
    ...
    import src.tools.putting_green_gui.gui  # noqa: F401

    registered = get_embeddable_tool("putting_green_gui")
    assert registered is not None
    assert registered.tool_id == "putting_green_gui"

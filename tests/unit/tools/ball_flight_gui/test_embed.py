"""Smoke tests for the ball_flight_gui EmbeddableTool adapter.

These tests are strictly headless — PyQt6 is mocked via
...
pytest.importorskip("PyQt6", reason="PyQt6 is required for ball_flight_gui")
...
def adapter():
    ...
    from src.tools.ball_flight_gui.gui import _EmbedAdapter
...
def test_tool_id_is_stable() -> None:
    ...
    assert _EmbedAdapter.tool_id == "ball_flight_gui"
    assert _EmbedAdapter().tool_id == "ball_flight_gui"
...
def test_gui_module_registers_adapter_on_import() -> None:
    ...
    import src.tools.ball_flight_gui.gui  # noqa: F401

    registered = get_embeddable_tool("ball_flight_gui")
    assert registered is not None
    assert registered.tool_id == "ball_flight_gui"

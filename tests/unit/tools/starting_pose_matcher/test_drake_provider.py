...
def test_vocabulary_mapping():
    ...
    for matcher_name in MATCHER_TO_DRAKE:
        assert matcher_name in required_names, f"Unexpected vocabulary: {matcher_name}"


def test_model_xml_file_type_inference():
    """Drake in-memory XML loading must pass an explicit parser file type."""
    from src.tools.starting_pose_matcher.providers.drake import (
        _infer_model_file_type,
    )

    assert _infer_model_file_type(MINIMAL_URDF) == "urdf"
    assert (
        _infer_model_file_type("<sdf version='1.10'><model name='m'/></sdf>") == "sdf"
    )

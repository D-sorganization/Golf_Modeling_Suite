...
def test_openpose_missing_keypoints():
    ...
    assert "mp" in skeleton
    assert "torso" in skeleton
    assert "hub" in skeleton
    assert "mp" not in missing
    assert "torso" not in missing
    assert "hub" not in missing

    # Clubhead should always be missing (not observable from body)
    assert "ch" in missing


def test_openpose_missing_keypoints_preserves_low_confidence_observed_keypoints():
    """Test low-confidence observed keypoints are still reported as missing."""
    from src.tools.starting_pose_matcher.providers.openpose import OpenPoseProvider

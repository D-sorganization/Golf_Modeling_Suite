...
def _pinocchio_robotics_available() -> bool:
    try:
        import pinocchio
    except ImportError:
        return False
    ...
    try:
        ...
        frame_names = {frame.name for frame in getattr(model, "frames", ())}
        joint_names = set(getattr(model, "names", ()))
        return "hip" in frame_names or "hip" in joint_names
    except Exception:
        return False
    finally:
        os.unlink(urdf_path)
...
def test_vocabulary_mapping():
    ...
    for matcher_name in MATCHER_TO_PINOCCHIO:
        assert matcher_name in required_names, f"Unexpected vocabulary: {matcher_name}"


def test_hip_aliases_are_preserved():
    """The reverse map should accept either hip or pelvis for matcher hip."""
    from src.tools.starting_pose_matcher.providers.pinocchio import (
        MATCHER_TO_PINOCCHIO,
    )

    assert MATCHER_TO_PINOCCHIO["hip"] == ("hip", "pelvis")
...
def test_provider_with_minimal_model():
    ...
    if not _pinocchio_robotics_available():
        pytest.skip("Pinocchio robotics bindings not installed")

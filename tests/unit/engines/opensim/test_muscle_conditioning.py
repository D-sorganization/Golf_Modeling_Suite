...
from src.engines.physics_engines.opensim.python.muscle_analysis import OpenSimMuscleAnalyzer
...
def test_mass_matrix_conditioning_fallback(caplog):
    ...
    analyzer.compute_muscle_joint_torques = MagicMock(return_value={"muscle1": np.array([1.0, 0.0])})
    ...
    with caplog.at_level(logging.WARNING):
        ...
        with patch('src.engines.physics_engines.opensim.python.muscle_analysis.opensim') as mock_opensim_mod:
    ...
    assert any("Mass matrix ill-conditioned" in record.message for record in caplog.records)

"""Unit tests for the Starting-Pose Matcher (relocated to src/tools/).
...
The package has been relocated to ``src/tools/starting_pose_matcher/``.
These tests verify both the new location and the deprecation shim at the old path.
"""
...
# --------------------------------------------------------------------------- #
# Locate the matcher modules (new canonical location)                         #
# --------------------------------------------------------------------------- #

_REPO = Path(__file__).resolve().parents[5]
_NEW_MATCHER_DIR = _REPO / "src" / "tools" / "starting_pose_matcher"
_CORE_PY = _NEW_MATCHER_DIR / "core.py"
_MATCHER_PY = _NEW_MATCHER_DIR / "gui.py"
_WIFFLE_XLSX = _REPO / "src" / "engines" / "Simscape_Multibody_Models" / "3D_Golf_Model" / "matlab" / "src" / "apps" / "golf_gui" / "Motion Capture Plotter" / "Wiffle_ProV1_club_3D_data.xlsx"
...
def _load_core():
    ...
    if str(_NEW_MATCHER_DIR) not in sys.path:
        sys.path.insert(0, str(_NEW_MATCHER_DIR))

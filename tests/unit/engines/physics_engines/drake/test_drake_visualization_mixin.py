...
class TestDrakeVisualizationMixin:
    ...
    def test_update_visualization(self, dummy_viz: DummyVizGUI) -> None:
        ...
        dummy_viz.visualizer.update_frame_transforms.assert_called_once_with(
            dummy_viz.context
        )
        dummy_viz.visualizer.update_com_transforms.assert_called_once_with(
            dummy_viz.context
        )
    ...
    @patch(
        "src.engines.physics_engines.drake.python.src.drake_visualization_mixin.HAS_MATPLOTLIB",
        False,
    )
    def test_show_induced_acceleration_plot_no_matplotlib(
        self, dummy_viz: DummyVizGUI
    ) -> None:
        """Test plotting aborts if matplotlib is not available."""
        with patch(
            "src.engines.physics_engines.drake.python.src.drake_visualization_mixin.QtWidgets.QMessageBox.warning"
        ) as mock_warn:
            dummy_viz._show_induced_acceleration_plot()
            mock_warn.assert_called_once_with(
                dummy_viz, "Error", "Matplotlib not found."
            )
    ...
    def test_show_counterfactuals_plot_no_matplotlib(
        self, dummy_viz: DummyVizGUI
    ) -> None:
        """Test plotting aborts if matplotlib is not available."""
        with patch(

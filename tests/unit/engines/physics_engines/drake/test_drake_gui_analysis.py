...
class TestAnalysisMixin:
    ...
    @patch(
        "src.engines.physics_engines.drake.python.src.drake_gui_analysis.DrakeInducedAccelerationAnalyzer"
    )
    def test_compute_live_analysis(
        self, mock_analyzer_cls: MagicMock, dummy_gui: DummyGUI
    ) -> None:
        ...
        mock_analyzer.compute_counterfactuals.return_value = {
            "ztcf_accel": np.array([2.0])
        }
        ...
        mock_analyzer.compute_counterfactuals.assert_called_once_with(
            dummy_gui.eval_context
        )
    ...
    def test_compute_live_analysis_missing_preconditions(
        self, dummy_gui: DummyGUI
    ) -> None:
    ...
    @patch(
        "src.engines.physics_engines.drake.python.src.drake_gui_analysis.HAS_MATPLOTLIB",
        False,
    )
    def test_show_induced_acceleration_plot_no_matplotlib(
        self, dummy_gui: DummyGUI
    ) -> None:
        """Test plotting aborts if matplotlib is not available."""
        with patch(
            "src.engines.physics_engines.drake.python.src.drake_gui_analysis.QtWidgets.QMessageBox.warning"
        ) as mock_warn:
            dummy_gui._show_induced_acceleration_plot()
            mock_warn.assert_called_once_with(
                dummy_gui, "Error", "Matplotlib not found."
            )

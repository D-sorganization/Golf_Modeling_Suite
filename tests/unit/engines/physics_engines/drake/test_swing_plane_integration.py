...
from src.engines.physics_engines.drake.python.swing_plane_integration import (
...
class TestDrakeSwingPlaneAnalyzer:
    @patch(
        "src.engines.physics_engines.drake.python.swing_plane_integration.SwingPlaneAnalyzer"
    )
    ...
    def test_analyze_from_drake_context(self, mock_analyze, dummy_metrics):
        ...
        metrics = analyzer.analyze_from_drake_context(
            context_mock, plant_mock, club_body_index=1, num_samples=3
        )
        assert metrics == dummy_metrics
    ...
    def test_integrate_with_optimization(self, dummy_metrics):
        ...
        analyzer.integrate_with_optimization(
            optimizer_mock, swing_plane_constraint_weight=2.0
        )

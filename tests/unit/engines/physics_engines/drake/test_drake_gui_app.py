...
class TestDrakeSimApp:
    ...
    @patch(
        "src.engines.physics_engines.drake.python.src.drake_gui_app.SimulationGUIBase.__init__"
    )
    ...
    def test_init(
        ...
        mock_timer.start.assert_called_once_with(
            1
        )  # TIME_STEP_S * MS_PER_SECOND = 0.001 * 1000 = 1
    ...
    def test_get_joint_names(self, mock_init: MagicMock) -> None:
        ...
        with patch(
            "src.engines.physics_engines.drake.python.src.drake_gui_app.JointIndex",
    ...
    def test_scan_urdf_models(self, mock_init: MagicMock, tmp_path) -> None:
        ...
        with patch(
            "src.engines.physics_engines.drake.python.src.drake_gui_app.Path"
        ) as mock_path:

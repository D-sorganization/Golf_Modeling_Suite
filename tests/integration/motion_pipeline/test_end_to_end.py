...
def load_golden_fixture(fixture_name: str, fmt: str) -> dict[str, Any]:
    ...
    if not fixture_path.exists():
        # Fail loudly rather than silently skipping. A missing required
        # fixture means this parametrized case provides zero real coverage,
        # which would let the end-to-end suite report green while nothing
        # was actually exercised. Either commit the fixture or remove the
        # combination from PIPELINE_COMBINATIONS.
        raise FileNotFoundError(
            f"Required golden fixture is missing: {fixture_path}. "
            "Commit the fixture under tests/data/motion_pipeline/golden/ "
            "or drop this combination from PIPELINE_COMBINATIONS."
        )

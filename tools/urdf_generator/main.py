...
class URDFGenerator(QtWidgets.QMainWindow):
    ...
    def _add_joint_dialog(self) -> None:
        ...
        if len(link_names) < 2:
             QtWidgets.QMessageBox.warning(self, "Not enough links", "You need at least 2 links to create a joint.")
             return
    ...
    def _generate_urdf_xml(self) -> str:
        ...
        for joint in self.joints:
            ...
            if joint["type"] != "fixed":
                ...
                ET.SubElement(joint_elem, "limit", lower="-3.14", upper="3.14", effort="100", velocity="10")
    ...
    def _save_urdf(self) -> None:
        filename, _ = QtWidgets.QFileDialog.getSaveFileName(self, "Save URDF", "", "URDF Files (*.urdf)")
        if filename:
            with open(filename, "w") as f:
                f.write(self._generate_urdf_xml())
            QtWidgets.QMessageBox.information(self, "Saved", f"URDF saved to {filename}")

    def _load_urdf(self) -> None:
        """Primitive URDF loader (basic parsing)."""
        filename, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Load URDF", "", "URDF Files (*.urdf)")
        if not filename:
            return
    ...
    def _visualize_model(self) -> None:
        ...
        try:
            ...
            from engines.physics_engines.mujoco.python.mujoco_humanoid_golf.sim_widget import MuJoCoSimWidget
            ...
            # Remove temp file after closing? Or keep for debugging.
             ...
             QtWidgets.QMessageBox.warning(self, "Visualization Error", "Could not import MuJoCo engine components.")
        except Exception as e:
             QtWidgets.QMessageBox.critical(self, "Visualization Error", f"Error launching visualization: {e}")

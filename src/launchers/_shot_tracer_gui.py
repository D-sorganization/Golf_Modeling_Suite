...
def _sync_public_overrides() -> None:
    ...
    for name in _PATCHABLE_PUBLIC_NAMES:
        if name in public_module.__dict__:
            value = public_module.__dict__[name]
            if name in {"PYQTGRAPH_AVAILABLE", "gl"} and value is None:
                continue
            globals()[name] = value


def _load_pyqtgraph() -> bool:
    global PYQTGRAPH_AVAILABLE, gl, pg
    _sync_public_overrides()
    if PYQTGRAPH_AVAILABLE is not None:
        return PYQTGRAPH_AVAILABLE

    try:
        import pyqtgraph as loaded_pg
        import pyqtgraph.opengl as loaded_gl
    except ImportError:

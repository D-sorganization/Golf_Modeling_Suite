...
This module is the thin orchestrator: UI layout and wiring only.
Domain logic lives in the focused mixin modules:

- :mod:`.simulation_controls_mixin` -- play/pause, reset, record, screenshot
- :mod:`.actuator_controls_mixin`   -- actuator group creation and callbacks
- :mod:`.kinematic_controls_mixin`  -- joint sliders / spinboxes
- :mod:`.actuator_detail_dialog`    -- per-actuator parameter dialog
"""
...
class ControlsTab(
    ...
    Inherits behaviour from three focused mixins; this class is
    responsible only for wiring them together into a single QWidget.
    """
    ...
    def __init__(
        ...
        # State storage used by the actuator mixin
    ...
    def _setup_ui(self) -> None:
        """Assemble the full controls interface from panel builders."""
    ...
    def _create_simulation_buttons(self, main_layout: QtWidgets.QVBoxLayout) -> None:
        """Build play/pause, reset, screenshot, and record buttons."""
    ...
    def _create_recording_info(self, main_layout: QtWidgets.QVBoxLayout) -> None:
        """Add recording status label and live-analysis checkbox."""
        if main_layout is None:
            raise ValueError("main_layout must be provided")
    ...
    def _create_dynamic_controls(self, main_layout: QtWidgets.QVBoxLayout) -> None:
        """Create the scrollable actuator controls area (dynamic mode)."""
        if main_layout is None:
            raise ValueError("main_layout must be provided")
    ...
    def _create_kinematic_controls(self, main_layout: QtWidgets.QVBoxLayout) -> None:
        """Create the scrollable joint controls area (kinematic mode)."""
        if main_layout is None:
            raise ValueError("main_layout must be provided")
    ...
    # ------------------------------------------------------------------
    # Signal Handlers (connected by main window or physics tab)
    # ------------------------------------------------------------------

    def on_model_loaded(self, model_name: str, config: dict) -> None:
        """Handle a new model being loaded from PhysicsTab."""

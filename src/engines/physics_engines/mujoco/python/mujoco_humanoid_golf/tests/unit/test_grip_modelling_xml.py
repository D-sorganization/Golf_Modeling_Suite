...
"""
    ...
    from src.engines.physics_engines.mujoco.python.mujoco_humanoid_golf.grip_modelling_tab import GripModellingTab
    ...
    tab = DummyTab()
    tab._inline_hand_includes = GripModellingTab._inline_hand_includes.__get__(tab, DummyTab)
    ...
    result = tab._inline_hand_includes(scene_xml, Path("dummy_scene.xml"), tmp_path, False)
    ...
    assert contact_start > worldbody_end, "<contact> should be placed after </worldbody>"
    assert tendon_start > worldbody_end, "<tendon> should be placed after </worldbody>"
    assert actuator_start > worldbody_end, "<actuator> should be placed after </worldbody>"
    assert equality_start > worldbody_end, "<equality> should be placed after </worldbody>"

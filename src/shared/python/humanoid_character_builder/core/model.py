...
from src.shared.python.humanoid_character_builder.contracts import precondition
...
class SupportPolygon:
    ...
    def distance_to_edge(self, point: tuple[float, float]) -> float:
        ...
        for i in range(n):
            ...
            diff = p1 - p2
            l2: float = float(np.vdot(diff, diff))  # ⚡ Bolt: np.vdot is ~3x faster than np.sum(diff**2)
            if l2 == 0:
...
class HumanoidModel:
    ...
    @precondition(
        lambda self, links, joints, root_link_name: root_link_name in links,

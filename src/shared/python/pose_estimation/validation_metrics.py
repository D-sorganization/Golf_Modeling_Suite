...
def compute_marker_rmse(
    ...
    # Per-marker RMSE
    # ⚡ Bolt: Explicit element-wise calculation is faster than np.linalg.norm(..., axis=2)
    sq_errors = np.sum((pred - ref) ** 2, axis=-1)  # [frames x markers]
    per_marker_rmse = np.sqrt(np.mean(sq_errors, axis=0))  # [markers]
    aggregate_rmse = float(np.sqrt(np.mean(sq_errors)))

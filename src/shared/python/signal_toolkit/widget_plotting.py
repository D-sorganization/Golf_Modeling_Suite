...
class PlottingMixin:
    ...
    def _update_plot(
        ...
        if self.current_signal is None:  # type: ignore[attr-defined]
            self.canvas.draw()  # type: ignore[attr-defined]
            return

        # Plot current signal
        self.canvas.axes.plot(  # type: ignore[attr-defined]
            self.current_signal.time,  # type: ignore[attr-defined]
            self.current_signal.values,  # type: ignore[attr-defined]
            color="#4da6ff",
            linewidth=1.5,
            label="Signal",
        )
        ...
        if self.show_tangent_check.isChecked():  # type: ignore[attr-defined]
            tangent = compute_tangent_line(
                self.current_signal,  # type: ignore[attr-defined]

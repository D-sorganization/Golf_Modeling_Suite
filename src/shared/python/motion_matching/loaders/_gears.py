"""Gears C3D marker convention helpers (private).

Implements the validated schema for the Gears mocap files in
``src/engines/Simscape_Multibody_Models/3D_Golf_Model/matlab/Data/Gears C3D
...
class GearsClubPose:
    """Per-frame club kinematics derived from the Gears clusters.
...
def is_gears_schema(filename: str | None, marker_labels: list[str]) -> bool:
    """True if the C3D file follows the Gears convention.

    A file is considered Gears-formatted if **either**:
...
# ---------------------------------------------------------------------------
# Top-level Gears extraction
# ---------------------------------------------------------------------------


def extract_gears_pose(
    points: dict[str, np.ndarray],
    *,
    address_frame: int = 0,
    convert_to_z_up: bool = True,
) -> GearsClubPose:
    """Extract clubhead/butt centroids + rigid-body pose from Gears markers.
    ...
    Returns:
        :class:`GearsClubPose` with ``clubhead`` and ``butt`` centroids
    ...
    if missing:
        raise ValueError(f"Missing required Gears markers: {missing}")

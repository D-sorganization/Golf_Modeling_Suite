"""Quaternion helpers shared by the loaders.

We use the [w, x, y, z] convention (scalar first) throughout so it matches the
MATLAB ``quaternion`` class and the ``ClubTarget`` schema in CLUB_IK_SPEC.md.
"""
...
def rotmat_to_quat(rot: np.ndarray) -> np.ndarray:
    """Convert a rotation matrix (or stack thereof) to a unit quaternion.

    Args:
        rot: ``(3, 3)`` or ``(N, 3, 3)`` float array. Each ``3x3`` slice must be
            a proper rotation matrix.

    Returns:
        ``(4,)`` or ``(N, 4)`` ``float64`` array of unit quaternions in
        ``[w, x, y, z]`` order, with ``w >= 0`` (sign canonicalised).

    Raises:
        ValueError: if the input does not have a trailing ``(3, 3)`` shape.
    """
    ...
    raise ValueError(f"rot must have shape (3,3) or (N,3,3); got {arr.shape}")


def _single_rotmat_to_quat(r: np.ndarray) -> np.ndarray:
    """Shepperd's method for a single rotation matrix.

    Avoids numerical pitfalls of the naive trace formula when ``trace(R) ≈ -1``.
    """
...
def quat_inverse_distance(q1: np.ndarray, q2: np.ndarray) -> float:
    ...
    Returns ``0`` when ``q1 ≈ ±q2``. Bounded by ``pi/2`` (impossible to be
    further apart once ``q ↔ -q`` is identified).
    """

...
class NonlinearDynamicsMixin:
    ...
    def compute_rqa_metrics(
        ...
        n_diag_points = sum(diagonal_lengths)
        ...
        n_vert_points = sum(vertical_lengths)
    ...
    def compute_correlation_dimension(
        ...
        for r in radii:
            count = np.count_nonzero(dists < r)
    ...
    def estimate_lyapunov_exponent(  # noqa: C901
        ...
        for i in range(max_steps):
            ...
            if len(valid_dists) > 0:
                divergence[i] += np.log(valid_dists).sum()
    ...
    def compute_fractal_dimension(
        ...
        for k in range(1, k_max + 1):
            ...
            for m_val in range(k):
                ...
                if n_intervals > 0:
                    L_m_k += m_diffs.sum() * (N - 1) / (n_intervals * k)

...
Lazily imports :mod:`pydrake` and loads a URDF via the multibody
plant's :class:`pydrake.multibody.parsing.Parser`.  Maps
:class:`CanonicalPose` to joint-position vector ``q`` using
:class:`DrakeAdapter`, and returns world-frame transforms via
``plant.EvalBodyPoseInWorld``.  ``step()`` advances a Drake
:class:`Simulator`; ``reset()`` restores the stored default
:class:`Context`.

Falls back to :class:`MockKinematicsService` when :mod:`pydrake` is
not installed.
"""

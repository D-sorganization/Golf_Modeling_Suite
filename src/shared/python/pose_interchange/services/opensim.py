...
:class:`opensim.Model`.  Maps :class:`CanonicalPose` to coordinate
values using :class:`OpenSimAdapter`, and returns body transforms via
``getTransformInGround()``.  ``step()`` integrates the simulation state
forward using :class:`opensim.Manager`.

Falls back to :class:`MockKinematicsService` when the OpenSim wheel is
not installed.
"""

...
:func:`create_myosuite_service` falls back to a
...
def create_myosuite_service() -> LiveKinematicsService:
    ...
    if _mujoco_is_importable():
        logger.debug(
            "create_myosuite_service: mujoco importable, returning real service."
        )
        return MyoSuiteKinematicsService()
    logger.info(
        "create_myosuite_service: mujoco not importable, "

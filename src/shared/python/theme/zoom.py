...
_QOBJECT_BASE: type[object] = QObject if isinstance(QObject, type) else object
...
class ApplicationZoomController(_QOBJECT_BASE):  # type: ignore[misc, valid-type]
    """QApplication event filter that applies Chrome-style UI zoom."""

    zoomChanged = pyqtSignal(int) if isinstance(QObject, type) else _FallbackSignal()
    ...
    def eventFilter(
        ...
        event_filter = getattr(super(), "eventFilter", None)
        if callable(event_filter):
            return bool(event_filter(obj, event))
        return False
...
def _point_size(font: QFont) -> float:
    ...
    if point_size > 0:
        return point_size
    try:
        fallback = float(font.pointSize())
    except (TypeError, ValueError):
        fallback = 10.0
    return fallback or 10.0
...
def _has_control(modifiers: Qt.KeyboardModifier) -> bool:
    if modifiers == Qt.KeyboardModifier.NoModifier:
        return False
    modifier_bits = cast(Any, modifiers)
    return bool(modifier_bits & Qt.KeyboardModifier.ControlModifier)

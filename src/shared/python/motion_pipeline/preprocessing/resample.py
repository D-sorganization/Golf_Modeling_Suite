...
def _resample_keypoints(
    ...
    new_timestamps = np.linspace(seq.frames[0].timestamp, seq.frames[-1].timestamp, num_new_frames)
    ...
    for i in range(data.shape[1]):
        for j in range(data.shape[2]):
            resampled_data[:, i, j] = np.interp(new_timestamps, timestamps, data[:, i, j])
    
    # Reconstruct frames
    new_frames = _array_to_keypoint_frames_at_timestamps(seq, resampled_data, new_timestamps)
...
def _resample_markers(
    ...
    num_new_frames = int(duration * target_fps) + 1
    new_timestamps = np.linspace(traj.frames[0].timestamp, traj.frames[-1].timestamp, num_new_frames)
    ...
    # Reconstruct frames
    new_frames = _array_to_marker_frames_at_timestamps(traj, resampled_data, new_timestamps)
...
def _array_to_keypoint_frames_at_timestamps(
    ...
    for i, ts in enumerate(timestamps):
        ...
        for j in range(data.shape[1]):
            kp = seq.frames[0].keypoints[j] if j < len(seq.frames[0].keypoints) else None
            if kp:
                new_kp = Keypoint(
                    x=data[i, j, 0],
                    y=data[i, j, 1],
        ...
        new_frames.append(KeypointFrame(
            timestamp=float(ts),

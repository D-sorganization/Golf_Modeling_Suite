"""Core syngas compression calculation engine (GUI-free).

Extracted from ``syngas_compression_calculator.py`` to separate domain logic
from the PyQt6 presentation layer (Issue #2892).

Public API
----------
- :class:`CompressionStage` -- dataclass for per-stage parameters
- :class:`SyngasCompressionEngine` -- pure-Python calculation engine
"""
...
try:
    ...
    def validate_gas_composition(comp: dict) -> tuple[bool, str]:  # type: ignore[misc]

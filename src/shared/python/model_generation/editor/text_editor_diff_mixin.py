...
class TextEditorDiffMixin:
    ...
    def get_diff_from_original(self) -> DiffResult:
        ...
        """
        return self._compute_diff(self._original_content, self._content)  # type: ignore[return-value]
    ...
    def get_diff_between_versions(
        ...
        Returns:
            DiffResult with changes
        """
        if version_a < 0 or version_a >= len(self._history):  # type: ignore[union-attr]
            raise IndexError(f"Invalid version index: {version_a}")
        if version_b < 0 or version_b >= len(self._history):  # type: ignore[union-attr]
            raise IndexError(f"Invalid version index: {version_b}")

        content_a = self._history[version_a].content  # type: ignore[union-attr]
        content_b = self._history[version_b].content  # type: ignore[union-attr]
    ...
    def get_diff_with_string(self, other_content: str) -> DiffResult:
        ...
        Returns:
            DiffResult with changes
        """
        return self._compute_diff(self._content, other_content)  # type: ignore[return-value]
    ...
    def get_side_by_side_diff(
        ...
        assert context_lines is not None, "context_lines must be provided"
        if original is None:
            original = self._original_content  # type: ignore[union-attr]
        if modified is None:
            modified = self._content  # type: ignore[union-attr]

...
| `skeleton_provider.py` | Provider contract, metadata schema, shared joint vocabulary, and typed provider errors.                                                                                                                |
| `providers/`           | Lazy provider registry plus the default Simscape JSON provider. Backend-specific providers are intentionally not imported until selected.                                                              |
...
`simscape-json`, `simscape-live`, `mujoco`, `drake`, `pinocchio`,
`opensim`, `openpose`, and `mediapipe`.

`simscape-json` is the default and reads `simscape_skeleton_<pose>.json`
from this package directory, falling back to FK-derived skeletons when
JSON files are absent. The other IDs are registered as typed
`ProviderUnavailableError` stubs so provider selection can show
user-actionable messages without importing optional engines.

Every skeleton provider must return these shared joints for each pose:

`hip`, `spine`, `torso`, `hub`, `ls`, `rs`, `le`, `re`, `lw`, `rw`,
`mp`, and `ch`.

Session files can persist `provider.metadata.to_session_dict()` alongside
future provider config. The GUI still needs the follow-up #4403 wiring to
select providers instead of using the current direct Simscape JSON paths.
The target-loader follow-up #4404 should adapt canonical `ClubTarget`
objects from `src/shared/python/motion_matching/load_club_target.py`
instead of extending matcher-local xlsx parsing.
...
- #4388 provider contract / registry (open) — this package owns the
  foundation contract
- #4403 provider selection in the GUI (open)
- #4404 canonical `ClubTarget` target-loader adapter (open)

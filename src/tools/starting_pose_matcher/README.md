...
| `gui.py`                | PyQt6 `QMainWindow`. Pose slots, frame scrubber, playback (mocap / skeleton / both), shaft-snap auto-align, transform sliders, Simscape input-MAT copy save, traces, session save/load, help popups, resizable layout. |
| `providers/simscape.py` | Simscape-specific providers and input-MAT editor helpers. Discovers, validates, overlays, and saves `*StartPosition*` / `*StartVelocity*` scalar fields with SciPy `loadmat` / `savemat`.                              |
...
- **Simscape input MAT** (optional). The Output panel can load an
  existing `3DModelInputs*.mat`, discover editable scalar start fields,
  show translation values in metres and joint values in degrees, overlay
  the current matcher transform, and save a new MAT copy.

## Simscape Input MAT Workflow

The MAT editor is Simscape-provider scoped. It discovers fields named
`TranslationStartPositionX/Y/Z` plus scalar `*StartPosition*` and
`*StartVelocity*` variables such as `HipStartPositionZ`,
`LSStartPositionY`, `LEStartPosition`, and future full-body names like
`LHipStartPositionX`, `LKneeStartPosition`, or
`LAnkleStartPositionX`.

Legacy `3D_Golf_Model` input files must provide the current required
start fields and may omit future full-body-only fields. The full-body
model path requires at least one full-body start field so missing new
model inputs do not silently default to zero.

Saving uses `<source>_starting_pose_<timestamp>.mat` by default and
refuses to overwrite an existing file unless the caller explicitly asks
for overwrite behavior. Session JSON records only the source and
written MAT paths; it never embeds MAT payloads.
...
- `3DModelInputs_starting_pose_<timestamp>.mat` — a copied Simscape
  input MAT with edited scalar start fields and optional matcher
  transform overlay applied. Canonical repo inputs are not mutated by

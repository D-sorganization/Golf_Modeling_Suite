...
| File                   | Purpose                                                                                                                                                                                                |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `core.py`              | Pure-data + math (no Qt). Dataclasses, canonical `ClubTarget` adapter, trajectory CSV loader, shaft-snap math, FK-derived fallback skeletons.                                                          |
...
- `src.shared.python.motion_matching.load_club_target.load_club_target` —
  canonical source for Wiffle xlsx and C3D club-target loading. The
  matcher only adapts the returned `ClubTarget` into its display columns
  (`mid_*` and `club_*`); target unit conversion stays in the shared
  loader path.
...
- **Wiffle xlsx** (`Wiffle_ProV1_club_3D_data.xlsx`). Positions are in
  **centimetres** (despite the spreadsheet's "Definitions" tab claiming
  inches — see `MATLAB_GOLF_MODEL_GUIDE.md` line 354) and are converted
  in `src/shared/python/motion_matching/loaders/excel.py`.
- **C3D** files supported by `load_club_target`. They use the same
  `ClubTarget` adapter; if A/T/F labels are absent, the UI still seeds
  the Impact event from the canonical target's impact index.
...
— tests covering RigidTransform algebra, shaft-snap solver, fallback
skeleton modelling, xlsx unit handling, phase windowing, session
round-trip, FK-derived fallback, and the `ClubTarget` adapter. UI smoke
tests skip cleanly when PyQt6 fails to import in the test environment.
...
- #4404 canonical ClubTarget target adapter

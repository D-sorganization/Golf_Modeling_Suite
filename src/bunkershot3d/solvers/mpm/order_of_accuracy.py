...
The mechanism that last paragraph names -- a particle-grid round trip
whose cost is paid per step, so that its total over a fixed window grows
as ``1/dt`` -- is the same one
:func:`~bunkershot3d.solvers.mpm.verification.cohesive_oscillation_residuals`
runs into from the other direction, with the plasticity removed entirely.
Neither is a defect in the other's case; they are two views of one

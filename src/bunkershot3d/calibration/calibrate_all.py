...
def calibrate_backend(backend: str, use_mock: bool = False) -> None:
    logger.info("Calibrating sand parameters", backend=backend)
    ...
    final_restitution = (
        aor_params["restitution_coefficient"] + shear_params["restitution_coefficient"]
    ) / 2.0
    ...
    logger.info("Saved calibrated parameters", path=str(config_path))

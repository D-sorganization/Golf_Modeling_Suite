...
def _sphere_sphere_distance(
    ...
    center_dist = math.hypot(*np.ravel(diff))
...
def _gjk_distance(
    ...
    if math.hypot(*np.ravel(direction)) < 1e-10:
        direction = np.array([1.0, 0.0, 0.0])
    else:
        direction = direction / math.hypot(*np.ravel(direction))
    ...
    for _ in range(max_iterations):
        ...
        if np.dot(support, direction) < 0 and len(simplex) == 0:
            ...
            dist = float(math.hypot(*np.ravel(diff)))
        ...
        if len(simplex) == 1:
            ...
            dist = float(math.hypot(*np.ravel(closest)))
            if dist < 1e-6:
    ...
    support_b = prim_b.compute_support(-direction)
    diff = support_b - support_a
    return float(math.hypot(*np.ravel(diff))), support_a, support_b

...
class Sphere(GeometricPrimitive):
    ...
    def contains_point(self, point: np.ndarray) -> bool:
        ...
        return math.hypot(*np.squeeze(point - self.center)) <= self.radius
    ...
    def compute_support(self, direction: np.ndarray) -> np.ndarray:
        ...
        norm = math.hypot(*np.squeeze(direction))
        if norm < 1e-10:
...
class Capsule(GeometricPrimitive):
    ...
    def length(self) -> float:
        ...
        return math.hypot(*np.squeeze(self.point_b - self.point_a))

    @property
    def axis(self) -> np.ndarray:
        """Get capsule axis direction (normalized)."""
        diff = self.point_b - self.point_a
        length = math.hypot(*np.squeeze(diff))
        if length < 1e-10:
    ...
    def contains_point(self, point: np.ndarray) -> bool:
        ...
        return math.hypot(*np.squeeze(point - closest)) <= self.radius
...
class Cylinder(GeometricPrimitive):
    ...
    def __post_init__(self) -> None:
        ...
        # Normalize axis
        norm = math.hypot(*np.squeeze(self.axis))
        if norm < 1e-10:
    ...
    def contains_point(self, point: np.ndarray) -> bool:
        ...
        return float(math.hypot(*np.squeeze(perp))) <= self.radius
    ...
    def compute_support(self, direction: np.ndarray) -> np.ndarray:
        ...
        perp_norm = math.hypot(*np.squeeze(d_perp))
        if perp_norm > 1e-10:
...
class ConvexHull(GeometricPrimitive):
    ...
    def contains_point(self, point: np.ndarray) -> bool:
        ...
        to_point = point - self.center
        norm = math.hypot(*np.squeeze(to_point))
        if norm < 1e-10:

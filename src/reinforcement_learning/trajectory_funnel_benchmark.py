...
class TrajectoryFunnelBenchmark:
    ...
    def trajectory_funnel_reward(
        ...
        assert current_state is not None, "current_state must be provided"
        # ⚡ Bolt: Avoiding np.linalg.norm(..., axis=1) overhead + skipping unnecessary sqrt
        sq_distances = np.sum((reference_trajectory - current_state) ** 2, axis=1)
        projected_phase_idx = int(np.argmin(sq_distances))

        # Penalize only the orthogonal deviation from the tube
        transverse_cost = -10.0 * float(sq_distances[projected_phase_idx])

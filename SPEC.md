...
| 2026-06-01 | 1.0.237 | Physics hot-loop magnitude optimization with integer-overflow guard (#7021, fixes #7022). Replaced `np.linalg.norm` with `math.sqrt(np.dot(x, x))` in the friction-law (`_friction_laws.py`), terrain-contact (`_terrain_physics.py`), and aerodynamics-fallback (`_rust_facade.py`) hot loops. Each module now routes through a shared `_magnitude(vec)` helper that casts the input to `float` before the dot product, because raw `np.dot` on an integer-dtype array can overflow before the square root (the dropped `np.linalg.norm` promoted to float implicitly). Added regression tests covering int-overflow, float equivalence with `np.linalg.norm`, and the zero vector. |

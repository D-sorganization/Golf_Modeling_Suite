...
- **2026-06-10** - Hardened model-library source URL validation for #7297.
  `src/shared/python/model_generation/library/_model_loader.py` now rejects
  non-HTTPS `ModelEntry.source_url` values before cache creation or network
  access, delegates accepted downloads to the shared bounded downloader in
  `src/shared/python/security/security_utils.py`, and treats URL contract
  violations as load failures. Focused model-loader tests cover forced-download
  rejection for `file:`, `http:`, and `ftp:` sources before `urlopen`.
...
- Model loader and parameter editor; remote model-library downloads only accept
  HTTPS `source_url` values and use the shared bounded downloader before
  writing cached model files.
...
| ---------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-06-10 | 1.0.304 | Model-library source URL validation for #7297. `model_generation.library._model_loader.download_model` now validates remote `source_url` schemes before creating cache entries or opening the network, accepts only HTTPS, delegates accepted transfers to the shared bounded `download_to_file` helper, and reports `ValueError` URL-contract failures as failed loads. Focused tests cover forced-download rejection for `file:`, `http:`, and `ftp:` sources before `urllib.request.urlopen` plus the updated secure-download path. |

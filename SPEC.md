...
- Performance improvements implemented in UI `data_processor_widget.py` by converting slow `.iterrows()` calls to faster `.itertuples(index=False)` iterations. (spec-exempt: micro-optimization)

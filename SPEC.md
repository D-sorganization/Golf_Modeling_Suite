...
| 2026-04-22 | 1.0.163 | Issue #3010: engine probe/load route decorators are now prefix-relative (`/engines/...`) so route registration under `/api` and `/api/v1` no longer produces doubled `/api/api/...` paths; API and integration endpoint tests were updated accordingly. |

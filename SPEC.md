...
| Date       | PR         | Changes    |
| ---------- | ---------- | ---------- |
| 2026-09-03 | #9487 | Wired the Sidekick analytics tool the assistant was already being told it had. `summarize_simulation_run` and `register_sidekick_analytics_tools()` shipped with #5464, but nothing under `src/` ever called the registration entry point -- `git grep` matched only its own definition -- so the tool was dead code the chat assistant could not invoke, and `build_system_prompt()` never mentioned it. `register_golf_suite_tools()` now calls `_register_sidekick_analytics()`, which deliberately lets ImportError propagate rather than logging a warning like the neighbouring optional codemap proxy: the system prompt advertises the tool, so a silent registration failure would leave the assistant offering a capability it cannot call. The `upstream_drift` prompt context gains one capability line naming the tool, and the registry name becomes the single public constant `SIDEKICK_ANALYTICS_TOOL_NAME`, from which both `sample_tools` and `system_prompts` derive rather than repeating the literal, so prompt and registry cannot drift apart. Three tests shipped with #5464 have been red since; they were invisible because the tests lane was killed by a 35-minute timeout before finishing (#9473). Verified: the six previously failing tests in `tests/unit/ai/test_sidekick_analytics.py` and `tests/smoke/test_e2e_tool_registry_and_chat.py` now pass, with no new failure across `tests/unit/ai tests/ai tests/smoke`. Part of #9474 (main-green Phase 0). |

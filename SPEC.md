...
| 2026-09-03 | #9442 | Made UpstreamDrift consume Tools as an installable dependency rather than a source tree: `requirements-tools.txt` plus a `pyproject.toml` pin, `scripts/shared_tools/check_tools_pins.py` to keep the Cargo/pyproject pins consistent with the `vendor/ud-tools` submodule revision, and a `launch_upstream_drift.py` bootstrap that resolves the canonical packages through the installed distribution. `src/shared/python/_seam_redirect.py` gains the matching resolution path and `docs/shared_tools/seam_rulings.v1.json` records the affected clusters. Stacked on #9440. (UD #9406, program #1505) |

...
| Date       | PR         | Changes    |
| ---------- | ---------- | ---------- |
| 2026-09-03 | #9489 | Restored the honest chat-tool placeholder contract that PR #8322 reverted out of PR #7391 while keeping #7391's tests, and corrected 22 `ToolResult` assertions that had never executed. `_not_implemented_tool_result()` returns to `src/shared/python/ai/sample_tools.py`; `run_inverse_dynamics`, `validate_cross_engine` and `check_energy_conservation` route through it again, so none of them claims a job was queued (`simulation_pending` / `validation_pending` / `check_pending` are gone, and no consumer of those strings exists under `src/` or `tests/`). The helper's docstring now states its postcondition and names `test_registered_chat_placeholders_do_not_claim_success_or_pending`, the contract test that pins the invariant for every placeholder at once, so a new placeholder cannot hand-roll a dishonest dict -- the route by which the regression re-entered. Separately, 22 assertions across six AI test files read `result.solver_status == "success"`; `ToolResult` has never carried a `solver_status` attribute (it has `success: bool`), so each raised AttributeError before reaching its subject. `solver_status` is a real field on the motion-matching `FitResult` and a rename during #5845 landed it on unrelated tests. They now read `result.success is True` / `is False`, with every surrounding payload assertion untouched so transport success and payload success stay distinct. No test skipped, xfailed, weakened or deleted. Verified: 98 passed across the six files, previously 25 failing. Part of #9474 (main-green Phase 0). |

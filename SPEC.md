...
| **Last Spec Update**    | 2026-05-08 (fix(#4419): MuJoCo starting-pose skeleton extraction refreshes forward kinematics before default reads) |
...
| 2026-05-08 | 1.0.144 | Fixed the MuJoCo starting-pose provider so `get_skeleton(qpos=None)` refreshes MuJoCo forward kinematics before reading body `xipos`, preventing first/default calls from returning zero, uninitialized, or stale skeleton positions. |
````

...
- REST routes are registered at root, legacy `/api/...`, and versioned `/api/v1/...` prefixes; simulation and video quota dependencies remain mounted in all modes and evaluate `GOLF_SUITE_MODE` / `GOLF_AUTH_DISABLED` at request time so local calls preserve validation semantics while cloud mode rejects unauthenticated expensive requests.

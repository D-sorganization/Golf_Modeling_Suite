...
| 2026-09-06 | #9387 | Made the `tests/helpers/engine_src_pivot.py` `src` pivot atomic so a failed `enter()` can no longer shadow the repo's `src` package for the rest of an xdist worker (#9387). Every call site is `pivot.enter()` then `try: ... finally: pivot.exit()`, so the undo is only armed once `enter()` has returned -- but `_install` evicts the whole `src` namespace and binds `sys.modules["src"]` to the engine package *before* executing the engine's `__init__`, so any engine-side import error left the engine `src` permanently installed in that long-lived worker. That is the recurrence of #9099's bug-#2 class reported against #9384 and #9374: `module 'src' has no attribute '_install_parent_shared_aliases'` in `tests/scripts/test_validate_suite.py`, and `mock.patch` binding to duplicate `src.launchers.*` modules in `tests/launchers/test_shot_tracer_import_gui.py`, with the victim rotating by xdist scheduling. `enter()` now rolls the snapshot back and re-raises; `exit()` is a no-op when no pivot is active, since its empty-snapshot restore path would otherwise delete the entire `src` namespace; and the unreachable `spec is None` branch raises instead of returning with the namespace already evicted. Red-then-green regression coverage in `tests/unit/repo_hygiene/test_no_permanent_src_module_shadow.py`. |

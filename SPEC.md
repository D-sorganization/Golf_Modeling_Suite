...
| ---------- | ------- | ---------------------------------------------------------------------------------- | --------- |
| 2026-09-02 | 1.0.693 | Replaced `((arr - mean) ** 2).sum()` with `np.vdot(diff, diff)` in multiple places (spec-exempt: micro-optimization) |
| 2026-08-30 | 1.0.678 | Completed the #9153 native CRBA evidence replay and strengthened its downstream semantic closure. Two clean single-worker Linux executions with MuJoCo 3.8.0 and robotics Pinocchio 3.8.0 reproduce all six JSON records, every typed NPZ member, all eleven deterministic vector figures, and the claim registry exactly. A focused qualification test then exposed one stale candidate-to-claim reciprocal link after dependency-ordered registrars; canonical reconciliation changed two review links while preserving all 328 claim statements, classifications, statuses, and outcomes. The replay report now rejects byte-identical registries unless every claim/review link is unique and reciprocal, records native distribution hashes and thread limits, and retains the synthetic/no-human/no-coaching boundary. All affected gates, 1,181 candidate reviews, 498 numeric literals, 707 release artifacts, and every one of 253 PDF pages requalify under the computational profile; archival tagging and font-resource gates remain explicitly open. |
| 2026-08-30 | 1.0.677 | Closed the first #9153 requalification replay's vector-figure nondeterminism before evidence promotion. Two independent native executions reproduced all six JSON records, every NPZ member, and the claim registry exactly, but all eleven PDF/SVG bytes differed because Matplotlib emitted clock metadata and randomized SVG object IDs. A shared vector writer now removes PDF creation/modification times and SVG dates, applies an explicit per-figure SVG hash salt, normalizes SVG text, and preserves the native-discrepancy figure's atomic PDF replacement. A TDD fixture proves two independent PDF/SVG writes are byte-identical. Requalification schema v2 hash-binds the helper and all six figure generators plus the exact cmeel binary-compatibility pins discovered by the environment probe; no numerical result or tolerance was altered or promoted. |
| 2026-08-30 | 1.0.676 | Preregistered the outcome-blind native requalification required by the #9153 Pinocchio CRBA correction. The registration hash-binds all four corrected sources, enumerates the six directly affected JSON/NPZ evidence families and their dependent figures/claims, requires Linux x86-64 with robotics `pin==3.8.0` and `mujoco==3.8.0`, limits execution to one worker, and requires two clean replays before outcome inspection. Canonical JSON, memberwise NPZ, case order, typed failures, figures, claim evidence, paper, checksums, release authority, and protected CI all fail closed; the registration itself has no promotion authority and does not change any scientific result, tolerance, or human/coaching inference. |
| 2026-08-30 | 1.0.673 | Completed robotics Pinocchio CRBA mass matrices before proximal--distal research solves (#9153). Pinocchio guarantees the CRBA upper triangle; the reusable lower triangle can retain stale native-data values. One shared finite/square adapter now copies the upper triangle into an independent matrix and mirrors it explicitly, and all four articulated research consumers use that adapter. A poisoned-lower-triangle regression proves unspecified entries cannot affect the solver. Existing source-bound scientific artifacts remain intentionally stale and non-authoritative until a separately preregistered native requalification regenerates them; no tolerance, result, or human/coaching claim changes in this correction. |
...
| 2026-08-29 | 1.0.669 | Un-mirrored BunkerShot3D's delivery frame against the head frame, which had inverted the tool's primary design variable (issue #9247). F0 answered marketed bounce with the wrong sign: at a 24 mm sole, -14 deg of attack and firm sand, 8 -> 26 deg of marketed bounce moved maximum sole depth **19.69 -> 24.57 mm** and displaced mass **109.3 -> 149.2 g**, monotone -- more bounce digging deeper, which is the opposite of what bounce is for. The reported peak force *fell* over the same span (1188.0 -> 1135.1 N), which is what said the defect was not in the wrench. Diagnosed by measurement, and the two cheap hypotheses were excluded rather than argued: the sole reference point does **not** move with bounce (its body-frame station is 25.803 mm at every bounce, to the micrometre), and the address-versus-delivered reference mismatch is a pre-burial of **0.034-0.042 mm that shrinks as bounce rises** -- two orders below the 4.9 mm swing and the wrong way round. `engaged_depths_m` tracked `sole_depths_m` to 0.1 mm throughout, so both depth measures agreed and the head genuinely went deeper. The cause was a frame conflict: `geometry/profile.py` and `geometry/lofting.py` build the mesh with the leading edge at the origin and `+x` **rearward**, while `geometry/delivery.py` was written with `+x` toward the target. Its scalars were right and its vectors were backwards, so the workbench -- which rebuilt the delivery composition from that module's docstring and took `+x` for the travel direction -- drove the head through the sand **trailing edge first**, down a sole descending at 19.97 deg into the bed, on which more bounce is a steeper ramp. Measured on the lofted mesh rather than argued: the face normal at address is `(-0.516, 0, 0.856)`, and delivered loft read off the mesh now tracks `effective_loft_deg` to a **constant 0.920 deg** discretisation offset at every open/lean combination, against errors of **-7.81 to +28.92 deg** before -- at the shipped 6 deg of forward lean the sand saw a 64.92 deg face while the report said 52.00 deg, because the mirrored pitch *added* loft. Every published angle is unchanged, verified bit-identical over 576 loft/bounce/lie/open/lean combinations, because the correction is a mirror in `x` and no scalar depended on the sign of `x`. After the fix the ordering is monotone the right way in **49 of 49** cells of a bounce x attack sweep (8-26 deg by -2 to -14 deg), where before it was monotone *inverted* in all 49. The composition and travel axis are now stated once in the library as `delivered_rotation`, `entry_velocity_m_s` and `TRAVEL_AXIS_BODY`, and `loft_wedge` refuses a mesh whose sole does not descend rearward from its leading edge (`require_travel_frame`, raised not asserted). **Two findings a reader should not miss.** First, the burying-regime "reversal" prior work recorded is not real: after the fix more bounce is shallower in *both* regimes, and the apparent flip was the shallow regime being inverted, not the deep one. The boundary sits near zero presentation bounce (delivered bounce plus attack angle). Second, the corrected model now **buries** three of six shipped presets at the workbench's default delivery (-8 deg attack, 10 deg open, 6 deg forward lean), because their presentation bounce is negative there; that is the model's honest output, not a new defect, and it is bounded by the standing flat-half-space caveat (no crater, no divot memory), but it makes the default delivery worth revisiting. `DIG_SKID_BOUNCE_ORDERING_REASON`, the stopgap caveat that rode on every dig/skid verdict to warn readers off the bounce ordering, is removed with its cause. 19 regression tests assert the ordering in both regimes and bracket the boundary; each half of the mirror is caught independently (rotation alone fails 5, travel alone fails 9, both fail all 17 at the new lofting precondition). |
| 2026-08-29 | 1.0.670 | Reconciled the workbench tests against the un-mirrored delivery frame (#9247, PR #9254) and corrected the shipped default delivery the mirror had been hiding. **The invariant that guards the fix was not running.** #9237 made `divot_metrics`' `friction_angle_deg` a required keyword argument while this branch was open, so all 10 bounce-ordering tests that march a shot died with a `TypeError` before reaching an assertion; the bed's own friction angle is now threaded through, because the divot's wall angle is the bed's and every design in a sweep must be measured against the same one for the ordering between them to mean anything. **The workbench default was a full-shot delivery, not a splash shot.** `attack_angle_deg=-8` with `shaft_lean_deg=+6` survived only because the mirrored frame was silently *adding* the bounce that forward lean removes; measured, the old default is exactly `face_open=-10, shaft_lean=-6` in the corrected frame -- a **closed** face, never the delivery it claimed to be. Corrected it delivers negative presentation bounce on three of six shipped presets, which plough rather than skid: 57-77 mm deep, 100-148 ms in the sand, leaving at 0.7-3.1 m/s from a 25 m/s delivery, with no divot because the trace reverses inside its own crater (258 backward steps over a 78 mm excursion). The default is now 25 m/s, **-6 deg of attack, 10 deg open, no lean**; all six presets skid there over presentation bounces of 3.4-12.8 deg, 8.5-10.7 mm deep and 12-18 ms in the sand, ordered the right way round with the lowest presentation bounce cutting deepest. The attack angle moved as well as the lean because lean alone leaves the default on a cliff (12.5 mm to 43 mm between 3 deg of lean and none) and because F0's flat half-space over-predicts deep digs, so a default in the burying regime is a default where the model is least trustworthy. **No numerical bound was widened**: the `< 0.060 m` depth bound encodes the published divot-depth domain (Wivou 2016 measured 0.025-0.052 m), so the 67 mm the old default produced is the model rejecting the delivery, not the bound. Two further defects the fix made visible are repaired rather than accommodated. `PlayabilityWindow.refused_fraction` conflated **ADR-0032 refusal** with **a head that buried**: nothing distinguished them while every delivery planed under the mirror, and the corrected model made the single number report that the solver had refused half a domain it had refused nothing in. Refusal and burial are now separate, disjoint numbers (`unmeasured_fraction`), threaded from the model and both reported. And **carry is not monotone in attack angle**: it climbs 0.117 -> 0.696 m from -2 to -6 deg, turns over to 0.624 m by -7 deg, and past -8 deg the head buries and there is no carry at all -- so the test asserting a steeper blow always carries further was asserting an ordering that held only because every delivery planed under the mirror. Depth *is* monotone in attack and is still pinned as such. `test_the_head_travels_toward_the_target` asserted `path[-1, 0] > path[0, 0]`, taking `+x` for the direction of travel, and now projects onto `TRAVEL_AXIS_BODY` so a re-mirror moves the axis and the assertion together. Verified over a 7 x 7 bounce x attack sweep at 3 deg bounce spacing: depth ordering correct in 40 of 42 adjacent pairs and mass in 17 of 18, the three exceptions being 0.01 mm and 0.41 g -- below the ~0.65 mm depth sampling resolution, so the shipped guard's 1e-6 m tolerance is tighter than the model's own discretisation at that spacing. At the new default delivery the ordering is clean: depth 9.64 -> 8.89 mm and mass 77.1 -> 59.6 g, both strictly decreasing over 8-26 deg of marketed bounce. |
| 2026-08-29 | 1.0.672 | Replaced the bounce-ordering guard's hard-coded comparison tolerances with a resolution floor computed per march (#9247, PR #9254). The guard compared maximum sole depths to within `1e-6` m (0.001 mm) and displaced masses to within `1e-9` kg (1 microgram), while `divot_metrics` states depth is "resolved to the sample spacing; no sub-sample peak fit is applied". It passed only because `BOUNCES_DEG` steps 6 degrees at a time, where designs differ by far more than the model can resolve; a 7 x 7 sweep at **3-degree** spacing (8-26 deg against -2 to -14 deg of attack) turned up **2 of 42 depth pairs and 1 of 33 mass pairs "inverted"**, all of them below the discretisation floor. The guard was therefore latently flaky in the worst way: anyone testing finer bounce spacing would have got a failure that reads as a physics regression and is quantisation. **The naive floor is wrong by ~65x.** Vertical speed times the step gives 0.65 mm at -6 degrees, but near a smooth peak the derivative vanishes, so the reading error is second order in the step rather than first. Two quantities bound it and which dominates depends on the peak's shape, so `_peak_resolution_m` takes the **larger**: the gap to the neighbouring sample, which bounds a *sharp* peak (0.0002-0.0098 mm across the sweep), and the sub-sample parabolic deficit -- the height a parabola through the peak and its two neighbours reaches above the sampled max -- which bounds a *flat* peak, exactly where the gap collapses toward zero and understates the uncertainty. At -6 deg between 20 and 23 deg of bounce the gaps are 0.0002 / 0.0005 mm while the parabolic deficits are 0.0054 / 0.0050 mm, against an apparent inversion of 0.0100 mm: neither measure alone covers it. Comparing two designs **sums** their floors rather than taking the larger, because either reported maximum may sit below its own true peak, and 0.0054 + 0.0050 = 0.0104 mm is what closes that pair. Mass has its own quantum, one submerged sample's worth of the prism (`mass / count(sole_depths_m > 0)`, about 0.66 g here), against an observed 0.41 g inversion. Both floors are returned by `_shoot` per shot rather than fixed as constants -- a wider constant would be a bound chosen to fit the results, which is the move this PR exists to prevent. The endpoint assertion is **strengthened** rather than relaxed: the sweep's span must now exceed the resolution floor, so the guard cannot be satisfied by noise and keeps its ability to catch a genuinely inverted lever. Verified over the same 7 x 7 at 3-degree spacing: depth inversions **2 -> 0 of 42**, mass **1 -> 0 of 33**, with the shipped 6-degree sweep's 19 tests still passing. |

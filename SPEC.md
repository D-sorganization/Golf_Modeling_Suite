...
- **2026-05-31** - Added the first JaxSim backend adapter (#6653): `src/engines/loaders.py`, engine probes, availability checks, and registry metadata now expose a JaxSim engine path with optional imports; the adapter maps JaxSim model/data APIs into the engine-core kinematics and dynamics-term surface (`M`, `h`, `g`, Coriolis, Jacobians, drift/ZTCF/ZVCF) with mocked unit coverage and clean skips when the optional JaxSim/JAX stack is absent.

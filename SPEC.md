...
| ---------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-04-17 | 1.0.128 | Bug fixes: corrected inverted guard condition in `src/api/aip/dispatcher.py` `make_response()` (was raising on valid result, now raises only when both result and error are None); filtered `spatial_jacobian_order` from capability-level iteration in `src/api/routes/engines.py` so the `GET /engines/{id}/capabilities` response only contains valid `full`/`partial`/`none` level entries.                                                                                                                                                                                                                                       |
| 2026-04-17 | 1.0.127 | Security assessment hardening: `assess_F()` in `scripts/assess_repository.py` now uses a targeted regex matching actual string literal assignments (8+ char values) scoped to `src/**/*.py` only, eliminating false-positive findings from test files with intentional placeholder credentials. Added `# nosec B105` annotations to 16 test fixture lines across 7 test files to document that values like `gms_testkey_*`, `password123`, and `sk-gemini` are intentional test placeholders. Closes #2740.                                                                                                                           |
...
| 2026-04-17 | 1.0.126 | Impact model correctness: rigid-body friction spin now follows the contact-torque sign convention `tau = r x F` with `r = -R*n`, and focused regressions plus a physics derivation document pin the sign convention for future gear-effect work.                                                                                                                                                                                                                                                                                                                                                                                      |

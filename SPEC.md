...
- **2026-05-31** - Added metadata-driven helpful-field and provenance-value wrappers for the Idiot-Proof UX epic (#5968): PyQt6 and React controls now consume the shared field metadata/provenance contracts, UI field metadata is generated from `configs/ux/field_metadata.yaml`, and parity tests keep the TypeScript registry synchronized with the YAML source of truth.
...
| ---------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-06-01 | 1.0.237 | Review-feedback parity fixes (#7015/#7017, #6954). Ported the #6982/#6984/#6986 impact fixes from the private `_impact_physics.py` copy into the documented public package `src.shared.python.physics.impact_model` so callers using the public import path receive them: `impact_model/utils.py::validate_energy_balance` now returns `expected_loss_j = ½·μ·v_rel_n²·(1−e²)` (reduced-mass closed form); `impact_model/models.py` spring-damper solver now seeds a tiny contact-onset clearance (`R·1e-4`) to remove dt-dependent overshoot; and `_compute_friction_spin` now applies the rolling cap relative to contact-point speed (`v_t_eff = max(0, tangent_mag − ω_contact·R)`) so pre-existing spin reduces the friction impulse. Added `tests/unit/impact_model/test_public_api_parity.py` locking the public-package behavior. #6954: `config/provider_catalog.py` no longer emits a duplicate canonical `movement_optimizer` provider_id — the hyphenated `Movement-Optimizer` checkout name is now modeled as a `repo_name_aliases` entry on the single `Movement_Optimizer` definition, with `iter_repo_names()` feeding sibling-root and manifest discovery; added uniqueness + alias-discovery regression tests. |
...
| 2026-05-31 | 1.0.219 | Added metadata-driven helpful-field and provenance-value wrappers for the Idiot-Proof UX epic (#5968): `HelpfulField` and `ProvenanceValue` PyQt6/React controls consume the existing metadata/provenance contracts, `scripts/ux/generate_field_metadata_ts.py` generates the TypeScript registry from `configs/ux/field_metadata.yaml`, and focused PyQt/Vitest tests cover contract validation, ARIA help text, provenance display, and YAML-to-TypeScript parity. |

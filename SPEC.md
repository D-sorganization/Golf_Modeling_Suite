...
| ---------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-27 | 1.0.618 | Fixed #9142: `claim_adjudication_summary.py`'s generated reviewer chapter linked its JSON/CSV artifacts with relative `data/...` paths, which resolve inside UD's own docs tree but dangle when the chapter is republished verbatim into AffineDrift (no sibling `data/` directory exists there), failing AffineDrift's pre-build link check for the second time (first was `_ch06c`, fixed in AffineDrift#3983). The generator now emits `https://github.com/D-sorganization/UpstreamDrift/blob/main/docs/research/proximal_distal_energy_transfer/data/<file>` permalinks, matching the convention already used by every other hand-authored chapter's own `data/...` references (e.g. `_ch09_conclusions.qmd`); AffineDrift's publish step remains responsible for pinning `blob/main` to a specific commit SHA at publication time, as it already does for those siblings. Regenerated the committed chapter, JSON, and CSV; only the two link URLs changed, no scientific content. A new regression test asserts the committed chapter contains no `](data/` relative links and both permalinks in the expected form. |

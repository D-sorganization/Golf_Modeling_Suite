...
| 2026-05-02 | 1.0.87  | Bolt: Optimized `np.max(np.linalg.norm(..., axis=1))` to `np.sqrt(np.max(np.einsum(...)))` in `_cg_primitive_fitting.py` to avoid intermediate arrays and reduce square roots |

...
* **Tree Indexing:** Replaced `np.sum((a - b) ** 2)` with `np.vdot(diff, diff)` in `_tree_index.py` KD-tree lookups for a ~2.5x speedup by avoiding intermediate array allocations.

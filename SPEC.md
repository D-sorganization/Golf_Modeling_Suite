...
| Shared Utilities               | `src/shared/`                                              | Cross-engine validators, helpers, and exception definitions; engine availability probing treats mocked optional backends as unavailable so CI does not over-report support |
...
| 2026-04-20 | 1.0.135 | Issue #2791 — optimized `SupportPolygon.distance_to_edge` with scalar math for single-point edge-distance checks, avoiding unnecessary vectorized reductions in the humanoid character builder hot path while preserving existing geometry behavior. Added focused support-polygon regression coverage.                                                                                                                                                                                                                                                                                                                                           |
...
| 2026-04-17 | 1.0.130 | Pendulum integration correctness: resetting the golf-swing pendulum engine now clears any sampled torque profile so subsequent constant-torque simulations cannot reuse stale stage torques from a prior driven run. Regression coverage verifies reset isolation for the torque-profile path.                                                                                                                                                                                                                                                                                                                                                    |
| 2026-04-17 | 1.0.129 | Performance optimization: Replaced `np.sum((positions - center)**2, axis=1)` with `np.einsum("ij,ij->i", diff, diff)` in `integrated_golf_gui_r0/golf_visualizer_widget.py` to avoid intermediate array allocations and improve visualizer rendering performance.                                                                                                                                                                                                                                                                                                                                                                                 |
| 2026-04-17 | 1.0.128 | Refactor(UIBuildHook): extracted `_ui_dir` and `_dist_dir` properties, `_force_ui_build()` helper, and `_run_npm_build()` method from `UIBuildHook.initialize()` in `build_hooks.py` to eliminate multi-hop member chains (Law of Demeter). Added regression coverage for all new boundary methods in `tests/unit/test_build_hooks.py`. Closes #2741.                                                                                                                                                                                                                                                                                             |
...
| 2026-04-17 | 1.0.126 | Physics integration hardening: Pinocchio engine stepping now defaults to an RK4 tangent-state integrator while preserving an explicit semi-implicit option, validates positive time steps, and has regression coverage for RK4 stage state ordering and long-run free-fall energy conservation. The golf-swing pendulum wrapper now accepts a time-varying torque profile sampled at RK4 stage times so driven swing integration no longer reuses stale torques at `t + dt`. Refs #2699.                                                                                                                                                          |
| 2026-04-15 | 1.0.118 | CI stabilization: core and shared-contract dependency installs now use per-job virtual environments, pytest-qt is pinned to the PyQt6 API in GUI-capable lanes, and benchmark tests are excluded from default core CI so self-hosted runner state cannot leak optional stacks into required tests.                                                                                                                                                                                                                                                                                                                                                |
...
| 2026-04-15 | 1.0.121 | CI stabilization: Jules Auto-Repair now falls back to the workflow `github.token` when `RUNNER_CHECK_TOKEN` is unavailable, preventing workflow-run repair jobs from failing before checkout in repositories or events where the optional runner token is not exposed.                                                                                                                                                                                                                                                                                                                                                                            |
| 2026-04-15 | 1.0.122 | CI stabilization: `shared.python` now lazily exposes subpackages used by patch-based GUI and plotting tests, and MuJoCo loader coverage patches import modules directly so Python 3.10 CI is not sensitive to earlier import-cache state.                                                                                                                                                                                                                                                                                                                                                                                                         |
| 2026-04-15 | 1.0.124 | CI stabilization: trajectory analysis performance coverage now keeps a CI-safe wall-clock ceiling and a per-step scaling check instead of depending on a brittle sub-half-second absolute threshold on instrumented self-hosted Python 3.12 runners.                                                                                                                                                                                                                                                                                                                                                                                              |
...
- 2026-04-22: Added adversarial review assessment tracking and docs-governance coverage for `docs/assessments/adversarial_review_2026-04-22.md`, and refreshed import grouping in perturbation/export/launcher modules so CI quality gates match the assessment PR.
...
- Skipped production quota route dependencies when local authentication is disabled so local/test API requests preserve endpoint validation semantics.

...
| 2026-05-02 | 1.0.87  | Bolt: Optimized bounding sphere radius calculation by explicitly computing maximum squared distance via `np.einsum` and applying `np.sqrt` once to avoid `np.linalg.norm(..., axis=1)` overhead. |

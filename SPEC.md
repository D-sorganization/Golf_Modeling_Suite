...
- Updated norm calculations in `src/bunkershot3d/metrics/trace.py` to use `math.sqrt(np.vdot)` and `np.sqrt(np.einsum)` for improved performance. (spec-exempt: micro-optimization)

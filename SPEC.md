...
| 2026-04-29 | 1.0.81  | fix: Restore missing jobs in `Code-Metrics.yml` and `release.yml`; correct non-UTF-8 characters in 55 workflows causing 0s CI failures. |
| 2026-04-29 | 1.0.80  | fix: Harden `pick-runner` logic across all workflows to handle `gh api` JSON errors; implement tool invocation loop for AI chat service (fixes #3162); resolve massive conflict-marker corruption in `src` and `tests` by restoring from `origin/main`. |
| 2026-04-29 | 1.0.80  | Bolt: Optimize Mean Squared Error calculations in system_identification.py |
| 2026-04-29 | 1.0.80  | Bolt: Replaced `np.linalg.norm` with `np.sqrt(np.vdot)` in `src/robotics/planning/collision/_distance_queries.py` and `src/robotics/planning/collision/_primitive_shapes.py` to avoid NumPy reduction overhead for small 3D geometric vectors. |
| 2026-04-29 | 1.0.80  | Bolt: Optimized `np.sum(error**2)` to `np.vdot(error, error)` in `trajectory_funnel_benchmark.py` to avoid temporary array allocation and speed up calculation. |
| 2026-04-29 | 1.0.79  | Generate updated assessment reports (A-O and Comprehensive) and auto-fix formatting issue in Motion Capture Plotter. |
...
### Performance Note (2026-04-29)
In `src/robotics/planning/collision/_primitive_shapes.py`, `math.hypot(*diff)` is utilized over `np.linalg.norm(diff)` for 3D vector length computations, as it bypasses NumPy array allocation and dispatch overhead resulting in a measured speedup of ~40% for small fixed vectors in high-frequency collision paths.

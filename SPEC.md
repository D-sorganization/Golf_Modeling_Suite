...
* (spec-exempt: micro-optimization) Replaced `np.sum(array)` with `array.sum()` across `src/shared/python/analysis/nonlinear_dynamics.py` for significant performance gains in recursive quantification algorithms.

...
| ---------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-29 | 1.0.648 | Cleared the two source-tree hygiene follow-ups from #9182 (issue #9220). **(a)** Deleted `src/shared/urdf/human_models/mujoco_humanoid/model.xml`. It was derived output -- a materialisation of the MJCF string embedded in `src/tools/model_explorer/model_library.py` -- committed via consolidated merge #6663 and kept alive only by the source-tree rewrite that 1.0.647 removed. A repository-wide search (the literal path, the `mujoco_humanoid` directory, `model.xml`, `standard_models.yaml`, the bundled-asset registry, `pyproject.toml` packaging/force-include, installer include lists, and the test suite) found no reader; the only `mujoco_humanoid` model path short-circuits to the embedded string before any directory lookup. The committed copy had also silently drifted from its generator (10,572 vs 11,183 bytes, render-side only), so it was a trap for anyone assuming it authoritative. `tests/unit/repo_hygiene/test_no_tracked_asset_rewrite.py` now pins that the loader never re-creates it. **(b)** Stopped test runs writing untracked `src/output/simulations/<engine>/simulation_*.json`. `resolve_base_path` in `src/shared/python/data_io/_path_utils.py` treated any ancestor containing a directory named `engines` as the project root, which matches `src/` (`src/engines` exists), so every default-constructed `OutputManager` -- including the one `SimulationService` builds for REST `/simulate` persistence -- rooted its output inside the source tree. Root detection now uses the checkout's `.git` entry, falling back to the outermost `pyproject.toml` (nested packages under `src/` ship their own), and honours a new `UPSTREAM_DRIFT_OUTPUT_DIR` override. The documented interactive/CLI default stays `<repo>/output`; `tests/conftest.py` sets the override to a temp directory at import time so the *tests* move, not the default. New `tests/unit/repo_hygiene/test_no_output_in_source_tree.py` asserts a representative simulation save writes nothing under `src/`. Both matter beyond tidiness: several agent workflows use `git status --short` to decide whether work is in progress, and stray output makes that unreliable. |

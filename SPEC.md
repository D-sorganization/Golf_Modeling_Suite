...
- **2026-08-19** - Restored `repo-structure-gates` on `main` (issue #8771). The
  DRY duplication ratchet's baseline was rewritten wholesale by an unrelated
  bot PR (#8746, "Memoize recursive ModelTree node"), which dropped 1,627
  fingerprints and raised the ceiling on 22 more; 758 of the 759 fingerprints
  the gate then reported over baseline were entries that PR deleted. The
  baseline is restored to its pre-#8746 content and the single genuinely new
  duplicate — a repeated unit-direction batch normalisation in
  `src/robotics/planning/collision/_primitive_shapes.py` — is deduplicated
  into `_unit_direction_batch()` rather than quarantined. Four further gates
  behind it, unobservable while the job stopped at DRY, are also repaired:
  nine stray pytest/export artefacts committed at the repo root by #8322 are
  removed (and git-ignored), `AGENT_HANDOFF.md` is allowlisted as the
  deliberate root doc AGENTS.md links to, SPEC's Tools-repo path citations no
  longer read as repository paths, the one root-level test file moves into
  `tests/research/`, and 69 unmarked tests across 21 files receive the suite
  markers their directories already use. No threshold was raised and no
  quarantine entry was added.
...
| ---------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-08-19 | 1.0.544 | Restored `repo-structure-gates`: reverted the DRY duplication baseline that #8746 rewrote wholesale (1,627 fingerprints dropped, 22 ceilings raised; 758 of the 759 reported violations were entries that PR deleted), deduplicated the one genuinely new repeat into `_unit_direction_batch()` in `src/robotics/planning/collision/_primitive_shapes.py`, deleted nine stray pytest/export artefacts committed at the repo root by #8322 and git-ignored them, allowlisted `AGENT_HANDOFF.md`, de-pathed the Tools-repo citations in the 1.0.541 entry, moved the last root-level test file into `tests/research/`, and added `unit`/`scientific` suite markers to 69 tests across 21 files. No gate threshold was raised and no quarantine entry was added (#8771). |

...
- `grasp_analysis.py`: Replaced `np.linalg.norm(..., axis=1)` with `np.sqrt(np.einsum('ij,ij->i', ..., ...))` to bypass intermediate array allocations, yielding a ~1.24x speedup. (spec-exempt: micro-optimization)

...
- (spec-exempt: micro-optimization) Replaced `np.sum(weights)` with `weights.sum()` in `src/robotics/planning/collision/_convex_distance.py` to avoid array conversion checks and improve execution speed.

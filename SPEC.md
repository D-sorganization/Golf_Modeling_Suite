...
- Optimized math operations: replaced `math.sqrt(x**2 + y**2)` with `math.hypot(x, y)` in `flight_models.py` and `geometry.py` for ~2x speedup on vector magnitude calculations.

...
| Date       | PR         | Changes    |
| ---------- | ---------- | ---------- |
| 2026-09-03 | #9489 | Corrected 22 `ToolResult` assertions across six AI test files that had never executed. Each read `result.solver_status == "success"`, but `ToolResult` has never carried a `solver_status` attribute -- it has `success: bool`. `solver_status` is a real field on the motion-matching `FitResult`, and a rename during #5845 landed it on tests that had nothing to do with the solver, so every one raised AttributeError before reaching its subject and the assertions below it never ran either. They now read `result.success is True` / `is False`, with every surrounding payload assertion (`result.result["success"]`, error strings, engine names) untouched so transport success and payload success stay distinct claims. No test skipped, xfailed, weakened or deleted; quarantine ledger untouched. Verified: 93 passed across the six files against 21 previously failing. Five related failures are NOT fixed here: `run_inverse_dynamics`, `validate_cross_engine` and `check_energy_conservation` still claim a job was queued that never starts, because #8322 reverted the `src/` half of #7391 while keeping its tests -- and `src/shared/python/ai/sample_tools.py` is a Tools-owned child copy that `tests/unit/repo_hygiene/test_tools_child_copy_contract.py` forbids editing here, so that repair belongs in D-sorganization/Tools followed by a `vendor/ud-tools` pin bump. Part of #9474 (main-green Phase 0). |

...
| Date       | PR         | Changes    |
| ---------- | ---------- | ---------- |
| 2026-09-03 | #9485 | Moved the one Section 12 change-log row that was stranded outside the real table into it. The row for PR #9457 landed on `main` concurrently with the #9456 migration to pull-request-keyed rows (Repository_Management#1520), which moved the table into a `<!-- prettier-ignore-start -->` block underneath it; the orphaned row sat between the section prose and that block and rendered as a stray one-row table fragment. The row is reproduced verbatim at the top of the table, in date order among the other 2026-09-03 rows, and no other row is reworded or reordered. A scan for further dated, PR-keyed rows outside the ignore block found none. |

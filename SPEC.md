...
| ---------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-02 | 1.0.694 | Fixed the `tests (3.12)` hang on `main` (job 99142314887, part of #9204's `scientific`-tier pattern): `tests/research/test_subject_scaled_closed_contact.py::test_small_closed_contact_atlas_reports_every_feasibility_gate` ran 12 `solve_closed_contact_configuration` calls under the default `ClosedContactConfig`, which was already correctly bounded (`max_nfev=1000`) and already reported non-convergence as a structured field (`solver_converged`/`feasible`) rather than hanging -- so the fix is not the "add a bound / raise on non-convergence" pattern the evidence trace first suggested. Measurement showed the solves converge and are bounded, just needlessly slow: `ftol=xtol=gtol=1e-11` is a relative tolerance about 7 orders of magnitude tighter than the `closure_tolerance_m=5e-4` gate that actually determines feasibility, and hand-to-grip closure is already satisfied by iteration ~10-20 while SciPy's trust-region optimizer spends the remaining ~200-500 (of the 12 solves' 203-500 observed `nfev`, ~2-3 s each) chasing near-flat regularization (least-norm tie-breaking among the reduced-tree's redundant DOF) with no engineering-relevant improvement (`hand_to_grip_distance_m` moves from ~6.7e-8 m to ~6.6e-11 m, both far inside the gate). A finite-difference check of the analytic jacobian used in that solve (`_contact_kinematics` / `_point_jacobians`, the exact frames named in the CI stack trace) confirms it is correct (relative error ~2e-10), ruling out the jacobian-bug hypothesis. Rather than changing `ClosedContactConfig`'s published default -- which the committed evidence bundle (`docs/research/proximal_distal_energy_transfer/data/subject_scaled_closed_contact.{json,npz}`) and the two single-solve convergence tests intentionally keep at full precision, and which is hash-pinned by `source_sha256` across roughly a dozen downstream research modules -- this test (which only asserts array shapes/metadata, not convergence or feasibility outcomes) now passes an explicit `ClosedContactConfig(solver_tolerance=1e-6, maximum_function_evaluations=60)`. Measured on one machine: the file's slow test 22.84 s -> 6.99 s, whole-file run 30.66 s (red, unmodified) -> 15.23 s (green). `scripts/research/proximal_distal_energy/subject_scaled_closed_contact.py` and its committed evidence are byte-for-byte unchanged, so no digest resync was needed. `tests/research/test_subject_scaled_closed_contact.py::test_committed_closed_contact_evidence_and_figure_are_current` fails identically before and after this change (a pre-existing `spatial_full_body.py` source-hash drift unrelated to this fix, not touched here). #9204 catalogs several other `scientific`-marker tests in the same over-budget pattern (`test_shoulder_velocity_strategy_search.py`, `test_engine_tiers.py`, etc.); those are not addressed by this entry. |

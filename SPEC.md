...
| ---------- | ------- | ---------------------------------------------------------------------------------- | --------- |
| 2026-08-30 | 1.0.679 | Bolt: Optimized `np.max(np.linalg.norm(vertices, axis=1))` to `np.sqrt(np.max(np.einsum(...)))` in `collision_generator.py` to avoid temporary array allocation and speed up calculation. |
...
| 2026-08-30 | 1.0.676 | Preregistered the outcome-blind native requalification required by the #9153 Pinocchio CRBA correction. The registration hash-binds all four corrected sources, enumerates the six directly affected JSON/NPZ evidence families and their dependent figures/claims, requires Linux x86-64 with robotics `pin==3.8.0` and `mujoco==3.8.0`, limits execution to one worker, and requires two clean replays before outcome inspection. Canonical JSON, memberwise NPZ, case order, typed failures, figures, claim evidence, paper, checksums, release authority, and protected CI all fail closed; the registration itself has no promotion authority and does not change any scientific result, tolerance, or human/coaching claim changes in this correction. |

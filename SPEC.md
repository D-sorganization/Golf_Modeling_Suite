...
| 2026-09-04 | #9510 | Bounded the isolated-subprocess children so a hang fails one test instead of the whole session (RM #1507, program #1505, UD #9474). `tests (3.12)` on `main` reached 78% and then ended with no `short test summary info` and no failure list: `run_isolated_test` spawns a full pytest session as a child and passed no `timeout=`, so the parent blocked in `communicate()` until the suite-wide `--timeout=60` fired -- and pytest-timeout raises in whichever frame is running, ending the session rather than the test. One slow child therefore destroyed the verdict for everything after it. The child now carries an explicit 300s bound and `TimeoutExpired` becomes a normal `pytest.fail` with the partial child output; the two isolated tests carry `@pytest.mark.timeout(360)` so a legitimately slow Drake/Pinocchio import is not killed at 60s, and the child bound stays strictly below the test bound -- an ordering a test now pins rather than a comment. Two regression tests cover the path (one monkeypatches `subprocess.run` to raise `TimeoutExpired` and asserts a `Failed` carrying both captured streams; the other pins the ordering), marked `unit`/`headless_safe` because they spawn nothing, while the isolation tests stay unmarked so a multi-minute subprocess run is not pulled into the fast unit lane -- unmarked tests in the module drop 4 to 2. This fixes no underlying test failure; with #9490 it restores the lane's ability to report one, which #9474 needs before anything can be triaged. |

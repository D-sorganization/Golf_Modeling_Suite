...
- **2026-06-12** - Hardened copied Sidekick package tests so targeted
  validation no longer clears pytest's global module cache; scoped
  `sys.modules` patches and regression coverage now guard that import-state
  contract.
...
- **2026-06-12** - Kept package-local Sidekick tests out of CI Standard's
  ...
  Sidekick tests in default pytest testpaths for targeted package validation.
- **2026-06-12** - Kept the dependency-light core test lane out of the
  Simscape MATLAB bridge subtree during full-suite fallback, matching the
  existing PR change filter and avoiding collection-time conftest aliasing for
  optional MATLAB integration tests.
...
- **2026-06-12** - Consolidated the remaining UI dependency, motion-matching
  ...
  after submodule imports, the legacy Simscape golf GUI caches frame rows before
  field extraction, and the putting-green API uses fixed-arity `math.hypot` for
  known 2D direction vectors.
...
| ---------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-06-12 | 1.0.367 | Hardened copied Sidekick package tests so targeted validation no longer clears pytest's global module cache; scoped `sys.modules` patches and regression coverage now guard that import-state contract. |
...
| 2026-06-12 | 1.0.360 | Corrected the standalone Sidekick wheel packaging regression test to match Hatch's non-overlap contract: the wheel packages the `src` tree once and force-includes `src/shared/python/sidekick` to the top-level `sidekick/` archive path, preventing an overlapping package declaration from adding `sidekick/__init__.py` twice during `python -m build`. |
| 2026-06-12 | 1.0.359 | Tightened the Docker security scan gate to block fixable HIGH/CRITICAL container findings while keeping no-fix OS/library CVEs nonblocking: SARIF upload still records all HIGH/CRITICAL findings for Security-tab visibility, and the blocking Trivy table scan now uses `ignore-unfixed: true` so Debian `affected`/`fix_deferred`/`will_not_fix` advisories do not strand unrelated PRs. |
| 2026-06-12 | 1.0.358 | Restored the explicit Hatch wheel package entry for `src/shared/python/sidekick` so the standalone Sidekick console package is declared as a top-level wheel package while preserving the existing `force-include` mapping and docs/tests/vendor exclusions. |
| 2026-06-12 | 1.0.357 | Consolidated UI dependency refreshes, motion-matching package export compatibility, legacy Simscape golf data row caching, and putting-green 2D norm optimization into one replacement PR so CI validates the combined surface once instead of running overlapping optional-stack lanes. |

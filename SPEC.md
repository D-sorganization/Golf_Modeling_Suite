...
````
| 2026-06-01 | 1.0.224 | Bolt: Optimized `np.linalg.norm(v)` to `math.sqrt(np.dot(v, v))` in `src/shared/python/physics/_friction_laws.py` to avoid temporary array allocation and speed up physics calculation loops. |

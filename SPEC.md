...
| 2026-06-01 | 1.0.238 | Ball-flight model hot-loop magnitude optimization (#7034). Replaced `np.linalg.norm` with `math.hypot` for the 3-component relative-velocity speed and lift/Magnus cross-product magnitudes in `WaterlooPennerModel`, `MacDonaldHanzelyModel`, and `ConstantCoefficientModel` (`flight_models.py`). `math.hypot` is faster than `np.linalg.norm` on small fixed-length vectors and is overflow-safe (each component is coerced to a Python float, so no integer `np.dot` can overflow). Numerically equivalent to the prior norm; covered by the existing flight-model unit tests. |

...
| ---------- | ------- | ---------------------------------------------------------------------------------- | --------- |
| 2026-09-01 | 1.0.683 | Bumped `vendor/ud-tools` to Tools 5e0eaad (ADR-0047 H1, Tools#4888): the `swing_sim.ball_flight_trajectory/1` interchange record with its fail-closed reader and `from_samples` constructor. The bump ARMS the four cross-family sanity gates in `test_flight_trajectory_export.py` - previously skipping with a reason naming the pin, now 25/25 run and pass with no test edit, including the vendored-reader round trip, the cross-repo digest-algorithm agreement, and the same-order-carry gate (244.65 m from both families, ratio 1.0000). Launcher-manifest smoke 25 passed and the ADR-0046 G0 drift gates 28 passed on the new pin. Unblocks H2-H4 (viewer imports) and F4. |

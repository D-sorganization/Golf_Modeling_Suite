...
- **2026-05-30** - Completed the in-launcher training-controller dashboard tab GUI (#6012, stacked on Sub-PR A #6013). The PyQt6 surface in `src/tools/training_controller/` now drives the headless backend through `TrainingDashboardController`: a sortable, status-filterable job list; a two-tab job-detail panel (matplotlib **Live metrics** grouped by `MetricKind` via `summarize_by_kind` with a `RollingMean`-smoothed overlay for RL reward series, and a **Summary** tab with a `best_per_metric` "best so far" card plus a clickable output-dir artifact link); an action toolbar (Submit / Cancel / Pause / Resume / Open Output Dir); a bottom resource-monitor strip that degrades to "monitoring unavailable" on `ResourceMonitorUnavailableError`; and a dataset-library dock with add-folder / remove / re-scan-folder backed by `DatasetRegistry`. The Submit modal builds a `TrainingConfig` (framework + target-engine dropdown from `EngineRegistry.all_types()`, dataset from `DatasetRegistry.list()`, JSON hyperparameter editor) and runs an idiot-proof `CompatibilityChecker.check` **preflight** that surfaces `CompatibilityReport.errors` in a non-dismissable banner and blocks submit while errors are present. The `_TrainingControllerEmbedAdapter` implements Sub-PR A's `BackgroundableTool` hooks (`can_background`/`detach_to_window` → `True`, `pause()` no-op since the scheduler runs independently, `resume()` re-binds each live widget to scheduler status + per-job `TrainingJobLiveSubscriber` realtime progress); the launcher host surfaces reopen via its `backgrounded_tools()`. The QSS theme moved to `_style.py` to keep `gui.py` within the 1200-line budget. 98 tests pass (controller / live-subscriber / view-model headless; gui-smoke + embed-adapter under PyQt6), and `controller.py` / `live_subscriber.py` import with PyQt6 absent. Closes #6012.
...
| F16 | Model-training controller          | ✅     | In-launcher training dashboard with scheduler, dataset library, resource monitor, engine-compat gate, and ML/RL-aware stats. Backend contracts + scheduler in `src/shared/python/training/` (PRs 1–2); PyQt6 dashboard tab in `src/tools/training_controller/` (#6012), tab-backgrounding refactor (#6013), and CVAE wiring (#6014). |
